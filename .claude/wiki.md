...
## Door 502 `wrong asset/rail` is an EMPTY WALLET, never swap the model (2026-09-13)
`no door quoted grok-4.6: https://api.gondola-ai.com: wrong asset/rail (base)` → gateway 503 → shim 502 is NOT a dead model. Solana USDC ran to dust, the rail picker fell through to Base ($0.0016 there), and every door that only quotes Solana refused — grok-4.6, deepseek-v4-flash AND claude-fable-5-1 all "502"d, while `abliterated-model` (~$0.0005/call) still paid out of the dust and looked healthy, which framed it as a model problem. Funding to 259 USDC made all four return 200. proxy.js now detects `/no door quoted|wrong asset\/rail/` on a 5xx and answers 402 with the funding blurb, naming the REQUESTED model.
