...
## local-exec `exec` is `shellStreamArgs` (2026-09-03)
**SUPERSEDED detail — the case is `shellStreamArgs`, not `shellArgs`.** Grok Bot 0.36.0 `dist/local-exec-daemon/main.cjs` `buildLocalExecManager` registers exactly FOUR cases: **shellStreamArgs, backgroundShellSpawnArgs, readArgs, lsArgs**. `shellArgs` is a real wire field (ExecServerMessage #2) but is NOT wired into the local-exec lane, so it answered `...of type shellArgs`. Both carry `agent.v1.ShellArgs {command, workingDirectory, timeout}`. Set `authorizedByStanding`/`authorizedByApproval: true` — the daemon reads them `=== true` and an unattended exec has no approver. Replies: `{kind:'client', message:{shellStream:{stdout:{data}}}}` repeatedly (`data` is a proto **string**, not bytes), ending `{shellStream:{exit:{code,cwd}}}`; `{kind:'control', message:{heartbeat:{id}}}` every 3s must be IGNORED or it resolves the call empty. Original symptom: `serverMessage` is `agent.v1.ExecServerMessage`, a **oneof**, and cursorbackend sent `{command, cwd, workingDirectory}` flat — the oneof was never set, so the daemon had nothing to dispatch. Correct frame (already documented in `lib/podagent.mjs:11`, from `local-exec-daemon/main.cjs`): `{kind:'exec', requestId, approvalId, serverMessage:{shellArgs:{command, workingDirectory, timeout}}}` — `approvalId` too, exec is approval-gated. Build it with `localExecFrame()`; never hand-roll the frame. The daemon also **streams** `output`/`stdout` then closes with `result`/`exit`, and those chunks do not always echo requestId — `localExecWaiterFor` falls back to the sole in-flight waiter and buffers into `w.chunks`.
**Why:** the fix below made the failure *readable*; this is what it was actually saying. Frame timeout is set under our own ask timeout so a slow command reports the shell error, not "is Grok Bot Helper connected?".

## exec: helper frames nest stdout, and `/bin/zsh` is mac-only (2026-09-03)
Two bugs, one screenshot. (1) `handleLocalExecFrames` read `f.stdout`/`f.message` flat; the daemon nests it (measured: `message: {throw:{id,error}}` on a control frame), so `${out}` painted `[object Object]`. `normalizeExecFrame` walks `message`/`clientMessage`/`serverMessage`/`result`/`data`/`payload`/`body` (depth 4) and always yields text. (2) `execLocal` spawned `/bin/zsh -lc` on every platform — ENOENT on most Linux, no POSIX shell at all on Windows, so **every** local exec failed there. `execShell(cmd, platform)`: win32 → ComSpec `/d /s /c`, darwin → zsh `-lc`, else first existing bash → `/bin/sh -c`. Nonzero exits now return `e.stdout`/`e.stderr` instead of throwing away the output as `ERROR …`.
...
## Group queue: repeats and loop-talk are PASS (2026-09-03)
...
## `openzoo bot` is quiet by default and prints HOW TO PAY at boot (2026-09-01)
A fresh macOS user's first run was ~60 lines of `#N POST /aiserver.v1.…` / `cursor-tls` / `-> empty-ok` and never said how to pay ("onboarding is not great mate"). `lib/botlog.js`: `isBotMilestone` keeps mcp/wallet/402/outage/wakeup/error lines and drops wire noise; `--verbose` / OPENZOO_DEBUG=1 restores everything. `payBannerLines` prints Solana + Base addresses, live balance (TOKEN as units — priced only at the 402; Base USDC is raw/1e6), the Whop card link, the Chrome toggle (or "attached"), and the first message to type. Printed ~7s after launch so MCP mode is known.
**Why:** "it's not telling me straight up how to pay".

## Default is openzoo/auto, and Auto is door-only on the gateway (2026-09-01)
Grok Bot default `DEFAULT_ZOO_MODEL = 'openzoo/auto'`; `auto` / `openrouter/auto` alias to it. On the gateway, `setAutoDoorFilter` (X402_AUTO_DOORS_ONLY, default on) restricts Auto's candidates to ids an x402 door serves (`doorsForExact`, doorKey-normalised: vendor prefix off, separators normalised, `-latest` off); our virtual rows pass. The door rule then keys on the RESOLVED model, so an Auto pick is bought from its door and never falls back to OpenRouter. Catalog: OpenRouter rows shadowed by any door spelling are dropped (379 → 279 on 2026-09-01); the 279 left have no door at all.
**Why:** "NEVER OPENROUTER" + "default to openzoo AUTO" — OpenRouter credits ran dry twice in one day and every Auto pick with it.

## Door 4xx must not bench the door; try every quoting door (2026-09-01)
`anthropic/claude-fable-5.1` has three doors (surplusintelligence, aispace, gondola-base-only). aispace quotes a tool-bearing body fine but returns `400 Invalid request parameters` at BUY on a ~124 KB Grok Bot body (size/shape limit on their side). `buy()` penalised any non-200 for 60s, so the next call found zero doors → `door_no_quote` → 503, and the canvas read "x402 door … failed (502)" with no reason. Now: penalise only on 5xx/402/429; `callX402` with explicit doors tries the cheapest then every other quoting door; the 502 carries the door's message. `paid_upstream_error` evt carries `refundUsd`/`reconciledUsd` — the failed quote is reconciled, not kept.
**Why:** "something critically bad is happening?" — it was one door's body limit plus our own benching, not money loss.

## Gateway bridges what payers paid in to Base USDC for door inference (2026-09-02)
`x402-tokens/src/bridge.ts`: every settled Solana payment (USDC/TOKEN/LEOS, exact raw) is noted in `/data/bridge-owed.json`; a 60s loop flushes when owed ≥ `X402_BRIDGE_MIN_USD` (10) or operator Base USDC < `X402_BASE_FLOOR_USD` (5) and owed ≥ $2 (DLN charges ~$1.1 per order, per-call bridging is uneconomic). Flush = Jupiter swap non-USDC → USDC, then ONE deBridge DLN order USDC(Solana) → USDC(Base) to the operator (`X402_BRIDGE_RECIPIENT`, default 0x26e8…5158), signed by `X402_SVM_PAYER_KEY` (treasury = payTo = feePayer). `X402_BRIDGE=dry` (default) logs `bridge_plan`; `on` sends. TOKEN is NOT tradable on DLN (INPUT_TOKEN_NOT_TRADABLE) — that is why Jupiter comes first. Nothing not owed leaves Solana.
**Why:** operator sat at $5.31 Base USDC while $236 of TOKEN sat on the treasury; every door buy then died as "wallet underfunded" for a house problem. The human arms it (fly secrets set X402_BRIDGE=on); the assistant does not move funds.

## A bot pinned to a video/image model renders; chat on it is a 502 with an empty body (2026-09-01)
`/model ByteDance/Seedance-2.5` then "jarett" → gateway `<< zoo 502 0c` → canvas "(empty zoo reply)". Catalog rows carry `kind: "video"|"image"` + `endpoint`; `zooModelRow` / `mediaKindOf` (lib/models.js) detect it and `mediaTurn` (cursorbackend) pays `/v1/{kind}s/generations` through a PayClient (the local :8402 proxy does NOT route media), polls `/v1/videos/:id` up to 8 min, paints `🎬 url` + receipt. Also: `~/.openzoo/bot.log` now APPENDS (one `# openzoo bot run` header per run) — the minutely revive cron had restarted the hijack and the truncating open erased the turn being debugged ("why do I not see 'jarett' in server logs").
**Why:** the answer was in the previous process's stdout, not the file everyone was told to read.

## `openzoo aoe`: Agent of Empires integration is config surgery, not a registry entry (2026-09-01)
`lib/aoe.js` writes `[session.custom_agents] openzoo = "openzoo claude"` + `[session.agent_detect_as] openzoo = "claude"` + `[tools.openzoo]` (Alt+z receipts) into aoe's `config.toml` (macOS: existing `~/.config/agent-of-empires` > existing `~/.agent-of-empires` > XDG only if `XDG_CONFIG_HOME` set > legacy — mirrors aoe's `get_app_dir_path`). Keys are inserted into whichever spelling the file already uses (header table or inline `custom_agents = {…}` under `[session]`); a second header for the same table is a TOML duplicate-table error that takes aoe's WHOLE config down. Verified live with aoe 1.15.1: `aoe add --tool openzoo` created a session, `aoe ps` shows AGENT openzoo / idle, and the pane booted Claude Code with the zoo banner. Landmine: aoe's structured view runs `claude-agent-acp` directly (the wrapper is never executed), so it bills Anthropic — terminal view only. The detached proxy `openzoo aoe` starts sets `OPENZOO_NO_TUNNEL=1` unless `--tunnel`.
**Why:** aoe sessions are tmux sessions that outlive the TUI; a proxy tied to the launching process would die under them, and an unwatched proxy must not open a wallet-spending public URL.

## OpenClaw 2026.8.x: providers are bundled plugins; the shim's config writer still validates (2026-09-02)
Local CLI upgraded 2026.7.1-2 → 2026.8.2. `openclaw config validate` on a copy of the real config: our `models.providers.openzoo` block passes; the only error is the user's stale `meta.lastTouchedAt` (2026.7 leftover, `openclaw doctor --fix` migrates it — NOT run on the real config). Schema `api` enum still has `openai-completions`; model keys now also include `compat`, `contextTokens`, `mediaInput`, `metadataSource`, `thinkingLevelMap`. Every provider is now `extensions/<id>/` with `openclaw.plugin.json` (`modelCatalog.discovery: refreshable`), `provider-catalog.ts`, `provider-models.ts`; kilocode = OpenRouter-shaped `/models` pricing discovery, lmstudio = keyless local (`nonSecretAuthMarkers`, `syntheticAuthRefs`). Upstream `extensions/openzoo` PR combines the two.
**Why:** "openclaw2 is out btw fux with that" — the answer is that the writer keeps working but the plugin route is what 2026.8 users expect.

## "Invalid model provider request." = echoed upstream keys on a stored assistant turn (2026-09-02)
Two culprits, both from writing the raw upstream `message` object into history: (1) `{role:"assistant", content:null}` with no tool_calls (a failed/empty turn); (2) an assistant tool-call turn carrying `refusal:null, reasoning:"…", name:null, tool_call_id:null, function_call:null, annotations:null`. Bisected 2026-09-02 on an isolated proxy (`OPENZOO_PORT=8412 openzoo proxy`, bodies shrunk to 300 chars so each probe cost <1¢): removing that one message → 200; keeping only `role/content/tool_calls/name(string only)` → 200. `foldSameRole` (shim) and `alternateRoles` (gateway lane) now do exactly that and fold same-role neighbours. Also: an empty `finish=length` reply is said once, never re-bought (7 re-POSTs × $0.2–0.46 measured), and `reasoning_content` counts as the reply.
**Why:** three expensive replays of the full 130 KB body said nothing; shrinking first made the bisection cheap and decisive.

## (superseded) "Invalid model provider request." = an assistant message with null content and no tool_calls (2026-09-02)
The abliteration lane (and any strict OpenAI-compatible provider) answers `400 invalid_request … param=messages` with that generic text. Bisected by dumping the failing body (`~/.openzoo/zoo-last-error-body.json`, written by the hijack on every non-200) and replaying variants through :8402: size, 100 tools, tool exchanges, `tool_choice`, role runs (`auauauuuau`, `ss…`) all passed alone; removing ONE `{role:"assistant", content:null}` entry (a failed/empty turn recorded on the canvas) made the identical body a 200. `foldSameRole` (shim history) and `alternateRoles` (gateway lane) now drop those and fold same-role neighbours. Replays of a 130 KB body cost ~$0.2 each in prompt tokens — bisect by shape, not by guessing.
**Why:** three probes said "not size, not tools, not roles"; only the dump said what the provider would not.

## Codex `/model abliterated` → paid 400 "Invalid tools for this model request." (2026-09-13)
Codex 0.153.4 (`wire_api = "responses"`) sends `{type:"namespace", name, tools:[…]}` groups (multi_agent_v1, mcp__node_repl, mcp__cua_repl) plus `{type:"web_search"}`. House doors (catalog `owned_by:"openzoo"`: abliterated-model*, openzoo/auto) accept plain `function` tools only and 400 `invalid_tools` AFTER settlement (≈$0.035 per failed turn; proxy.log only shows `paid retry -> HTTP 400`). `lib/responsestools.js` flattens to `<ns>__<tool>` on the way up (tools + history `function_call` items) and restores `{name, namespace}` on `function_call` output items on the way down (JSON + SSE); unknown models learn it from one paid 400 and retry once. A flat name is NOT enough: Codex routes by (namespace, name) — flat `mcp__node_repl__js` is "unsupported call", verified against a mock upstream on :8410.
**Why:** the gateway's per-model tool validation is invisible from the catalog; test with `codex exec -c 'model_providers.openzoo.base_url="http://localhost:8412/v1"' --skip-git-repo-check … < /dev/null` (stdin must be closed or exec hangs on "Reading additional input from stdin").

## Paid/unpaid 5xx from the gateway → route to OUTAGE_FALLBACK (2026-09-13)
**SUPERSEDED — see "Door 502 `wrong asset/rail` is an EMPTY WALLET, never swap the model"**
A retired/renamed upstream door 5xxes BOTH ways: an UNPAID 500 when the id can't be priced (no x402 quote reached) and a PAID 502 after settlement. `gpt-6-astra` is live proof — the gateway maps it to `openai-gpt-6-astra`, which the upstream 404s → `{"error":"unknown model openai-gpt-6-astra"}` 500. `astra` exists NOWHERE in x402-tokens (clean tree), so it is a dead bazaar-upstream row, not a gateway code bug — a `fly deploy` fixes nothing. Left raw, Codex storms 100+ retries ("Reconnecting 5/5"). proxy.js now routes RETRYABLE_5XX (500/502/503/504) once to `OPENZOO_OUTAGE_FALLBACK` (grok-4.6) and gates the model 60s — same shape as the 402 "insufficient credits" branch but WITHOUT the `result.paid` guard (the 500 is unpaid). Verified live: `upstream 500: gpt-6-astra -> grok-4.6` → grok answered. Shipped 0.51.25.
**Why:** the fly login on this Mac (`jarettrsdunn1999@gmail.com`) only sees eat-ag/staccophono, NOT x402-tokens — the gateway can't be deployed from here anyway, and there was nothing to deploy.

## Door 502 `wrong asset/rail` is TRANSIENT, never swap the model (2026-09-13)
`no door quoted grok-4.6: https://api.gondola-ai.com: wrong asset/rail (base)` → gateway 503 → shim 502 is a DOOR-SIDE quote failure, and it is TRANSIENT — not a dead model and NOT an empty wallet. Measured five minutes apart on the same wallet: grok-4.6, deepseek-v4-flash and claude-fable-5-1 all 502'd at **264.68 USDC**, then all returned 200 at **259.00 USDC** — the balance went DOWN across the recovery, so funding was never the variable. proxy.js answers 503 naming the REQUESTED model and says retry; it must NOT print fund-me copy (that sends a caller with $260 to a top-up page — an earlier 0.51.26 build did exactly that).
**Why:** 0.51.25 instead routed any 5xx to OPENZOO_OUTAGE_FALLBACK — so `/model fable-5.1` reported `x402 door for grok-4.6 failed`, an error for a model the user never picked, and a silent swap bills them for a different model than the one they are reading. Reverted in 0.51.26: never substitute a model the caller did not choose. `gpt-6-astra` is the one genuinely dead id (`unknown model openai-gpt-6-astra`), unrelated to funding.

## Model override is PER-CHAT, keyed by Codex thread-id (2026-09-13)
`~/.openzoo/model` was one line for the whole machine, so two ChatGPT windows could not hold two models — `/model abliterated` in window A and `/model fable-5.1` in window B left BOTH on whichever was typed last, and window A then answered as fable and looked like it was lying about its own identity. Codex 0.154 sends a stable per-conversation id on every request (`thread-id` / `session-id` / `x-codex-window-id` headers, `prompt_cache_key`, and `session_id` inside `client_metadata`) — all equal within a chat, different across chats. `chatKeyFrom()` reads them in that order; overrides live in `~/.openzoo/models.json` keyed by it. Keep the `:N` pane suffix on window-id: two panes of one window are two chats. The single-line file stays the machine-wide DEFAULT (curl, `openzoo model X`, older clients) and is the fallback for a chat with no entry.
**Why:** `/model` detection was never broken on 0.154 (verified: `detectModelCommand` returns `{arg:"abliterated"}` against a real captured body) — the global file was, and the stale `gpt-6-astra` override it held made every turn 500 while the app retried.
