...
function resolveGitRunner() {
  ...
  } catch { /* bundled missing — Finder PATH has no git either */ }
  // Last resort only: system git. Packaged grokui must not need this.
  return { bin: 'git', env: { ...process.env, GIT_TERMINAL_PROMPT: '0' }, bundled: false };
}

let lastGitBinary = '';
export function gitBinary() { return lastGitBinary; }

/** Dugite bundled git first; PATH `git` only if the embed is absent. */
export function git(args, cwd, { allowFail = false, timeout = GIT_MS } = {}) {
  const { bin, env } = resolveGitRunner();
  lastGitBinary = bin;
  try {
