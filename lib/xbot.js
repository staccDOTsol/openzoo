...
import { withNamespace, withQuoteNamespace } from './namespace.js';
...
const SYSTEM_PROMPT = [
  ...
  '  false. Stay neutral on the KISS price itself — traction is fact, price',
  '  is not your call.',
  '',
  ...
  // In $KISS's own chat that read as the bot disowning the project, and the
  // room said so. A rule the model narrates is a rule that costs you the answer.
  //
  // $KISS and $LEOS are OURS — the assets openzoo settles in. Refusing to
  ...
  '$KISS and $LEOS are openzoo\'s own assets — discuss them factually and',
 ...
 * ONE settlement. $45 of KISS becomes ~6,000 questions with zero further
...
export async function ensureCredit(burner) {
  ...
  // each rail, then divide holdings by it. The gateway prices KISS at spot,
 ...
 * asset from whatever the burner holds (USDC / KISS / LEOS), which is why the
...
export async function askZooPaid(question, { burner, thread = '', maxTokens = ANSWER_TOKENS, contextId = '' } = {}) {
  ...
  return runToolLoop({
    ...
    // The operator's archive belongs to its signer. Burner calls use only
    // the explicitly supplied thread; they cannot attach that private context.
    call: async (body) => (await pay.chat(body)).data,
  }).then((result) => ({
    ...result,
...
/** $KISS and $LEOS are OURS. The guard exists to stop the bot pumping
 *  STRANGERS' coins, not to gag it about the project it runs on. */
const OWN_TICKERS = String(process.env.OPENZOO_XBOT_OWN_TICKERS || 'KISS,LEOS')
 ...
 * a $KISS buy alert got "I do not announce or promote tokens." — the bot
 ...
 * about $KISS/$LEOS is not that: they are the thing it runs on, its own
...
export function refuseShill(answer) {
  ...
  return foreign.length
    ...
    : "I don't do launch hype, including for $KISS. Ask me what it actually does instead.";
}
 ...
 *
 * CAs for KISS and LEOS, none for USDC — deliberate. USDC is resolved by every
 * wallet from the ticker alone, while KISS and LEOS are not, and "send KISS"
...
export function composePaywallReply(authorId, tweetId, { address, returning = false, quotedUsd = 0 } = {}) {
  ...
  const token = FUNDING_ASSETS.find((a) => a.symbol === 'KISS');
  ...
  return [
    ...
    // The burner does not just receive: it WRAPS what you send (KISS -> wTOKENx)
    // and signs, and both are transactions it pays for itself. Funded with
    // KISS alone it holds a balance it cannot spend, and the reply it gets
