...
function grokBotImageNames(bin = grokBotBinary()) {
  const names = new Set(['Grok Bot.exe', 'Grok_Bot.exe', 'grokbot.exe']);
...
export function grokBotPids(run = execSync, platform = process.platform) {
  const quiet = { encoding: 'utf8', shell: true, windowsHide: true, stdio: ['pipe', 'pipe', 'pipe'] };
  try {
    if (platform === 'win32') {
      const pids = [];
      for (const name of grokBotImageNames()) {
        try {
          const out = run(`tasklist /FI "IMAGENAME eq ${name}" /FO CSV /NH`, quiet);
          pids.push(...parseTasklistPids(out));
        } catch { /* image not running */ }
      }
      return [...new Set(pids)];
    }
    return run(`pgrep -f ${JSON.stringify(grokBotBinary())}`, quiet)
...
function pidHasNeedle(pid, needle, run = execSync, platform = process.platform) {
  const n = Number(pid);
...
function somethingTalksToPort(port, run = execSync, platform = process.platform) {
  try {
    if (platform === 'linux') {
...
export function inspectGrokBotHijack(url, port = 8443, run = execSync, cdpPort = GROKBOT_CDP_PORT, platform = process.platform) {
  const pids = grokBotPids(run, platform);
  const needle = `CURSOR_API_BASE_URL=${url}`;
  const envHit = pids.some((pid) => pidHasNeedle(pid, needle, run, platform));
  const tcpHit = somethingTalksToPort(port, run, platform);
  const cdpNeedle = `--remote-debugging-port=${Number(cdpPort)}`;
  const chipDebug = pids.some((pid) => pidHasNeedle(pid, cdpNeedle, run, platform));
...
/**
 * Turn OFF the local-tool approval prompt so exec/list_dir actually run.
 *
 * `localUseRefusal` in dist/local-exec-daemon/main.cjs only honours the
 * frame's `authorizedByStanding` when the daemon is `serverAuthoritative`;
 * ours is not, so it falls through to the user's own setting and answers
 * "That action was not approved on the user's computer, so nothing ran."
 * The setting is `localToolPermission` in `<userData>/settings.json`,
 * default "ask", and `localToolPermissionCeiling` CAPS it
 * (resolveSandLocalToolPermission takes the lower of {never:0,ask:1,always:2})
 * — so raising the choice without also clearing a "never"/"ask" ceiling
 * changes nothing.
 */
export function seedGrokBotLocalToolPermission({
  home = homedir(),
  env = process.env,
  platform = process.platform,
  permission = 'always',
  ...
  for (const dir of grokBotUserDataDirs(home, env, platform)) {
    ...
    try {
      const j = JSON.parse(readFileSync(p, 'utf8'));
      if (j && typeof j === 'object') prev = j;
    } catch { /* missing or junk — the app writes it on first settings change */ }
    const next = { ...prev, localToolPermission: permission };
    // A ceiling of "ask"/"never" would silently clamp us straight back.
    if (permission === 'always') delete next.localToolPermissionCeiling;
    // The app writes this file with JSON.stringify(e, null, 2).
    writeFileSync(p, JSON.stringify(next, null, 2), { encoding: 'utf8', mode: 0o600 });
    written.push(p);
  }
  return { written, permission };
}
...
export async function runBot(argv = []) {
  ...
  try {
    const perm = seedGrokBotLocalToolPermission();
    console.error(`openzoo: local-tool permission=always → ${perm.written.length} settings.json`);
  } catch (e) {
    console.error(`openzoo: local-tool permission seed failed (${e.message}) — exec will say "not approved"`);
  }
