 ...
 * ({ USDC, KISS } in USD) — printed as `?` when unknown so nothing is invented.
 */
export const MINTS = {
  USDC: 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v',
  KISS: '7K2iAPzHddrghwBF7S7oA9qHr7dDR4QvFmdD1sZgRJxF',
...
export function payBannerLines({ solana, evm, balances = null, whop = 'https://whop.com/staccoverflow/openzoo', chromeMode = 'own-profile', mints = MINTS } = {}) {
  ...
  const lines = [
    ...
    `                   USDC   ${mints.USDC}`,
    `                   KISS  ${mints.KISS}   (half price)`,
    `                   LEOS   ${mints.LEOS}   (half price)`,
    `         Base    ${evm}`,
    `                   USDC   ${mints.BASE_USDC}`,
    balances ? `         balance Solana USDC ${usd(balances.USDC)} · KISS ${units(balances.TOKEN_UNITS)} · LEOS ${units(balances.LEOS_UNITS)} · Base USDC ${usd(balances.BASE_USDC)}   (recheck: openzoo balance)` : '         balance unknown right now — run: openzoo balance',
...
/** Live balances for the banner: Solana USDC/KISS/LEOS + Base USDC. Never throws; unknowns are null. */
export async function quickBalances(addrs, { timeoutMs = 6000 } = {}) {
  ...
  try {
    ...
    const [u, t, l] = await within(Promise.all(['USDC', 'KISS', 'LEOS'].map((sym) => (find(sym) ? tokenBalance(conn, w.keypair.publicKey, find(sym).mint) : { ui: 0 }))), timeoutMs);
