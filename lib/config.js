...
export const config = {
  ...
  // KISS is the default so a wallet holding it spends it. The picker walks
  // best-first and takes the first AFFORDABLE row, so wallets without KISS
  ...
  token: process.env.OPENZOO_TOKEN || 'KISS',
...
/**
 * RPC endpoint BY CHAIN ID — the only lookup a 402 row can be resolved with.
 *
 * A row names its chain as `eip155:<id>` and nothing else. The rail NAMES
 * above are a coarser bucket: railOf() collapses every chain that is not Base
 * or Robinhood into the single name "evm", so a lookup keyed by rail name
 * cannot tell Polygon from Arbitrum and answers with whatever the fallback
 * branch happens to be.
 *
 * MEASURED 2026-09-09: the payment preflight did exactly that — for any
 * non-Base EVM row it read the balance over the ROBINHOOD RPC, asked a Polygon
 * USDC address that is not a contract there, threw, was swallowed by the
 * preflight's catch, and signed an authorization for a chain the wallet held
 * nothing on. The facilitator then reverted it and the caller was told
 * "execution reverted" while 76 USDC sat on Base. Keyed by chain id, that
 * cannot happen: either we can read the chain, or we say we cannot.
 */
export const EVM_RPC_BY_CHAIN = {
  8453: () => config.baseRpcUrl,
  4663: () => config.rhRpcUrl,
  137: () => process.env.OPENZOO_POLYGON_RPC || 'https://polygon-rpc.com',
  42161: () => process.env.OPENZOO_ARBITRUM_RPC || 'https://arb1.arbitrum.io/rpc',
  10: () => process.env.OPENZOO_OPTIMISM_RPC || 'https://mainnet.optimism.io',
  480: () => process.env.OPENZOO_WORLD_RPC || 'https://worldchain-mainnet.gateway.tenderly.co',
};

/** RPC for a row's `eip155:<id>` network, or null when we cannot read that chain. */
export function evmRpcForNetwork(network) {
  const m = /^eip155:(\d+)$/.exec(String(network || ''));
  if (!m) return null;
  const f = EVM_RPC_BY_CHAIN[Number(m[1])];
  return f ? f() : null;
}
...
/**
 * "USDC or KISS on Solana · USDC on Base" — the funding hint for exactly the
...
export async function liveRails() {
  const { withQuoteNamespace } = await import('./namespace.js');
  const r = await fetchHeaders(`${config.apiBase}/v1/chat/completions`, {
    method: 'POST',
    headers: withQuoteNamespace({ 'content-type': 'application/json' }),
