...
async function runTurn(threadId, userText, onEvent, images) {
    ...
    for (const m of t.members) {
      ...
      if (runCmd) {
        ...
        if (t.runMode === 'auto') {
          ...
          const shown = `$ ${command}\n${output}`;
          t.history.push({ who: 'bot', text: shown, name: m.name, color: m.color, thinking: memberThink.thinking });
          paint({ type: 'final', name: m.name, color: m.color, text: shown, thinking: memberThink.thinking });
          memberReply = shown;
  ...
  if (runCmd) {
    ...
    if (t.runMode === 'auto') {
      ...
      if (!stillMine()) return;
      const shown = `$ ${command}\n${output}`;
      t.history.push({ who: 'bot', text: shown, thinking: settled.thinking });
      paint({ type: 'final', name: t.name, color: t.color, text: shown, thinking: settled.thinking });
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Always-on strip. ◎ stays a toggle; this one never hides. Bottom-left
     above #bar, under #hud, and pointer-events none so it cannot steal
     compose / bubble clicks. */
  #dockHud { position: absolute; left: 14px; bottom: 72px; z-index: 200;
    pointer-events: none; background: rgba(14,14,17,.94); border: 1px solid #333340;
    border-radius: 8px; padding: 4px 8px; color: #8e8e93;
...
<script>
  ...
  async function render() {
    ...
    for (const h of full.history) {
      addRow(h.who, h.text, h.color || t.color, h.name || t.name,
        h.runId ? { id: h.runId, status: h.runStatus, output: h.runOutput } : undefined, h.images, h.thinking);
    }
  ...
  function placeDockHud() {
    const dock = document.getElementById('dockHud');
    const bar = document.getElementById('bar');
    if (!dock || !bar || !mainEl) return;
    const gap = bar.getBoundingClientRect().height + 8;
    dock.style.bottom = Math.max(72, Math.round(gap)) + 'px';
  }
  function fmtDockX(n) {
  ...
  if (typeof ResizeObserver !== 'undefined') {
    ...
    if (barEl) new ResizeObserver(() => placeDockHud()).observe(barEl);
  }
