...
function grokroomSend(t, task) {
  ...
  t.roomEcho = Array.isArray(t.roomEcho) ? t.roomEcho : [];
  t.roomEcho.push(body);
  if (t.roomEcho.length > 50) t.roomEcho = t.roomEcho.slice(-50);
  t.lastActivityAt = Date.now();
  saveThreads();
  grokroomPost(t.room.id, body).catch((e) => {
    const i = (t.roomEcho || []).indexOf(body);
    if (i !== -1) t.roomEcho.splice(i, 1); // never landed — don't skip it if it shows up later
...
function grokroomEnsureThread(roomId, name, addr) {
  ...
  const t = newGroupThread([]);
  t.name = name || ('# ' + roomId);
  t.color = colorFor(t.name);
  t.members = [];
  ...
  try {
    ...
    for (const r of rooms) {
      ...
      for (const m of msgs) {
        ...
        if (!body) continue;
        // my own sends already echoed locally as the human's bubble — skip once
        if (from === GROKROOM_HANDLE && Array.isArray(t.roomEcho) && t.roomEcho.indexOf(body) !== -1) {
          t.roomEcho.splice(t.roomEcho.indexOf(body), 1);
          dirty = true;
          continue;
        }
        grokroomMemberEnsure(t, from);
        t.history.push({ who: 'bot', name: from, text: body, sig: msig, ts: m.ts || Date.now() });
        if (m.ts && m.ts > (t.lastActivityAt || 0)) t.lastActivityAt = m.ts;
        dirty = true;
      }
      if (dirty) saveThreads();
    }
  } catch { /* room server down — next tick retries */ }
