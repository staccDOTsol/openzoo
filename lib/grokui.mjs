...
import { ensureHarness, getHarnessState, kickHarnessAutostart, shouldSkipHarnessAutostart } from './harness-install.js';
...
async function runAutoClaudeTurn(t, userText, images, paint, stillMine, turnAbort) {
  ...
  if (!shouldSkipHarnessAutostart() && !harness.ready) {
    paint({ type: 'start', name: t.name, color: t.color, detail: harness.message || 'Installing Claude…' });
    await ensureHarness({
      onStatus(s) {
        if (!stillMine()) return;
        paint({ type: 'status', name: t.name, color: t.color, detail: s.message });
      },
    });
  }
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  #harnessBar { display: none; padding: 6px 20px; font-size: 12px; font-weight: 500; color: #f28c4d;
                background: #1a120c; border-bottom: 1px solid #3a2418; }
  #harnessBar.show { display: block; }
...
<body>
  ...
  <div id="main">
    ...
    <div id="harnessBar" hidden data-component="harness-status"></div>
    <div id="hud">
...
<script>
  ...
  function placeHud() {
    ...
    const headerBottom = chatHeader.getBoundingClientRect().bottom;
    const barBottom = bar && bar.classList.contains('show') ? bar.getBoundingClientRect().bottom : headerBottom;
    const top = Math.max(headerBottom, barBottom) - mainEl.getBoundingClientRect().top + 8;
    hud.style.top = Math.max(8, Math.round(top)) + 'px';
  }
  async function refreshHarnessBar() {
    const bar = document.getElementById('harnessBar');
    if (!bar) return;
    try {
      const h = await (await fetch(API + '/harness', { cache: 'no-store' })).json();
      if (h && (h.installing || (h.message && !h.ready))) {
        bar.hidden = false;
        bar.classList.add('show');
        bar.textContent = h.message || 'Installing Claude…';
      } else {
        bar.hidden = true;
        bar.classList.remove('show');
      }
      placeHud();
    } catch (e) { /* first-run bar is advisory */ }
  }
  setInterval(refreshHarnessBar, 1500);
  refreshHarnessBar();
  function fmtDockX(n) {
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/harness') {
    res.writeHead(200, { 'content-type': 'application/json' });
    res.end(JSON.stringify(getHarnessState()));
    return;
  }
  ...
  if (req.method === 'POST' && req.url === '/drive') {
    ...
    req.on('end', async () => {
      ...
      if (modeCmd && t) {
        ...
        else void kickHarnessAutostart();
...
server.listen(PORT, BIND, () => {
  ...
  // Window / /threads must answer first. Harness install continues after paint
  // (same rule as :8402 — do not await Claude/Node/openzoo before listen).
  void kickHarnessAutostart();
});
