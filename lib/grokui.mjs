...
import { adaptiveTopK, brain, brainRace, brainStream, tierModels, MODEL, PROXY, TIER_NAMES, normalizeTier, setCompletionsFetchGiveUp } from './podagent.mjs';
import { fetchRetry, isRetryableFetchError } from './fetch.js';
import { routeChatBody } from './modelroute.js';
import { config } from './config.js';
import { claudeZooEnv, resolveOpenzooClaude } from './launch.js';
import { findPackedNodePty, loadNodePtyFrom, resolvePackedOpenzooClaude } from './packed-runtime.js';
...
import { openStoredIdeSession, publicIdeSession } from './hosted-ide.js';
import {
...
import {
  runClaudeCode, setClaudeRunnerForTest, claudeRunnerOverridden, toolStatusLine, CLAUDE_MISSING,
  sanitizeClaudeCanvas, closeClaudeSession, GROKUI_RESERVED_SLASH, claudeModelArg,
  claudeInteractiveArgs, PTY_PENDING, hasLiveClaudeSession,
} from './claudecode.js';
import {
  isClaudeSubagentTool, claudeSubagentSpec, claudeSubagentHopText,
} from './grokui-subagents.js';
import { ensureHarness, shouldSkipHarnessAutostart } from './harness-install.js';

const LIB_DIR = path.dirname(fileURLToPath(import.meta.url));
const VENDOR_DIR = path.join(LIB_DIR, 'vendor');
const VENDOR_FILES = new Set(['xterm.js', 'xterm.css', 'fit.js']);
...
/** Paths OCC / Chat print when they write HTML. */
const HTML_WRITE_RE = /\b(?:File written|File updated|Wrote|Edited|MULTIEDIT|Write|Edit)\s*:?\s+([^\n]+?\.(?:html?|HTML?))/gi;
const HTML_PATH_RE = /(?:^|[\s:="'`(])((?:~|\/|\.\/)[^\s"'<>]+\.(?:html?|HTML?)|[\w./@$-]+\.(?:html?|HTML?))/g;

function cleanHtmlPathHit(raw) {
  return String(raw || '').trim().replace(/[.,;:)"'\]]+$/, '');
}

/** Relpath of a previewable .html inside the thread dir, or ''. */
function relInThreadDir(threadId, spec) {
  const raw = cleanHtmlPathHit(spec);
  if (!raw || !isPreviewableRel(raw)) return '';
  const cwd = dirFor(threadId);
  if (!cwd) return isPreviewableRel(raw) && !raw.includes('/') && !raw.includes('\\') ? raw : '';
  try {
    const root = path.resolve(expandHome(cwd));
    const full = path.isAbsolute(expandHome(raw)) || raw.startsWith('~') || raw.startsWith('./')
      ? path.resolve(expandHome(raw))
      : path.resolve(root, raw);
    if (full !== root && !full.startsWith(root + path.sep)) return '';
    if (!existsSync(full)) return '';
    return full === root ? 'index.html' : full.slice(root.length + 1).split(path.sep).join('/');
  } catch {
    return '';
  }
}
...
function htmlRelFromText(text, threadId) {
  const s = stripAnsi(text);
  const hits = [];
  HTML_WRITE_RE.lastIndex = 0;
  let m;
  while ((m = HTML_WRITE_RE.exec(s))) hits.push(m[1]);
  HTML_PATH_RE.lastIndex = 0;
  while ((m = HTML_PATH_RE.exec(s))) hits.push(m[1]);
  for (const raw of hits) {
    const rel = relInThreadDir(threadId, raw);
    if (rel) return rel;
    const base = path.basename(cleanHtmlPathHit(raw));
    if (threadId && isPreviewableRel(base)) {
      const inside = relInThreadDir(threadId, base);
...
function walkHtmlRels(root, maxDepth = 4) {
  const out = [];
  const walk = (dir, rel, depth) => {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) { walk(full, nextRel, depth + 1); continue; }
      if (!isPreviewableRel(e.name)) continue;
      let mtime = 0;
      try { mtime = statSync(full).mtimeMs; } catch { /* skip */ }
      out.push({
        rel: nextRel,
        depth: nextRel.split('/').length,
        index: /^index\.html?$/i.test(e.name),
        mtime,
      });
    }
  };
...
/** Shallowest index.html, else newest .html under the thread cwd. */
function findPlayableHtmlRel(threadId) {
  const root = dirFor(threadId);
  if (!root || !existsSync(root)) return '';
  const rows = walkHtmlRels(root);
  if (!rows.length) return '';
  const indexes = rows.filter((r) => r.index).sort((a, b) => a.depth - b.depth || b.mtime - a.mtime);
  if (indexes.length) return indexes[0].rel;
  rows.sort((a, b) => b.mtime - a.mtime || a.depth - b.depth);
  return rows[0].rel;
}

async function emitAgentPreview(threadId, rel) {
  if (!threadId || !isPreviewableRel(rel)) return null;
  const port = await ensureWorkspacePort();
  if (!port) return null;
  const ev = { type: 'preview', rel, url: workspaceFileUrl(threadId, rel) };
  emitToThread(threadId, ev);
  return ev;
}

const lastAgentPreviewStamp = new Map();

function previewStamp(threadId, rel) {
  try {
    const root = dirFor(threadId);
    return `${rel}:${statSync(path.join(root, rel)).mtimeMs}`;
  } catch {
    return rel;
  }
}

function maybeEmitAgentPreview(threadId, text) {
  const clean = stripAnsi(text);
  let rel = htmlRelFromText(clean, threadId);
  if (!rel && /(?:Wrote|Edited|File written|File updated|index\.html|\.html?\b)/i.test(clean)) {
    rel = findPlayableHtmlRel(threadId);
  }
  if (!rel) return;
  const stamp = previewStamp(threadId, rel);
  if (lastAgentPreviewStamp.get(threadId) === stamp) return;
  lastAgentPreviewStamp.set(threadId, stamp);
  void emitAgentPreview(threadId, rel);
}
...
// Quiet Claude TUI / chrome-only canvas / pending PTY. Auto never falls
// through to Ask/chat-completions — these are retry-or-paint states.
function isClaudeFallbackReply(text) {
  const s = String(text || '').trim();
  if (!s) return true;
  if (s === '(no response)') return true;
  if (s === PTY_PENDING || s === CLAUDE_MISSING) return true;
  if (/^(?:error:\s*)?upstream HTTP \d+\s*$/i.test(s)) return true;
  if (/openzoo-claude CLI not found/i.test(s)) return true;
  if (/openzoo-claude is installing/i.test(s)) return true;
  if (/^Auto is starting/i.test(s)) return true;
  return false;
}
function isAutoPtyPending(text) {
  const s = String(text || '').trim();
  return s === PTY_PENDING || s === CLAUDE_MISSING || /^Auto is starting/i.test(s)
    || /openzoo-claude is installing/i.test(s);
}
...
function shouldKeepAuto(t, reply, userText) {
  if (!t || !isAgentMode(t)) return false;
...
const SIDECAR_SESSION_URL = process.env.OZ_SIDECAR_SESSION_URL || 'http://127.0.0.1:8402/v1/session';
const SIDECAR_STARTING = 'sidecar starting…';
const SIDECAR_WAIT_MS = Number(process.env.OZ_SIDECAR_WAIT_MS || 90_000);
const SIDECAR_POLL_MS = Number(process.env.OZ_SIDECAR_POLL_MS || 500);
let sidecarSessionOverride = null;
function setSidecarSessionForTest(fn) {
  sidecarSessionOverride = typeof fn === 'function' ? fn : null;
}

function noteSidecarUnreachable() {
  for (const t of threads.values()) t.liveStatus = SIDECAR_STARTING;
  for (const t of threads.values()) {
    try { emitToThread(t.id, { type: 'status', detail: SIDECAR_STARTING }); } catch { /* later */ }
  }
}
setCompletionsFetchGiveUp(noteSidecarUnreachable);

/** Occupied TCP ≠ healthy. 402 (Pay) is healthy. One 2s probe — heal, not retry-storm. */
async function sidecarSessionReady() {
  if (sidecarSessionOverride) return sidecarSessionOverride();
  try {
    const r = await fetch(SIDECAR_SESSION_URL, { signal: AbortSignal.timeout(2500) });
    return r.ok || r.status === 402;
  } catch { return false; }
}

async function waitForSidecarSession(paint, stillMine, turnAbort, t) {
  // Injected Claude/brain backends do not talk to :8402. Do not block those
  // unit tests for 90s — unless a test explicitly overrides the session probe.
  if (!sidecarSessionOverride && (claudeRunnerOverridden() || brainAskOverride)) {
    return true;
  }
  if (await sidecarSessionReady()) return true;
  if (t) t.liveStatus = SIDECAR_STARTING;
  paint?.({ type: 'status', name: t?.name, color: t?.color, detail: SIDECAR_STARTING });
  const deadline = Date.now() + SIDECAR_WAIT_MS;
  while (Date.now() < deadline) {
    if (!stillMine?.() || turnAbort?.signal?.aborted) return false;
    await new Promise((r) => setTimeout(r, SIDECAR_POLL_MS));
    if (await sidecarSessionReady()) return true;
    if (t) t.liveStatus = SIDECAR_STARTING;
    paint?.({ type: 'status', name: t?.name, color: t?.color, detail: SIDECAR_STARTING });
  }
  return sidecarSessionReady();
}

/** After :8402 is back, retry the same send. Do not park SIDECAR_STARTING. */
async function recoverSidecarAsk(err, askFn, paint, stillMine, turnAbort, t) {
  if (!(isRetryableFetchError(err) || /fetch failed/i.test(String(err && err.message)))) {
    return `error: ${err.message}`;
  }
  noteSidecarUnreachable();
  const up = await waitForSidecarSession(paint, stillMine, turnAbort, t);
  if (!stillMine()) return '';
  if (!up) return SIDECAR_STARTING;
  try {
    return await askFn();
  } catch (e2) {
    if (isRetryableFetchError(e2) || /fetch failed/i.test(String(e2 && e2.message))) {
      noteSidecarUnreachable();
      return SIDECAR_STARTING;
    }
    return `error: ${e2.message}`;
  }
}

function visibleBotCount(t) {
  return (t?.history || []).filter(isVisibleBotReply).length;
}

function autoClaudeTurnProducedVisible(t, botsBefore, lastReply) {
  if (isClaudeFallbackReply(lastReply)) return false;
  return visibleBotCount(t) > botsBefore;
}
...
const SLASH_COMMANDS = [
  ...
  { name: '/mode', args: 'chat|agent', help: 'Agent = cloud IDE (zoo.openzoo.fun/api/ide/session); Chat = completions (ask|auto still work)' },
];

/** Claude TUI slashes that grokui must not intercept in Agent mode. /model is
 *  Claude's picker here — grokui's /model pin stays on Chat. */
const CLAUDE_SLASH_IN_AUTO = new Set(['agents', 'tasks', 'context', 'model', 'goal']);

/** Canonical run modes: chat | agent. Old threads may still store ask | auto. */
...
function isAgentMode(tOrMode) {
  const m = (tOrMode && typeof tOrMode === 'object') ? tOrMode.runMode : tOrMode;
  return normalizeRunMode(m) === 'agent';
}
function runModeLabel(tOrMode) {
  return isAgentMode(tOrMode) ? 'agent' : 'chat';
}
...
function isGrokuiOwnedSlash(line, runMode) {
  ...
  if (isAgentMode(runMode) && CLAUDE_SLASH_IN_AUTO.has(cmd)) return false;
  return SLASH_COMMANDS.some((c) => c.name.slice(1).toLowerCase() === cmd)
    || cmd === 'pay' || cmd === 'hud';
}

/** One live OCC Ink PTY per thread. Reuse if cur && !cur.dead. Never kill
 *  on cwd mismatch — only /dir (and leaving Agent) kills and respawns. */
const agentPtys = new Map();
const AGENT_PTY_BUF_CAP = 512 * 1024;
let agentPtySpawner = null;

function setAgentPtySpawnerForTest(fn) {
  agentPtySpawner = typeof fn === 'function' ? fn : null;
}

function killAgentPty(threadId) {
  const cur = agentPtys.get(threadId);
  if (!cur) return;
  agentPtys.delete(threadId);
  cur.dead = true;
  try { cur.kill(); } catch { /* gone */ }
}

/** Packed occ first. Path / npx only if the dmg/exe/AppImage has no tree. */
function agentPtySpawnSpec({
  env = process.env, execPath = process.execPath, model,
} = {}) {
  const resolved = resolvePackedOpenzooClaude({ env, execPath }) || resolveOpenzooClaude(env);
  if (!resolved) return null;
  const ptyEnv = claudeZooEnv({ ...env }, { port: Number(env.OPENZOO_PORT || config.port) });
  ...
  return {
    ...
    args: [...resolved.prefixArgs, ...claudeInteractiveArgs({ model: claudeModelArg(model), system: '' })],
    via: resolved.via,
...
function loadAgentNodePty() {
  return loadNodePtyFrom(findPackedNodePty())
    || loadNodePtyFrom(path.join(LIB_DIR, '..', 'node_modules', 'node-pty'))
    || loadNodePtyFrom(path.join(LIB_DIR, '..', 'grokui-app', 'node_modules', 'node-pty'));
}

function defaultSpawnAgentPty(spec, { cwd, cols, rows } = {}) {
  const pty = loadAgentNodePty();
  if (!pty?.spawn) {
...
function ensureAgentPty(tOrId, opts = {}) {
  const t = (tOrId && typeof tOrId === 'object') ? tOrId : threads.get(tOrId);
  if (!t) return null;
  const cur = agentPtys.get(t.id);
  ...
  if (!spec) return null;
  const cwd = t.dir || WORKSPACE_DIR;
  let handle;
  try {
    handle = (agentPtySpawner || defaultSpawnAgentPty)(spec, {
      cwd, cols: opts.cols || 80, rows: opts.rows || 24,
    });
  } catch {
    return null;
  }
  const sess = {
    ...
    args: spec.args,
    write: (s) => handle.write(s),
    resize: (c, r) => handle.resize?.(c, r),
    kill: () => handle.kill(),
  };
  handle.onData((chunk) => {
    const buf = Buffer.isBuffer(chunk) ? chunk : Buffer.from(String(chunk), 'utf8');
    sess.buf = Buffer.concat([sess.buf, buf]);
    if (sess.buf.length > AGENT_PTY_BUF_CAP) {
      sess.buf = sess.buf.subarray(sess.buf.length - AGENT_PTY_BUF_CAP);
    }
    emitToThread(t.id, { type: 'pty', data: buf.toString('base64') });
    const tail = sess.buf.subarray(Math.max(0, sess.buf.length - 8000)).toString('utf8');
    maybeEmitAgentPreview(t.id, tail);
  });
  handle.onExit(() => { sess.dead = true; });
  agentPtys.set(t.id, sess);
  return sess;
}

function writeAgentPty(threadId, data) {
  const sess = ensureAgentPty(threadId);
  if (!sess) return false;
  sess.write(data);
  return true;
}

/** Trailing CR/LF only. Never strip a leading `/`. */
function stripPtyLineTail(s) {
  let out = String(s ?? '');
  while (out.length) {
...
/** OCC slashes are commands only at a `>` prompt. Look at the last visible
 *  line after stripping ANSI so Ink color codes do not hide it. */
function ptyLooksReady(sess) {
...
function markGoalSet(threadId, text) {
  if (!/^\/goal\b/i.test(text)) return;
  const t = threads.get(threadId);
  if (!t || t.goalSet) return;
  t.goalSet = true;
  saveThreads();
}
...
/** After ESC, do not trust a stale `>` still in the buf. Wait until the
 *  buffer grows AND then looks ready. Timeout ~2.5s, then the caller writes. */
...
/** OCC slashes (`/goal` `/model` `/help` …) MUST be the first token at `>`.
 *  Writing ESC then `/goal` in one burst ate the first `/` (OCC saw `goal`).
 *  Regular text: first write is line+CR immediately; later Esc, 80ms, line+CR.
 *  Do not wait for `>` on normal text (that ate `hi`). */
async function writeAgentPtyLine(threadId, line) {
  const sess = ensureAgentPty(threadId);
  if (!sess) return false;
  const text = stripPtyLineTail(line);
  markGoalSet(threadId, text);
...
function agentTierModelLine(tier) {
  if (tier === 'auto') return '/model openzoo/auto';
  if (tier === 'grok4.6') return '/model x-ai/grok-4.6';
  return '';
}
...
async function sessionStats() {
  try {
    const s = await (await fetchRetry(`${PROXY}/session`, { signal: AbortSignal.timeout(2000) }, { onGiveUp: noteSidecarUnreachable })).json();
...
async function handleSlash(task, t) {
  ...
  if (cmd === 'tier') {
    ...
    const prev = t.tier || 'auto';
    t.tier = want; saveThreads();
    // Only when the user actually changes the dial — never on render/tick
    // re-applying the same /tier (that dumped /model into a busy TUI).
    if (isAgentMode(t) && prev !== want) {
      const line = agentTierModelLine(want);
      if (line) await writeAgentPtyLine(t.id, line);
    }
    if (want === 'auto') {
  ...
  if (cmd === 'models') {
    try {
      const list = await (await fetchRetry(`${PROXY}/models`)).json();
  ...
  if (cmd === 'all') {
    ...
    if (isAgentMode(t) && pingCanWake(t)) wakeOnPing(t, arg);
  ...
  if (cmd === 'ping') {
    ...
    return crew.map((x) => {
      ...
      if (x.id === t.id) {
        if (isAgentMode(t) && pingCanWake(t)) {
          wakeOnPing(t, arg);
  ...
  if (cmd === 'mode') {
    if (!arg) {
      return `Run mode is ${runModeLabel(t)}.`
        + (isAgentMode(t)
          ? ' Agent = cloud IDE (code-server + Cline) via POST/GET /api/ide/session. No subscription key → locked / Pay. Local OCC TUI can stay for Chat-less use.'
          : ' Chat = completions; RUN: waits for your approval.')
        + ' Switch with /mode chat or /mode agent (ask|auto still work).';
    }
    const next = normalizeRunMode(arg.split(/\s+/)[0]);
    if (!next) return 'Usage: /mode chat|agent  (ask|auto still work)';
    t.runMode = next;
    if (next !== 'agent') {
      closeClaudeSession(t.id);
      killAgentPty(t.id);
    }
    saveThreads();
    return next === 'agent'
      ? 'Run mode set to agent — cloud IDE at zoo.openzoo.fun/api/ide/session. Subscribe (Pay) if there is no key. Local OCC is not the Agent pane.'
      : 'Run mode set to chat — completions; RUN: waits for your approval.';
  }
  return null; // not one of ours — /dir falls through to its handler
}
...
function childrenOf(parentId) {
  return [...threads.values()].filter((x) => x.parent === parentId);
}

/**
 * Claude Task / Agent → the same sidebar thread SPAWN: creates.
 * Nested Task on a child Auto turn becomes a grandchild. Repeat name
 * reuses the existing crew member (wake), never a second identical row.
 */
function adoptClaudeSubagent(parent, spec) {
  if (!parent || !spec?.name) return null;
  const base = String(spec.name).trim();
  if (!base) return null;
  const kids = childrenOf(parent.id);
  let child = kids.find((c) => c.name.toLowerCase() === base.toLowerCase());
  let fresh = false;
  if (!child) {
    const existing = findByName(base);
    if (existing && existing.id !== parent.id && rootOf(existing).rootId === rootOf(parent).rootId) {
      child = existing;
    }
  }
  if (!child) {
    if (kids.length >= SPAWN_MAX_CHILDREN) return null;
    let name = base;
    let n = 2;
    while (findByName(name) && findByName(name).id !== parent.id) {
      name = `${base} ${n++}`;
    }
    if (name.toLowerCase() === String(parent.name || '').toLowerCase()) {
      name = `${base} 2`;
      while (findByName(name)) name = `${base} ${++n}`;
    }
    child = newThread(name, parent.id, undefined, spec.task || name);
    fresh = true;
  }
  if (child.status === 'thinking') return { child, fresh: false };
  const task = String(spec.task || '').trim();
  if (fresh) {
    kickTurn(child.id, childKickoff(parent, child.name, task || child.name, { fresh: true })).catch(() => {});
  } else if (task) {
    attachChildDir(child, parent, task);
    wakeOnPing(child, task);
  } else {
    wakeOnPing(child);
  }
  return { child, fresh };
 ...
 */
const ENSURE_HARNESS_SEND_MS = 2500;

// Orange Auto is Claude Code on a PTY. waitIdle hard-caps ~90s (spinner /
// think / keepFold must not reset it). Idle+text is not goal-done — stay
// on the live PTY (stayLive) so the harness can keep looping. Do not invent
// '(no response)' so Ask/completions steal the send. turnAbort still cancels
// the PTY; do not abort it for a timer.
async function runClaudeCodeBounded(opts) {
  const ptyAbort = new AbortController();
  const turnSignal = opts?.signal;
  const onTurnAbort = () => { try { ptyAbort.abort(); } catch { /* already */ } };
  if (turnSignal) {
    if (turnSignal.aborted) onTurnAbort();
    else turnSignal.addEventListener?.('abort', onTurnAbort, { once: true });
  }
  try {
    return await runClaudeCode({ ...opts, signal: ptyAbort.signal });
  } finally {
...
async function runAutoClaudeTurn(t, userText, images, paint, stillMine, turnAbort) {
  ...
  const paintHops = () => {
    const hopText = claudeSubagentHopText(adopted);
    if (!hopText) return;
    paint({ type: 'status', name: t.name, color: t.color, detail: hopText });
    paint({ type: 'spawn', name: t.name, color: t.color, detail: hopText });
  };
  const takeTool = (name, input) => {
    if (isClaudeSubagentTool(name)) {
      const spec = claudeSubagentSpec(name, input);
      if (adopted.some((h) => h.child.name.toLowerCase() === spec.name.toLowerCase())) {
        paintHops();
        return;
      }
      const hop = adoptClaudeSubagent(t, spec);
      if (hop && !adopted.some((h) => h.child.id === hop.child.id)) adopted.push(hop);
      paintHops();
      return;
    }
  ...
  const result = await runClaudeCodeBounded({
    ...
    onEvent(folded) {
      ...
      if (folded.kind === 'tool') {
        takeTool(folded.name, folded.input);
        return;
      }
      ...
      if (folded.kind === 'tui') {
        ...
        for (const tool of folded.tools || []) takeTool(tool.name, tool.input);
        return;
      }
  ...
  if ((result.ptyPending || result.missing) && !result.paymentFailed) {
    t.liveStatus = PTY_PENDING;
    paint({ type: 'status', name: t.name, color: t.color, detail: PTY_PENDING });
    return PTY_PENDING;
  }
  let finalText = result.paymentFailed || result.text || visible || '';
  if (!result.paymentFailed) {
    finalText = sanitizeClaudeCanvas(finalText, { error: result.error });
  }
  // Never invent an empty-response placeholder so Ask/completions steal the send.
  if (!finalText && hopText) finalText = hopText;
  if (!finalText && result.error) finalText = 'upstream HTTP 400';
  if (!finalText && (result.live || hasLiveClaudeSession(t.id))) finalText = visible || '';
  if (!finalText) finalText = PTY_PENDING;
  if (isAutoPtyPending(finalText) && !result.paymentFailed && !hopText) {
    t.liveStatus = PTY_PENDING;
    paint({ type: 'status', name: t.name, color: t.color, detail: PTY_PENDING });
    return PTY_PENDING;
  }
...
async function runTurn(threadId, userText, onEvent, images) {
  ...
  const paint = (ev) => {
    ...
    if (ev.type === 'delta' || ev.type === 'think' || ev.type === 'status' || ev.type === 'start' || ev.type === 'race' || ev.type === 'tool' || ev.type === 'spawn') t.lastDeltaAt = Date.now();
    onEvent?.(ev);
  };
  ...
  let lastReply = '';
  try {
  // Ask and Auto both need :8402. Waiting here paints "sidecar starting…"
  // so the renderer can heal-sidecar; do not send into an unreachable proxy.
  await waitForSidecarSession(paint, stillMine, turnAbort, t);
  if (!stillMine()) return;
  if (t.members) {
    ...
    for (const m of t.members) {
      ...
      try {
        ...
        r = await recoverSidecarAsk(e, async () => (
          onEvent
            ? (await brainStream(msgs, (delta, meta) => {
              if (meta?.think) paint({ type: 'think', name: m.name, color: m.color, delta });
              else paint({ type: 'delta', name: m.name, color: m.color, delta });
  ...
  // Agent mode is interactive Claude Code on a PTY (`openzoo claude` env).
  // Chat still uses chat/completions. Agent never does — if the harness is
  // not ready, paint PTY_PENDING and retry attach. Do not skip PTY.
  if (isAgentMode(t)) {
    persistUserTurn(t, userText, images);
    saveThreads();
    if (!shouldSkipHarnessAutostart()) {
      try {
        await Promise.race([
          ensureHarness(),
          new Promise((resolve) => setTimeout(resolve, ENSURE_HARNESS_SEND_MS)),
        ]);
      } catch { /* retry attach below */ }
    }
    const retryMs = claudeRunnerOverridden()
      ? 20
      : Number(process.env.OZ_AUTO_PTY_RETRY_MS || 400);
    const retries = claudeRunnerOverridden()
      ? 2
      : Number(process.env.OZ_AUTO_PTY_RETRIES || 8);
    lastReply = await runAutoClaudeTurn(t, userText, images, paint, stillMine, turnAbort);
    for (let n = 0; stillMine() && isAutoPtyPending(lastReply) && n < retries; n++) {
      t.liveStatus = PTY_PENDING;
      paint({ type: 'status', name: t.name, color: t.color, detail: PTY_PENDING });
    ...
    if (!stillMine()) return;
    usedClaude = true;
    if (isAutoPtyPending(lastReply) && !t.history.some((h) => h.who === 'bot' && h.text === PTY_PENDING)) {
      t.history.push({ who: 'bot', text: PTY_PENDING });
      paint({ type: 'final', name: t.name, color: t.color, text: PTY_PENDING });
    }
    bindThread(t).catch(() => {});
    return;
  }
  ...
  if (isAgentMode(t) && !claudeFallback) extras.push({ role: 'system', content: AUTO_DIRECTIVE });
  ...
  try {
    ...
    reply = await recoverSidecarAsk(e, () => ask(), paint, stillMine, turnAbort, t);
  }
  if (!stillMine()) return;
  ...
  if (runCmd) {
    const command = runCmd;
    if (isAgentMode(t) && !claudeFallback) {
      emitLiveStatus(t.id, paint, peekDirectiveStatus('', command));
      const output = await execCommand(command, dirFor(t.id));
      noteRunForCorpus(t.id, command, { cwd: dirFor(t.id) });
      if (!stillMine()) return;
  ...
  if (isAgentMode(t) && !claudeFallback && ack !== null && ack !== undefined
  ...
  bindThread(t).catch(() => {});
    ...
    if (stillMine() && !chained && !parked) {
      if (isAgentMode(t) && usedClaude && hasLiveClaudeSession(t.id)) {
        // Live PTY is still looping — do not park / show a Continue chip.
        t.status = 'thinking';
        if (!t.liveStatus) t.liveStatus = 'Claude Code via OpenZoo…';
      } else if (!usedClaude && !claudeFallback && shouldKeepAuto(t, lastReply, userText)) {
...
function threadSummary(t) {
  ...
  return { id: t.id, name: t.name, color: t.color, parent: t.parent, status: t.status,
    ...
    dir: t.dir || WORKSPACE_DIR, runMode: runModeLabel(t),
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Desktop pane actions: top-left of #chatHeader, next to the thread name / cwd.
     Not a second titlebar. Hidden on coarse/narrow (phone) viewports. */
  #paneActions { display: flex; align-items: center; gap: 3px; flex: 0 0 auto;
                 -webkit-app-region: no-drag; user-select: none; }
  .actbtn { border: 0; background: #1c1c1e; color: #c8c8c2; font: 600 11px/1 inherit;
            padding: 5px 8px; border-radius: 7px; cursor: pointer;
            -webkit-app-region: no-drag; user-select: none; }
  .actbtn:hover { color: #ececec; background: #2c2c2e; }
  .actbtn:focus-visible { outline: 2px solid #6ab0ff; outline-offset: 2px; }
  #actStop { display: none; color: #f28c4d; }
  body.agent-mode #actStop { display: inline-flex; }
  #actStop:hover { color: #ffb27a; background: #2c2c2e; }
  @media (pointer: coarse) and (max-width: 720px) { #paneActions { display: none; } #goalTip { display: none !important; } }
  #goalTip { display: none; color: #8e8e93; font-size: 12px; line-height: 1.35;
             padding: 0 4px 6px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  body.agent-mode #goalTip.show { display: block; }
  ...
  .hdirrow { display: flex; align-items: center; gap: 6px; min-width: 0; }
  ...
  .dirpick { flex: 0 0 22px; width: 22px; height: 22px; border: 0; background: transparent;
             color: #8e8e93; border-radius: 6px; cursor: pointer; padding: 0;
             display: flex; align-items: center; justify-content: center; }
  .dirpick:hover { color: #ececec; background: #2c2c2e; }
  .dirpick svg { width: 14px; height: 14px; }
  /* Run-mode toggle: Chat (completions) vs Agent (openzoo-claude PTY).
     /mode chat|agent; ask|auto remain aliases. The Agent chip must not say Auto
     — Auto is the openzoo/auto model. */
  ...
  .modebtn.chat.on, .modebtn.ask.on { background: #b8f240; }
  .modebtn.agent.on, .modebtn.auto.on { background: #f28c4d; }
  .modebtn:focus-visible { outline: 2px solid #6ab0ff; outline-offset: 2px; }
  /* SPEND DIAL. Two selects rather than more pill toggles: Chat/Agent is a safety
  ...
  .html-preview-chrome { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
  .html-preview-open { display: inline-block; margin-bottom: 6px; font-size: 12px; color: #6ab0ff;
                       text-decoration: underline; cursor: pointer; }
  .html-preview-chrome .html-preview-open { margin-bottom: 0; }
  .html-preview-close { appearance: none; background: transparent; border: 0; color: #8e8e93;
                        font-size: 18px; line-height: 1; cursor: pointer; padding: 2px 8px; }
  .html-preview-close:hover { color: #ececec; }
  ...
  /* Folded chain-of-thought. Not the Agent run-mode chip — that is /mode agent.
  ...
  #bar { padding: 10px 16px 18px; position: relative; flex: 0 0 auto; }
  #row-input { display: flex; align-items: center; gap: 8px; }
  #agentTerm { flex: 1; min-height: 0; display: none; }
  body.agent-mode #agentTerm { display: flex; flex-direction: column; flex: 1; min-height: 0; overflow: hidden; }
  body.agent-mode #log { display: none; }
  #agentPreview { display: none; flex-direction: column; min-height: 0; overflow: hidden;
                  background: #111; border-top: 1px solid #1c1c1e; }
  body.agent-mode #agentPreview.show { display: flex; flex: 0 0 38%; min-height: 160px; max-height: 46%; }
  body.agent-mode.agent-ide #agentPreview.show { flex: 1 1 auto; min-height: 0; max-height: none; }
  body.agent-mode.agent-ide #agentTerm { display: none; }
  body.agent-mode.agent-locked #agentTerm { display: none; }
  #agentPreview .html-preview-chrome { flex: 0 0 auto; padding: 0 4px 0 0; }
  #agentPreview .html-preview-open { margin-bottom: 0; padding: 4px 10px; }
  #agentPreview iframe.html-preview { flex: 1; width: 100%; min-height: 140px; height: auto;
                                      border: 0; border-radius: 0; }
  /* Narrow / phone: Cline + code-server is the Agent. Full-bleed — no sidebar,
     header, composer, or preview-chrome strip around VS Code. Close X stays
     as a floating hit target. Wide desktop iframe layout is unchanged. */
  @media (max-width: 720px), (pointer: coarse) and (max-width: 920px) {
    body.agent-mode.agent-ide #agentPreview.show {
      position: fixed; inset: 0; width: 100%; height: 100%; height: 100dvh;
      z-index: 1100; flex: none; max-height: none; min-height: 0;
      border: 0; background: #000; -webkit-app-region: no-drag;
    }
    body.agent-mode.agent-ide #agentPreview iframe.html-preview {
      position: absolute; inset: 0; width: 100%; height: 100%; min-height: 0;
      border: 0; border-radius: 0; transform: none; zoom: 1;
    }
    body.agent-mode.agent-ide #agentPreview .html-preview-chrome {
      position: absolute; top: 0; right: 0; left: auto; z-index: 1101;
      padding: calc(4px + env(safe-area-inset-top, 0px)) calc(4px + env(safe-area-inset-right, 0px)) 0 0;
      background: transparent; pointer-events: none; justify-content: flex-end;
    }
    body.agent-mode.agent-ide #agentPreview .html-preview-open { display: none; }
    body.agent-mode.agent-ide #agentPreviewClose {
      pointer-events: auto; color: #ececec; font-size: 28px; line-height: 1;
      padding: 10px 14px; text-shadow: 0 1px 4px #000;
    }
    body.agent-mode.agent-ide #bar,
    body.agent-mode.agent-ide #chatHeader,
    body.agent-mode.agent-ide #sidebar,
    body.agent-mode.agent-ide #dragbar,
    body.agent-mode.agent-ide #goalTip { display: none; }
  }
  /* Composer sits under the TUI. Absolute bottom:0 ate the last OCC rows. */
  body.agent-mode #bar { position: relative; left: auto; right: auto; bottom: auto; z-index: 6;
                         background: #000; flex: 0 0 auto; }
  body.agent-mode #agentTerm .xterm-helper-textarea { pointer-events: none; }
...
<body>
  ...
  <div id="main">
    <div id="chatHeader">
      <div id="paneActions" data-component="pane-actions" role="toolbar" aria-label="Chat actions">
        <button type="button" class="actbtn" id="actCopy" title="Copy selection">Copy</button>
        <button type="button" class="actbtn" id="actPaste" title="Paste into Message">Paste</button>
        <button type="button" class="actbtn" id="dirPickBtn" title="Change working folder (same as /dir)">Dir</button>
        <button type="button" class="actbtn" id="actStop" title="Interrupt the Agent">Stop</button>
      </div>
      <div id="chatHeaderId"></div>
      <div id="headerDials">
      <div id="modeToggle" data-component="run-mode-toggle" role="group" aria-label="Chat or Agent mode">
        <button class="modebtn chat on" id="modeChat" data-mode="chat"
                title="Chat completions. RUN: waits for your approval.">Chat</button>
        <button class="modebtn agent" id="modeAgent" data-mode="agent"
                title="openzoo-claude on a PTY — long-running Agent loop">Agent</button>
      </div>
    ...
    <div id="log"></div>
    <div id="agentTerm"></div>
    <div id="agentPreview">
      <div class="html-preview-chrome">
        <a id="agentPreviewOpen" class="html-preview-open" target="_blank" rel="noopener">open</a>
        <button type="button" id="agentPreviewClose" class="html-preview-close" title="Close preview" aria-label="Close preview">×</button>
      </div>
      <iframe id="agentPreviewFrame" class="html-preview" tabindex="-1" sandbox="allow-scripts allow-same-origin allow-forms" title="preview"></iframe>
    </div>
    ...
    <div id="bar">
      ...
      <div id="goalTip">Pro tip: /goal &lt;job&gt; — one slash. Agent keeps working until it's done.</div>
      <div id="row-input">
        <div id="pill">
...
<script src="/vendor/fit.js"></script>
<script>
  function isRetryableFetch(err, status) {
    if (status === 429 || status === 502 || status === 503 || status === 504) return true;
    if (status && status >= 400 && status < 500) return false;
    if (!err) return false;
    var m = String(err.message || err);
    var c = String(err.code || (err.cause && err.cause.code) || '');
    if (err.name === 'AbortError' || err.name === 'TimeoutError') return true;
    if (m.indexOf('fetch failed') >= 0) return true;
    return /ECONNREFUSED|ECONNRESET|ETIMEDOUT|ENOTFOUND/.test(c + ' ' + m);
  }
  async function fetchRetry(url, init) {
    ...
    for (var i = 0; i <= delays.length; i++) {
      try {
        ...
        lastRes = res;
        if (res.ok || !isRetryableFetch(null, res.status)) return res;
        lastErr = res;
      } catch (err) {
        lastErr = err;
        if (!isRetryableFetch(err, 0)) throw err;
      }
      if (i === delays.length) break;
      var wait = delays[i] * (0.8 + Math.random() * 0.4);
      await new Promise(function (r) { setTimeout(r, wait); });
    }
    try { requestSidecarHeal(); } catch (e) {}
  ...
  function renderHeader(t) {
    document.getElementById('chatHeaderId').innerHTML =
      ...
      '<div class="hname"><div>' + t.name + '</div>' +
      '<div class="hdirrow"><div class="hdir" title="' + escapeHtml(t.dir || '') +
      '">' + escapeHtml(t.dir || '') + '</div></div></div>';
    setModeButtons(runModeLabel(t));
    setDials(t);
  }
  async function pickThreadDir() {
    let dir = null;
    if (window.electronAPI && typeof window.electronAPI.pickDirectory === 'function') {
      try { dir = await window.electronAPI.pickDirectory(); } catch (e) { dir = null; }
    }
    if (!dir) {
      try { dir = window.prompt('Working folder path', ''); } catch (e) { return; }
    }
    if (!dir) return;
    await echoSlash('/dir ' + dir);
  }
  const dirPickBtn = document.getElementById('dirPickBtn');
  if (dirPickBtn) dirPickBtn.addEventListener('click', pickThreadDir);
  ...
  let syncingDials = false;
  function setDials(t) {
    const tierSel = document.getElementById('tierSel');
    const raceSel = document.getElementById('raceSel');
    if (!tierSel || !raceSel) return;
    // tick()/render() re-applies the server value. Do not fire change — that
    // would echo /tier and dump /model into the PTY every 1.2s.
    syncingDials = true;
  ...
  async function savePastedSub() {
    ...
    try {
      ...
      setSubNote('Subscription key saved · no x402');
      await openWallet();
      if (document.body.classList.contains('agent-mode')) openAgentIde(true);
    } catch (e) {
      setSubNote(e.message || String(e), true);
    }
  }
  async function forgetSub() {
    ...
    await openWallet();
    if (document.body.classList.contains('agent-mode')) lockAgentIde();
  }
  ...
  async function openSitrep() {
    ...
    if (proxyDown) requestSidecarHeal();
  ...
  document.getElementById('tierSel').addEventListener('change', (e) => {
    if (syncingDials) return;
    setDial('tier', e.target.value);
  });
  document.getElementById('raceSel').addEventListener('change', (e) => {
    if (syncingDials) return;
    setDial('race', e.target.value);
  });

  // The toggle reflects the SERVER's value rather than local state — the mode
  // is per-thread and also settable by typing "/mode agent", so anything that
  // tracked it client-side would drift the moment either path was used.
  function runModeLabel(tOrMode) {
    const m = (tOrMode && typeof tOrMode === 'object') ? tOrMode.runMode : tOrMode;
    const s = String(m || '').toLowerCase();
    return (s === 'agent' || s === 'auto') ? 'agent' : 'chat';
  }
  function setModeButtons(mode) {
    const agent = mode === 'agent' || mode === 'auto';
    document.getElementById('modeChat').className = 'modebtn chat' + (!agent ? ' on' : '');
    document.getElementById('modeAgent').className = 'modebtn agent' + (agent ? ' on' : '');
    applyAgentMode(agent);
  }

  let agentTerm = null;
  let agentFit = null;
  let agentFitTimer = null;
  let agentSelTimer = 0;
  let lastPtySize = '';
  function syncGoalTip() {
    const el = document.getElementById('goalTip');
    if (!el) return;
    const t = knownThreads.find((x) => x.id === activeId);
    const agent = document.body.classList.contains('agent-mode');
    el.classList.toggle('show', !!(agent && t && !t.goalSet));
  }
  function applyAgentMode(agent) {
    const was = document.body.classList.contains('agent-mode');
  ...
  function ensureAgentTerm() {
    if (agentTerm) return;
    const el = document.getElementById('agentTerm');
    ...
    agentTerm = new Terminal({
      ...
      fontSize: 13,
      theme: { background: '#000000', foreground: '#ececec', cursor: '#b8f240' },
      allowProposedApi: true
    });
    const FitCls = (typeof FitAddon !== 'undefined' && FitAddon.FitAddon)
      ? FitAddon.FitAddon
      : (typeof FitAddon !== 'undefined' ? FitAddon : null);
    agentFit = FitCls ? new FitCls() : null;
    if (agentFit) agentTerm.loadAddon(agentFit);
    agentTerm.open(el);
    // Click (no drag) focuses Message. Drag keeps xterm selection so Copy
    // can read it. disableStdin stays true — keystrokes never land in OCC.
    var agentPtr = null;
    el.addEventListener('pointerdown', function (e) {
      agentPtr = { x: e.clientX, y: e.clientY };
    }, true);
    ...
    if (typeof agentTerm.onSelectionChange === 'function') {
      agentTerm.onSelectionChange(function () {
        clearTimeout(agentSelTimer);
        agentSelTimer = setTimeout(function () {
          var t = '';
          try { t = agentTerm && typeof agentTerm.getSelection === 'function' ? String(agentTerm.getSelection() || '') : ''; } catch (err) { t = ''; }
          if (!t) return;
          copyText(t).then(function (ok) {
            if (ok) {
              var btn = document.getElementById('actCopy');
              showCopiedToast(btn ? btn.getBoundingClientRect() : null);
            }
          });
        }, 40);
      });
  ...
  function fitAgentTerm() {
    const el = document.getElementById('agentTerm');
    if (!el || !document.body.classList.contains('agent-mode')) return;
    if (el.clientHeight < 40) return;
    if (!agentTerm) { ensureAgentTerm(); if (!agentTerm) return; }
    var atBottom = true;
    try {
      var buf = agentTerm.buffer.active;
      atBottom = (buf.viewportY + agentTerm.rows) >= (buf.baseY + buf.length);
    } catch (e) { atBottom = true; }
    if (agentFit) {
      try { agentFit.fit(); } catch (e) {}
    }
    var key = agentTerm.cols + 'x' + agentTerm.rows;
    if (activeId && key !== lastPtySize) {
      lastPtySize = key;
      fetch(API + '/threads/' + activeId + '/pty-size', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ cols: agentTerm.cols, rows: agentTerm.rows })
      });
    }
    if (atBottom) { try { agentTerm.scrollToBottom(); } catch (e) {} }
  }
  window._ozFitAgent = fitAgentTerm;
  window.addEventListener('resize', function () {
    clearTimeout(agentFitTimer);
    agentFitTimer = setTimeout(fitAgentTerm, 160);
  });
  ...
  function scrubBotText(s) {
    ...
    s = s.replace(/<tool-use\\b[^>]*>[\\s\\S]*?<\\/tool-use>/gi, '');
    s = s.replace(/<tool-use\\b[^>]*>[\\s\\S]*$/i, '');
    s = s.replace(/^[ \\t]*(?:#\\s*)?currentDir\\b[^\\n]*\\n?/gim, '');
    s = s.replace(/^[ \\t]*\\[(?:LS|Read|Write|Edit|Bash|Glob|Grep|Task|Agent|NotebookEdit)\\][ \\t]*running\\b[^\\n]*\\n?/gim, '');
  ...
  function htmlPreviewUrl(text) {
    ...
    m = /(?:File written|File updated):\\s+([^\\n]+?\\.(?:html?|HTML?))/i.exec(s);
    if (m) return servedHrefForPath(m[1].replace(/[.,;)"']+$/, '')) || clientWorkspaceUrl(relFromDiskPath(m[1]) || m[1].split('/').pop());
  ...
  function previewFrame(url, key) {
    ...
    chrome.className = 'html-preview-chrome';
    ...
    close.type = 'button';
    close.className = 'html-preview-close';
    ...
    bindPreviewFocusGuard(frame);
    return wrap;
  }
  var agentPreviewHidden = {};
  var agentIdeHidden = false;
  var agentIdeLive = false;
  var agentIdeClicked = false;
  var agentIdeSession = null;
  function focusMessageComposer() {
    try { if (inp) inp.focus(); } catch (e) {}
  }
  function shouldKeepIdeFocus(frame) {
    return !!(frame && frame.getAttribute('data-kind') === 'ide' && agentIdeClicked);
  }
  function bouncePreviewFocus(frame) {
    if (!frame) return;
    if (shouldKeepIdeFocus(frame)) return;
  ...
  function bindPreviewFocusGuard(frame) {
    if (!frame || frame.getAttribute('data-focus-guard') === '1') return;
    frame.setAttribute('data-focus-guard', '1');
    if (frame.getAttribute('data-kind') !== 'ide') frame.setAttribute('tabindex', '-1');
    frame.addEventListener('focus', function () {
      if (shouldKeepIdeFocus(frame)) return;
      bouncePreviewFocus(frame);
    });
    frame.addEventListener('load', function () {
      if (!shouldKeepIdeFocus(frame)) bouncePreviewFocus(frame);
      try {
        var w = frame.contentWindow;
        var doc = frame.contentDocument || (w && w.document);
        if (w) w.addEventListener('focus', function () {
          if (shouldKeepIdeFocus(frame)) return;
          bouncePreviewFocus(frame);
        });
        if (doc) doc.addEventListener('focusin', function () {
          if (shouldKeepIdeFocus(frame)) return;
          bouncePreviewFocus(frame);
        }, true);
      } catch (e) {}
  ...
  function clearAgentIde() {
    agentIdeLive = false;
    agentIdeClicked = false;
    document.body.classList.remove('agent-ide');
    document.body.classList.remove('agent-locked');
    var frame = document.getElementById('agentPreviewFrame');
    if (frame) {
      frame.removeAttribute('data-kind');
      frame.setAttribute('tabindex', '-1');
      frame.setAttribute('sandbox', 'allow-scripts allow-same-origin allow-forms');
      frame.title = 'preview';
    }
  }
  function lockAgentIde() {
    agentIdeHidden = false;
    agentIdeSession = null;
    clearAgentIde();
    document.body.classList.add('agent-locked');
    hideAgentPreview();
    openWallet();
  }
  function showAgentIde(sess) {
    var url = sess && sess.url ? String(sess.url) : '';
    if (!url) return;
    agentIdeLive = true;
    agentIdeClicked = false;
    agentIdeHidden = false;
    document.body.classList.remove('agent-locked');
    document.body.classList.add('agent-ide');
    var frame = document.getElementById('agentPreviewFrame');
    if (frame) {
      frame.setAttribute('data-kind', 'ide');
      frame.setAttribute('tabindex', '0');
      frame.setAttribute('sandbox', 'allow-scripts allow-same-origin allow-forms allow-popups allow-downloads allow-modals');
      frame.title = 'cloud IDE';
    }
    showAgentPreview(url, 'ide:' + (sess.id || url), true);
  }
  function openAgentIde(force) {
    if (!document.body.classList.contains('agent-mode')) return;
    if (!force && agentIdeHidden) return;
    document.body.classList.remove('agent-locked');
    if (!force && agentIdeSession && agentIdeSession.url) {
      showAgentIde(agentIdeSession);
      return;
    }
    fetch(API + '/api/ide/session', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: '{}',
    }).then(function (r) {
      return r.json().then(function (j) { return { r: r, j: j }; }).catch(function () { return { r: r, j: {} }; });
    }).then(function (x) {
      if (!document.body.classList.contains('agent-mode')) return;
      var status = x.r.status;
      var j = x.j || {};
      if (status === 401 || j.status === 401) { lockAgentIde(); return; }
      if (j && j.url) { agentIdeSession = j; showAgentIde(j); return; }
      document.body.classList.remove('agent-ide');
      pullAgentPreview();
    }).catch(function () {
      document.body.classList.remove('agent-ide');
  ...
  function hideAgentPreview(dismiss) {
    var box = document.getElementById('agentPreview');
    var frame = document.getElementById('agentPreviewFrame');
    if (box) box.classList.remove('show');
    if (dismiss && activeId) {
      agentPreviewHidden[activeId] = true;
      if (agentIdeLive) agentIdeHidden = true;
      if (frame) {
        try { frame.removeAttribute('src'); } catch (e) {}
        frame.removeAttribute('data-rel');
      }
    }
    if (dismiss) clearAgentIde();
    requestAnimationFrame(function () { if (window._ozFitAgent) window._ozFitAgent(); });
  }
  function showAgentPreview(url, rel, force) {
    var box = document.getElementById('agentPreview');
    var frame = document.getElementById('agentPreviewFrame');
    var open = document.getElementById('agentPreviewOpen');
    if (!box || !frame || !url) return;
    var ideRel = String(rel || '').indexOf('ide:') === 0;
    if (agentIdeLive && !ideRel) return;
    if (!ideRel) {
      frame.removeAttribute('data-kind');
      frame.setAttribute('tabindex', '-1');
      frame.setAttribute('sandbox', 'allow-scripts allow-same-origin allow-forms');
    ...
    if (force && activeId) delete agentPreviewHidden[activeId];
    if (!force && activeId && agentPreviewHidden[activeId]) return;
    box.classList.add('show');
    if (open) { open.href = url; }
    var key = String(rel || url);
    var same = frame.getAttribute('data-rel') === key && frame.getAttribute('src');
    if (same) {
      try { if (frame.contentWindow) frame.contentWindow.location.reload(); } catch (e) { frame.src = url; }
    } else {
      frame.setAttribute('data-rel', key);
      frame.src = url;
    }
    bindPreviewFocusGuard(frame);
    requestAnimationFrame(function () {
      if (!shouldKeepIdeFocus(frame)) bouncePreviewFocus(frame);
      if (window._ozFitAgent) window._ozFitAgent();
    });
  }
  function noteAgentHtmlFromText(text) {
    var url = htmlPreviewUrl(text);
    if (url) { showAgentPreview(url); return; }
    var m = /(?:File written|File updated|Wrote|Edited):\\s+([^\\n]+?\\.(?:html?|HTML?))/i.exec(String(text || ''))
      || /((?:\\/Users\\/|\\/home\\/|\\/workspace\\/|\\/tmp\\/)\\S+\\.(?:html?|HTML?))/i.exec(String(text || ''));
    if (!m) return;
    var p = m[1].replace(/[.,;:)"']+$/, '');
    var href = servedHrefForPath(p) || clientWorkspaceUrl(relFromDiskPath(p) || p.split('/').pop());
    if (href) showAgentPreview(href, relFromDiskPath(p) || p);
  }
  function pullAgentPreview() {
    if (!activeId || !document.body.classList.contains('agent-mode')) return;
    fetch(API + '/threads/' + activeId + '/preview').then(function (r) { return r.json(); }).then(function (j) {
      if (j && j.url) showAgentPreview(j.url, j.rel);
      else hideAgentPreview();
    }).catch(function () {});
  }
  (function wirePreviewChrome() {
    var closeBtn = document.getElementById('agentPreviewClose');
    if (closeBtn) {
      closeBtn.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        hideAgentPreview(true);
        focusMessageComposer();
      });
    }
    bindPreviewFocusGuard(document.getElementById('agentPreviewFrame'));
    document.addEventListener('pointerdown', function (e) {
      var box = document.getElementById('agentPreview');
      if (agentIdeLive && box && box.contains(e.target)) agentIdeClicked = true;
      else agentIdeClicked = false;
    }, true);
    document.addEventListener('focusin', function (e) {
      var t = e.target;
      if (!t) return;
      if (t.id === 'agentPreviewFrame' || (t.tagName === 'IFRAME' && t.classList && t.classList.contains('html-preview'))) {
        if (shouldKeepIdeFocus(t)) return;
        bouncePreviewFocus(t);
      }
    }, true);
  })();
  ...
  function agentTermSelection() {
    if (!agentTerm) return '';
    try {
      if (typeof agentTerm.hasSelection === 'function' && !agentTerm.hasSelection()) return '';
      if (typeof agentTerm.getSelection === 'function') return String(agentTerm.getSelection() || '');
    } catch (e) {}
    return '';
  }
  let paneCopyHold = '';
  async function copyFromBar() {
    let text = paneCopyHold;
    paneCopyHold = '';
    if (!text) text = agentTermSelection();
    if (!text) text = selectedText();
    if (!text) return;
    const snap = snapshotSelection();
    const btn = document.getElementById('actCopy');
    const rect = selectedRect() || (btn ? btn.getBoundingClientRect() : null);
    let ok = false;
    try { ok = await copyText(text); }
    finally { restoreSelection(snap); }
    if (ok) showCopiedToast(rect);
  }
  async function readClipboardText() {
    try {
      if (window.electronAPI && typeof window.electronAPI.readText === 'function') {
        const t = await window.electronAPI.readText();
  ...
  function insertIntoMessage(text) {
    ...
    if (send) send.classList.toggle('show', el.value.trim().length > 0);
  }
  async function pasteFromBar() {
    const text = await readClipboardText();
    if (!text) return;
    // Prefer Message insert so they can edit, then Enter sends to the PTY.
    // Writing the clipboard to the Agent PTY is allowed only when Message
    // is empty AND this is the Paste *button* — never from a keyboard paste.
    // Default stays Message; do not dump into OCC on a mis-click.
    insertIntoMessage(text);
  }
  function stopAgent() {
    if (!activeId || !document.body.classList.contains('agent-mode')) return;
    fetch(API + '/threads/' + activeId + '/pty', {
      method: 'POST',
      body: String.fromCharCode(27)
    });
  }
  const actCopy = document.getElementById('actCopy');
  const actPaste = document.getElementById('actPaste');
  const actStop = document.getElementById('actStop');
  if (actCopy) {
    actCopy.addEventListener('pointerdown', function () {
      paneCopyHold = agentTermSelection() || selectedText();
    });
    actCopy.addEventListener('click', function () { copyFromBar(); });
  }
  if (actPaste) actPaste.addEventListener('click', function () { pasteFromBar(); });
  if (actStop) actStop.addEventListener('click', function () { stopAgent(); });
  ...
  async function render() {
    ...
    renderHeader(t);
    syncGoalTip();
    inp.placeholder = (runModeLabel(t) === 'agent')
      ? 'Message — Enter sends to the TUI even while busy. Esc in the TUI interrupts.'
      : 'Message ' + t.name;
    const full = await loadActiveMessages();
    if (!full || full.id !== activeId) return;
    if (full.goalSet) t.goalSet = true;
    syncGoalTip();
  ...
  function connectStream(id) {
    ...
    es.onmessage = (e) => {
      ...
      if (ev.type === 'pty') {
        ...
        if (!agentTerm) return;
        if (ev.reset) {
          var replay = String(ev.data || '');
          if (replay && replay === lastPtyReset) return;
          lastPtyReset = replay;
          try { agentTerm.reset(); } catch (err) {}
          lastPtySize = '';
        }
        if (ev.data) {
          ...
          agentTerm.write(bytes);
          var ptyText = '';
          try { ptyText = new TextDecoder('utf-8').decode(bytes); } catch (err) { ptyText = bin; }
          noteAgentHtmlFromText(ptyText);
        }
        return;
      }
      if (ev.type === 'preview' && ev.url) {
        showAgentPreview(ev.url, ev.rel, true);
        return;
      }
      if (ev.type === 'start') {
        streamBuf = '';
        ...
        streamStatus = ev.detail || streamStatus;
        paintStream();
        loadThreads();
      }
      else if (ev.type === 'status') {
        streamStatus = ev.detail || streamStatus;
        if (streamStatus === 'sidecar starting…') requestSidecarHeal();
        paintStream();
      }
  ...
  async function submit() {
    ...
    // Agent: Message stays usable while Ink redraws / tools run. Do not take
    // the sending lock — a second Enter must still hit the PTY.
    if (document.body.classList.contains('agent-mode')) {
      if (!task || !activeId) return;
      const word = task.charAt(0) === '/' ? task.slice(1).split(' ')[0].toLowerCase() : '';
      const local = word === 'mode' || word === 'tier' || word === 'help' || word === 'dir';
      inp.value = '';
      send.classList.remove('show');
      try { inp.focus(); } catch (e) {}
      if (word === 'goal') {
        const kt = knownThreads.find((x) => x.id === activeId);
        if (kt) kt.goalSet = true;
        syncGoalTip();
      }
      if (local) {
        await echoSlash(task);
        return;
      }
      fetch(API + '/threads/' + activeId + '/pty', {
        method: 'POST',
        body: task + String.fromCharCode(13)
      });
      return;
    }
    ...
    let persisted = false;
    try {
      const r = await fetchRetry(API + '/drive', {
        method: 'POST', headers: { 'content-type': 'application/json' },
  ...
  var lastHealAt = 0;
  function requestSidecarHeal() {
    var now = Date.now();
    if (lastHealAt && now - lastHealAt < 15000) return;
    lastHealAt = now;
    try {
      if (window.electronAPI && typeof window.electronAPI.healSidecar === 'function') {
        window.electronAPI.healSidecar();
      }
    } catch (e) { /* browser / non-electron — re-ping is enough */ }
  }
  function fmtDockX(n) {
  ...
  async function refreshHud() {
    try {
      ...
      if (spent > 0) {
        ...
        // Session direct/spent (never first-call). Ember only when cogs > spent
        // — a house meter bug, not a correctly billed 400. Failures cost you.
        if (cogsOver) {
          hintEl.className = 'hhint show';
          hintEl.innerHTML = '<b>cogs above paid.</b> house bug — our cost has no matching billed. '
            + 'you pay for every entrant we actually launched; failures still cost you.';
        } else {
...
const server = http.createServer((req, res) => {
  ...
  // Desktop Agent: cloud code-server + Cline. Bearer is the stored OpenZoo
  // subscription key — never ANTHROPIC_API_KEY, never an open URL, never
  // the key in the JSON body. 401 if no / invalid key (same as mobile).
  {
    const idePath = String(req.url || '').split('?')[0];
    if ((req.method === 'POST' || req.method === 'GET') && idePath === '/api/ide/session') {
      (async () => {
        const out = await openStoredIdeSession({ method: req.method });
        const pub = publicIdeSession(out);
        res.writeHead(out.ok ? 200 : (out.status || 401), { 'content-type': 'application/json' });
  ...
  if (req.method === 'GET' && req.url === '/hud-summary') {
    (async () => {
      ...
      try {
        const r = await fetch('http://127.0.0.1:8402/v1/session', { signal: AbortSignal.timeout(2500) });
  ...
  if (req.method === 'GET' && sse) {
    ...
    const streamThread = threads.get(id);
    if (streamThread && isAgentMode(streamThread)) {
      const sess = ensureAgentPty(streamThread);
      try {
        res.write(`data: ${JSON.stringify({
          type: 'pty',
          data: sess?.buf?.length ? sess.buf.toString('base64') : '',
          reset: true,
        })}\n\n`);
      } catch { /* closed */ }
      const rel = htmlRelFromText(sess?.buf ? sess.buf.toString('utf8') : '', id) || findPlayableHtmlRel(id);
      if (rel && workspacePort) {
        try {
          res.write(`data: ${JSON.stringify({
            type: 'preview',
            rel,
            url: workspaceFileUrl(id, rel),
          })}\n\n`);
        } catch { /* closed */ }
      }
    }
  ...
  {
    const vendor = /^\/vendor\/([^/?#]+)$/.exec(req.url || '');
    if (req.method === 'GET' && vendor && VENDOR_FILES.has(vendor[1])) {
      const file = path.join(VENDOR_DIR, vendor[1]);
      if (!existsSync(file)) {
        res.writeHead(404, { 'content-type': 'text/plain' });
        res.end('missing vendor file');
        return;
      }
      const mime = vendor[1].endsWith('.css') ? 'text/css' : 'application/javascript';
  ...
  {
    const ptyWrite = /^\/threads\/([^/]+)\/pty$/.exec(req.url || '');
    if (req.method === 'POST' && ptyWrite) {
      const t = threads.get(ptyWrite[1]);
      if (!t) {
      ...
      req.on('end', () => {
        ...
        if (raw.length === 1 && raw[0] === 27) writeAgentPty(t.id, raw);
        else void writeAgentPtyLine(t.id, raw.toString('utf8'));
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end('{"ok":true}');
      });
      return;
  ...
  {
    const ptySize = /^\/threads\/([^/]+)\/pty-size$/.exec(req.url || '');
    if (req.method === 'POST' && ptySize) {
      const t = threads.get(ptySize[1]);
      if (!t) {
        res.writeHead(404, { 'content-type': 'application/json' });
        res.end('{"ok":false}');
        return;
      ...
      req.on('end', () => {
        ...
        try {
          const j = JSON.parse(Buffer.concat(chunks).toString('utf8'));
          cols = Math.max(2, Number(j.cols) || 0);
          rows = Math.max(1, Number(j.rows) || 0);
        } catch { /* ignore */ }
        const sess = ensureAgentPty(t);
        if (sess && cols && rows) sess.resize(cols, rows);
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end('{"ok":true}');
      });
      return;
  ...
  {
    const previewGet = /^\/threads\/([^/]+)\/preview$/.exec(req.url || '');
    if (req.method === 'GET' && previewGet) {
      const id = previewGet[1];
      ...
      const sess = agentPtys.get(id);
      const rel = htmlRelFromText(sess?.buf ? sess.buf.toString('utf8') : '', id) || findPlayableHtmlRel(id);
      void (async () => {
        const port = rel ? await ensureWorkspacePort() : null;
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end(JSON.stringify(rel && port ? { rel, url: workspaceFileUrl(id, rel) } : { rel: '', url: '' }));
      })();
      return;
    }
  }
  ...
  if (req.method === 'DELETE' && req.url.startsWith('/threads/')) {
    ...
    if (doomed) {
      killAgentPty(doomed.id);
      closeClaudeSession(doomed.id);
      finishChildDir(doomed);
    }
  ...
  if (req.method === 'POST' && req.url === '/drive') {
    ...
    req.on('end', async () => {
      ...
      if (/^\//.test(task.trim())) {
        ...
        if (!(isAgentMode(t) && !isGrokuiOwnedSlash(task.trim(), t.runMode))) {
      ...
      if (dirCmd) {
        ...
        if (ok) {
          t.dir = full;
          killAgentPty(t.id);
          if (isAgentMode(t)) ensureAgentPty(t);
      ...
      // "/mode chat|agent" (ask|auto aliases). Agent is the cloud IDE
      // (POST/GET /api/ide/session). Chat stays on completions; RUN: waits for approve/deny.
      const modeCmd = /^\/mode\s+(chat|agent|auto|ask)\b/.exec(task.trim());
      if (modeCmd) {
        const next = normalizeRunMode(modeCmd[1]);
        t.runMode = next;
        if (next !== 'agent') {
          closeClaudeSession(t.id);
          killAgentPty(t.id);
      ...
      ack(true, { persisted: true });
      // Agent: Message composer POSTs to /pty (works while tools run).
      // Do not runTurn. Leftover /drive (and Claude slashes) still write
      // the PTY — xterm stdin is disabled so typing never lands in Ink.
      if (isAgentMode(t)) {
        if (task) void writeAgentPtyLine(t.id, String(task));
        return;
      }
...
server.listen(PORT, BIND, () => {
  console.log(`[grokui] http://${BIND === '0.0.0.0' ? 'localhost' : BIND}:${PORT}`);
  // Window paints on /threads. Install openzoo-claude into ~/.local/bin
  // after listen — never dump an npx recipe, never block chrome.
  if (!shouldSkipHarnessAutostart()) {
    void ensureHarness().catch((e) => {
      console.error('[openzoo] openzoo-claude install:', e && e.message);
    });
  }
});
...
export {
  ...
  openStoredIdeSession, publicIdeSession,
  htmlRelFromText, findPlayableHtmlRel, relInThreadDir,
  ...
  isAgentMode, normalizeRunMode, runModeLabel,
  ensureAgentPty, killAgentPty, writeAgentPty, writeAgentPtyLine, ptyLooksReady, agentPtySpawnSpec, setAgentPtySpawnerForTest, CLAUDE_SLASH_IN_AUTO,
  ...
  autoClaudeTurnProducedVisible, visibleBotCount, isAutoPtyPending, PTY_PENDING,
  sidecarSessionReady, waitForSidecarSession, setSidecarSessionForTest, SIDECAR_STARTING,
  recoverSidecarAsk,
  noteSidecarUnreachable, fetchRetry, isRetryableFetchError,
  isClaudeSubagentTool, claudeSubagentSpec, claudeSubagentHopText, adoptClaudeSubagent,
};
