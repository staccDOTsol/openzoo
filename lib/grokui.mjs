...
function rememberAutoCommand(t, command) {
  if (!t) return 1;
  const key = normalizeAutoCommand(command);
  if (!key) return 1;
  if (t.lastAutoCommand === key) t.autoCommandRuns = (t.autoCommandRuns || 1) + 1;
  else {
...
function identicalCommandCapped(t, command) {
  if (!t) return false;
  const key = command != null ? normalizeAutoCommand(command) : t.lastAutoCommand;
...
async function runTurn(threadId, userText, onEvent, images) {
    ...
    for (const m of t.members) {
      ...
      if (runCmd) {
        ...
        if (t.runMode === 'auto') {
          ...
          const botText = stopSame ? `${command}\n${AUTO_SAME_COMMAND_STOP}` : command;
          t.history.push({
          ...
          memberReply = stopSame ? `${shown}\n${AUTO_SAME_COMMAND_STOP}` : shown;
          if (stopSame) t.autoStopped = true;
  ...
  if (t.runMode === 'auto' && !t.autoStopped && !isAutoStopReply(reply)
