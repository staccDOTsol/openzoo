...
async function handleSlash(task, t) {
  ...
  if (cmd === 'tier') {
    ...
    if (isAgentMode(t)) {
      ...
      if (line) writeAgentPty(t.id, line + String.fromCharCode(13));
    }
    if (want === 'auto') {
...
const server = http.createServer((req, res) => {
  ...
  }
  {
    ...
    if (req.method === 'POST' && ptyWrite) {
      ...
      req.on('end', () => {
        ...
        if (sess) sess.write(Buffer.concat(chunks));
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end('{"ok":true}');
      });
      return;
  ...
  if (req.method === 'POST' && req.url === '/drive') {
    ...
    req.on('end', async () => {
      ...
      if (isAgentMode(t)) {
        if (task) writeAgentPty(t.id, String(task) + String.fromCharCode(13));
        return;
      }
...
export {
  ...
  ensureAgentPty, killAgentPty, writeAgentPty, agentPtySpawnSpec, setAgentPtySpawnerForTest, CLAUDE_SLASH_IN_AUTO,
