...
function isEmptyAutoPty(result, visible, finalText) {
  if (result?.paymentFailed || result?.missing) return false;
  const text = String(finalText || result?.text || visible || '').trim();
...
async function runAutoClaudeTurn(t, userText, images, paint, stillMine, turnAbort) {
  ...
  if (isEmptyAutoPty(result, visible, finalText)) {
    // Empty PTY — caller falls through to chat completions (Ask/Auto path).
    return '';
  }
...
async function runTurn(threadId, userText, onEvent, images) {
  ...
  // Empty PTY (idle silence, no text) falls through to that Ask path so a
  // down :8402 / empty TUI cannot swallow the turn.
  if (t.runMode === 'auto') {
...
export {
  ...
  handleSlash, isGrokuiOwnedSlash, newThread, setRunTurnForTest, setBrainAskForTest, setClaudeRunnerForTest, setSidecarSessionForTest, runTurn,
  runAutoClaudeTurn, autoClaudePrompt, sidecarSessionReady, waitForSidecarSession, isEmptyAutoPty, SIDECAR_STARTING,
