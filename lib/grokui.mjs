...
import { brain, brainBest, brainRace, brainStream, tierModels, MODEL, PROXY, TIER_NAMES } from './podagent.mjs';
...
function newThread(name, parent, members) {
  ...
  const t = { id, name, color: members ? members[0].color : colorFor(name), parent: parent || null,
    ...
    ...(p?.tier ? { tier: p.tier } : {}),
    ...(p?.race ? { race: p.race } : {}),
    ...(p?.raceMode ? { raceMode: p.raceMode } : {}) };
  threads.set(id, t);
  saveThreads();
  return t;
}
...
const SLASH_COMMANDS = [
  ...
  { name: '/race', args: '<0-4> | best <2-4>', help: 'ask N random models from the tier at once — fastest wins, or judged best' },
...
async function handleSlash(task, t) {
  ...
  if (cmd === 'race') {
    if (!arg) {
      return `This thread: ${Number(t.race) || 0
        ? `${t.raceMode === 'best' ? 'best of' : 'racing'} ${t.race} models per turn` : 'not racing'}\n`
        + 'Set with  /race <0-4>        — fastest wins. N models drawn AT RANDOM from the tier, asked at\n'
        + '                              once; the first REAL answer wins, the rest are discarded.\n'
        + '          /race best <2-4>   — best wins. Same N, but it waits for all of them and a cheap\n'
        + '                              model judges them blind (A/B/C/D) and picks one.\n'
        + 'fastest is as quick as the quickest entrant; best is as slow as the SLOWEST, plus a judging\n'
        + 'call. Only best can improve an answer — fastest just delivers one sooner, and on a hard\n'
        + 'question it rewards whichever model thought least.\n'
        + 'You pay for every entrant either way, so 4 costs about 4x a normal turn.';
    }
    // "best 3", "3 best", or a bare "best" (keeps the current N, or uses 3).
    const words = arg.trim().toLowerCase().split(/\s+/);
    const wantsBest = words.includes('best') || words.includes('judge') || words.includes('judged');
    const num = words.map(Number).find((x) => Number.isFinite(x));
    const n = num === undefined
      ? (wantsBest ? (Number(t.race) || 3) : NaN)
      : Math.max(0, Math.min(4, Math.round(num)));
    if (!Number.isFinite(n)) return `"${arg}" is not a number. Use /race 0 to turn it off, /race 2-4, or /race best 3.`;
    if (n < 2) { delete t.race; delete t.raceMode; saveThreads(); return 'Racing off — one model per turn.'; }
    t.race = n;
    if (wantsBest) t.raceMode = 'best'; else delete t.raceMode;
    saveThreads();
    const pool = await tierModels(t.tier || 'medium', 99);
    return (wantsBest
      ? `Best of ${n} per turn, drawn at random from the ${t.tier || 'medium'} tier (${pool.length} in the pool).\n`
        + 'All of them run to completion, then a cheap model reads the answers blind and picks one.\n'
        + `Slower than a plain race — you wait for the slowest of the ${n} — and it adds one small judging call.`
      : `Racing ${n} models per turn, drawn at random from the ${t.tier || 'medium'} tier (${pool.length} in the pool).\n`
        + 'First real answer wins.')
      + `\nCosts about ${n}x a normal turn — you pay for the losers too.`
...
async function runTurn(threadId, userText, onEvent, images) {
  ...
  const ask = async (attempt = 0) => {
    ...
    if (!t.model && race >= 2) {
      ...
      // 'best' waits for all of them and has a cheap model judge; 'fast' takes
      // whoever finishes first. Opposite trades — see brainBest.
      const run = t.raceMode === 'best' ? brainBest : brainRace;
      return (await run(callMsgs, emit, t.contextId, models)).trim();
    }
...
function threadSummary(t) {
  ...
  return { id: t.id, name: t.name, color: t.color, parent: t.parent, status: t.status,
    ...
    tier: t.tier || 'medium', race: Number(t.race) || 0, raceMode: t.raceMode || 'fast', model: t.model || '' };
}
  ...
  <div id="main">
    <div id="chatHeader">
      ...
      <select class="dial" id="raceSel" data-component="model-race" aria-label="Race models"
        ...
        <optgroup label="fastest wins">
          <option value="2">race 2</option>
          <option value="3">race 3</option>
          <option value="4">race 4</option>
        </optgroup>
        <optgroup label="judged — best wins">
          <option value="best 2">best of 2</option>
          <option value="best 3">best of 3</option>
          <option value="best 4">best of 4</option>
        </optgroup>
      </select>
...
<script>
  ...
  function setDials(t) {
    ...
    raceSel.value = (t.race || 0) < 2 ? '0' : (t.raceMode === 'best' ? 'best ' + t.race : String(t.race));
