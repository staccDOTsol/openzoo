...
async function runTurn(threadId, userText, onEvent, images) {
  ...
  if (t.runMode === 'auto') {
    ...
    // waitIdle treated a quiet TUI as done; sanitizeClaudeCanvas wiped chrome;
    // the user bubble was on disk and the bot row was empty. Pop that empty
    // row and fall through to the Ask/Auto chat-completions path so this
    // turn gets a reply. Payment-failed / missing-CLI stay as Claude results.
...
export {
  ...
  isClaudeFallbackReply, popClaudeFallbackBot,
};
