...
const SLASH_COMMANDS = [
  ...
  { name: '/mode', args: 'chat|agent', help: 'Agent = cloud IDE (zoo.openzoo.fun/ide/session); Chat = completions (ask|auto still work)' },
];
  ...
  if (cmd === 'mode') {
    if (!arg) {
      return `Run mode is ${runModeLabel(t)}.`
        + (isAgentMode(t)
          ? ' Agent = cloud IDE (code-server + Cline) via POST /ide/session. No subscription key → locked / Pay. Local OCC TUI can stay for Chat-less use.'
    ...
    saveThreads();
    return next === 'agent'
      ? 'Run mode set to agent — cloud IDE at zoo.openzoo.fun/ide/session. Subscribe (Pay) if there is no key. Local OCC is not the Agent pane.'
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/ide/session') {
    (async () => {
  ...
  if (req.method === 'POST' && req.url === '/drive') {
    ...
    req.on('end', async () => {
      ...
      // (POST /ide/session). Chat stays on completions; RUN: waits for approve/deny.
