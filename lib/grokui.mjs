...
import {
  ...
  sanitizeClaudeCanvas, closeClaudeSession, GROKUI_RESERVED_SLASH,
} from './claudecode.js';
import {
...
function enqueueAutoHop(t, threadId, userText, onEvent) {
  ...
  // Never inject NUDGE / AUTO_CONTINUE as a user turn after that.
...
async function runTurn(threadId, userText, onEvent, images) {
  ...
  if (t.runMode === 'auto' && !t.autoStopped && !t.lastRunFailed && !isAutoStopReply(reply)
    ...
    // One nudge per user ask. Twelve "That reply announced work…" user
    // bubbles after a failed RUN is the measured loop. After a failed RUN
    // or a nudge that already fired, stop — do not inject NUDGE as a user turn.
    if (t.autoNudged) {
      t.autoStopped = true;
      return;
    }
...
export {
  ...
  persistUserTurn, persistAssistantTurn,
