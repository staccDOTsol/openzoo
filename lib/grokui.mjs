...
/** Occupied TCP ≠ healthy. 402 (Pay) is healthy. */
async function sidecarSessionReady() {
  if (sidecarSessionOverride) return sidecarSessionOverride();
  try {
    const r = await fetch(SIDECAR_SESSION_URL, { signal: AbortSignal.timeout(2000) });
...
function agentPtySpawnSpec({
  ...
  return {
    ...
    args: [...resolved.prefixArgs, ...claudeInteractiveArgs({ model: claudeModelArg(model) })],
    via: resolved.via,
...
/** OCC only treats slashes at a `>` prompt. Interrupt first (ESC), then the
 *  line + CR. Never put `\r` / `\n` / `\x1b` in APP_HTML — callers use this. */
...
async function writeAgentPtyLine(threadId, line) {
  const sess = ensureAgentPty(threadId);
  if (!sess) return false;
  sess.write(String.fromCharCode(27));
  await new Promise((resolve) => setTimeout(resolve, 80));
  if (sess.dead) return false;
  sess.write(stripPtyLineTail(line) + String.fromCharCode(13));
  return true;
}
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  @media (pointer: coarse) and (max-width: 720px) { #paneActions { display: none; } }
...
const server = http.createServer((req, res) => {
  ...
  }
  {
    ...
    if (req.method === 'POST' && ptyWrite) {
      ...
      req.on('data', (d) => chunks.push(d));
      req.on('end', () => {
        void writeAgentPtyLine(t.id, Buffer.concat(chunks).toString('utf8'));
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end('{"ok":true}');
      });
      return;
...
export {
  ...
  ensureAgentPty, killAgentPty, writeAgentPty, writeAgentPtyLine, agentPtySpawnSpec, setAgentPtySpawnerForTest, CLAUDE_SLASH_IN_AUTO,
