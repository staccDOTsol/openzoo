...
// they want, a fact only they have. "Should I proceed?" is a stalled offer,
...
function shouldParkAuto(reply) {
  return isDoneReply(reply) || isBlockingQuestion(reply);
}
...
function wakeOnPing(x, extra) {
  ...
  // Re-arm the AUTO budget so a kid that hit the cap actually works again.
  // runTurn sets status=thinking synchronously, so the sidebar does not
  // flash "pinged" then idle before the hop starts.
  x.autoSteps = 0;
  delete x.autoNudged;
...
async function runTurn(threadId, userText, onEvent, images) {
  ...
  // AUTO DEFAULT: keep going.
  //
  // A kid that RAN ls, saw the listing, and said "ok" used to go idle —
  // MEASURED: one ping, one RUN, status=idle, job unfinished. A race that
  // failed entirely posted `(race: every model failed)` and parked the
  // parent the same way. AUTO means: do not stop unless DONE:, a real
  // blocking question, ask mode, or the step cap.
  if (t.runMode === 'auto' && !shouldParkAuto(reply)) {
...
export {
  ...
  AUTO_CONTINUE, pingWakeText, pingCanWake, shouldParkAuto, isBlockingQuestion,
  isRaceFailReply, childKickoff, findByName, attachChildDir, finishChildDir,
