...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  #agentPreview .html-preview-open { padding: 4px 10px; }
...
<body>
  ...
  <div id="main">
    ...
    <div id="agentPreview">
      ...
      <iframe id="agentPreviewFrame" class="html-preview" sandbox="allow-scripts allow-same-origin allow-pointer-lock allow-forms" title="preview"></iframe>
    </div>
...
<script>
  ...
  function hideAgentPreview() {
    var box = document.getElementById('agentPreview');
    if (box) box.classList.remove('show');
  }
  function showAgentPreview(url, rel) {
    var box = document.getElementById('agentPreview');
  ...
  function connectStream(id) {
    ...
    es.onmessage = (e) => {
      ...
      if (ev.type === 'preview' && ev.url) {
        showAgentPreview(ev.url, ev.rel);
        return;
      }
      if (ev.type === 'start') {
        streamBuf = '';
