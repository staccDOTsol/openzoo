...
async function runTurn(threadId, userText, onEvent, images) {
  ...
  if (t.members) {
    ...
    for (const m of t.members) {
      ...
      try {
        ...
        r = (isRetryableFetchError(e) || /fetch failed/i.test(String(e && e.message)))
          ? (noteSidecarUnreachable(), SIDECAR_STARTING)
          : `error: ${e.message}`;
      }
      if (!stillMine()) return;
  ...
  try {
    ...
    if (isRetryableFetchError(e) || /fetch failed/i.test(String(e && e.message))) {
      noteSidecarUnreachable();
      reply = SIDECAR_STARTING;
    } else {
