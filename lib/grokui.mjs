...
// Quiet Claude TUI / --model miss / chrome-only canvas / missing CLI.
// 1.5.99–1.6.0 PTY Auto treated that as a finished turn (or skipped PTY
// until a prior bot row existed, then swallowed send #2+). Empty PTY,
// "(no response)", HTTP-N, and missing CLI fall through to Ask/Auto
// chat-completions. Do not gate that on "thread already has a bot row".
function isClaudeFallbackReply(text) {
  ...
  if (s === CLAUDE_MISSING) return true;
  return false;
}
 ...
 */
const AUTO_CLAUDE_PTY_MS = Number(process.env.OZ_AUTO_CLAUDE_PTY_MS || 3000);
const ENSURE_HARNESS_SEND_MS = 2500;

// Hung PTY used to await forever, so persistUserTurn saved "hi" and
// completions never ran (UI stuck on "waiting on model…"). Cap the PTY
// on its own AbortController — do not abort turnAbort; completions use that.
async function runClaudeCodeBounded(opts, ms) {
  const cap = Math.max(50, Number(ms ?? process.env.OZ_AUTO_CLAUDE_PTY_MS ?? AUTO_CLAUDE_PTY_MS));
  ...
  const fallback = {
    text: '(no response)', error: false, missing: false, paymentFailed: '', sessionId: '',
  };
  let timer;
  const timeout = new Promise((resolve) => {
    timer = setTimeout(() => {
      try { ptyAbort.abort(); } catch { /* already */ }
      resolve(fallback);
    }, cap);
  });
  let pty;
  try {
    pty = Promise.resolve(runClaudeCode({ ...opts, signal: ptyAbort.signal })).catch(() => fallback);
  } catch {
    pty = Promise.resolve(fallback);
  }
  try {
    return await Promise.race([pty, timeout]);
  } catch {
    return fallback;
  } finally {
    clearTimeout(timer);
...
async function runAutoClaudeTurn(t, userText, images, paint, stillMine, turnAbort) {
  ...
  // Empty / chrome-only / HTTP-N / missing CLI: do not commit a dead bot
  // row and do not return as success. runTurn falls through to Ask/Auto
  // chat-completions. Missing used to paint CLAUDE_MISSING and idle.
  // A Task fan-out IS the turn — compact "Messaged N Bots", not a swallow.
  if ((isClaudeFallbackReply(finalText) || result.missing) && !result.paymentFailed) {
    if (!hopText) return finalText;
    finalText = hopText;
  }
...
async function runTurn(threadId, userText, onEvent, images) {
  ...
  // Try PTY, then completions if that turn is empty / missing / HTTP-N / no
  // NEW visible bot row. Do not skip PTY until a prior bot exists (1.6.0
  // ate send #2+), and do not return as success on an empty canvas.
  if (t.runMode === 'auto') {
    ...
    if (!shouldSkipHarnessAutostart()) {
      ...
      } catch { /* completions still run */ }
    }
    const botsBefore = visibleBotCount(t);
    lastReply = await runAutoClaudeTurn(t, userText, images, paint, stillMine, turnAbort);
    if (!stillMine()) return;
    if (autoClaudeTurnProducedVisible(t, botsBefore, lastReply)) {
      usedClaude = true;
      bindThread(t).catch(() => {});
      return;
    }
...
export {
  ...
  autoClaudeTurnProducedVisible, visibleBotCount,
