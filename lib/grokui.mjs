...
import {
  looksLikeDirectiveAsBash, isFailedExecOutput, normalizeBashBody, grepFingerprint,
  nearIdenticalBash, parseWriteDirective, writeDirectiveStopText,
} from './bashloop.js';
...
// Soft hop counter per user message. Each hop is a paid call. This is a hard
// stop — finally / stale-thinking / enqueueAutoHop must not kick after the cap.
// A paying user burned money because AUTO_CONTINUE kept firing after errors.
...
// Ping/resume on Auto-Claude — not AUTO_CONTINUE. That string tells the
// model to emit WRITE:/RUN: and then bash `WRITE:5d_chess.py`.
const AUTO_CLAUDE_WAKE = '(resume Claude Code session)';
...
const AUTO_EMPTY_PTY_STOP = 'Auto stopped — Claude Code returned no output (empty PTY). Not retrying.';
const AUTO_SAME_COMMAND_STOP = 'Auto stopped — the same command ran twice. Not retrying.';
...
function isUpstreamHttpError(text) {
  const s = String(text || '').trim();
  if (!s) return false;
  if (/\(upstream error/i.test(s)) return true;
  if (/\bupstream HTTP\s*[45]\d\d\b/i.test(s)) return true;
  if (/\bHTTP\s*5\d\d\b/i.test(s)) return true;
  return false;
}
function isEmptyModelReply(text) {
  const s = String(text || '').trim();
  return !s || s === '(no response)' || s === AUTO_EMPTY_PTY_STOP;
}
/** Fatal for AUTO — never inject AUTO_CONTINUE / AUTO_RACE_RETRY / AUTO_EMPTY_RETRY. */
function isAutoStopReply(text) {
  if (isPaymentFailed(text)) return true;
  if (isUpstreamHttpError(text)) return true;
  if (isTransientModelFail(text)) return true;
  if (isEmptyModelReply(text)) return true;
  if (String(text || '').includes(AUTO_SAME_COMMAND_STOP)) return true;
  if (isFailedExecOutput(text)) return true;
  return false;
}
function isEmptyClaudeReply(text) {
  return isEmptyModelReply(text);
}
function normalizeAutoCommand(cmd) {
  return String(cmd || '').replace(/\s+/g, ' ').trim();
}
function rememberAutoCommand(t, command) {
  if (!t) return 1;
  const key = grepFingerprint(command) || normalizeAutoCommand(command);
  if (!key) return 1;
  if (t.lastAutoCommand === key || nearIdenticalBash(t.lastAutoCommandRaw, command)) {
    t.autoCommandRuns = (t.autoCommandRuns || 1) + 1;
  } else {
    t.lastAutoCommand = key;
    t.lastAutoCommandRaw = normalizeBashBody(command);
    t.autoCommandRuns = 1;
  }
  return t.autoCommandRuns;
}
function identicalCommandCapped(t, command) {
  if (!t) return false;
  if (command != null && t.lastAutoCommandRaw && nearIdenticalBash(t.lastAutoCommandRaw, command)) {
    return (t.autoCommandRuns || 0) >= 2;
  }
  const key = command != null ? (grepFingerprint(command) || normalizeAutoCommand(command)) : t.lastAutoCommand;
  if (!key || t.lastAutoCommand !== key) return false;
  return (t.autoCommandRuns || 0) >= 2;
}
...
// Park on: ask mode, pendingRun, DONE:, 5xx/402/empty-wallet/paymentFailed/
// "(upstream error"/empty completion, identical-command cap, or AUTO_MAX_STEPS.
// Empty exec may hop once (AUTO_EMPTY_RETRY); a second identical RUN stops.
function shouldKeepAuto(t, reply, userText) {
  if (!t || t.runMode !== 'auto') return false;
  if (t.pendingRun || t.autoStopped) return false;
  if ((t.autoSteps || 0) >= AUTO_MAX_STEPS) return false;
  if (t.lastRunFailed) return false;
  if (isAutoStopReply(reply) || isAutoStopReply(userText)) return false;
  if (isFailedExecOutput(reply) || isFailedExecOutput(userText)) return false;
  if (isDoneReply(reply)) return false;
  if (identicalCommandCapped(t)) return false;
  return true;
}
function persistAssistantTurn(t, text) {
  if (!t || text == null) return;
  if (Array.isArray(t.messages)) {
    const last = t.messages[t.messages.length - 1];
    if (!(last && last.role === 'assistant' && last.content === text)) {
      t.messages.push({ role: 'assistant', content: String(text) });
    }
  }
  saveThreads();
}
/** Flush Claude Code's session id as soon as init arrives so a later hop
 *  can `--resume` and see work already done (fate.so, compile artifacts). */
function persistClaudeSession(t, sessionId) {
  const sid = String(sessionId || '').trim();
  if (!t || !sid) return false;
  if (t.claudeSessionId === sid) return false;
  t.claudeSessionId = sid;
  saveThreads();
  return true;
}
function isHarnessContinueText(text) {
  const s = String(text || '');
  return s === NUDGE || s === AUTO_CONTINUE || s === AUTO_RACE_RETRY
    || s === AUTO_EMPTY_RETRY || s.startsWith('AUTO_EMPTY_RETRY:');
}
/** Ask-mode harness injections. Claude Code on Auto must never see these
 *  as user turns — they tell it to emit WRITE:/RUN: as the first line,
 *  then it bash's `WRITE:5d_chess.py` (command not found). */
function isAskHarnessInjection(text) {
  const s = String(text || '');
  if (!s) return false;
  if (isHarnessContinueText(s)) return true;
  if (s === AUTO_DIRECTIVE) return true;
  if (/AUTO MODE IS ON for this thread/.test(s)) return true;
  if (/Emit the directive NOW/.test(s) && /WRITE:|RUN:/.test(s)) return true;
  if (/Emit the next directive now/.test(s) && /WRITE:|RUN:/.test(s)) return true;
  return false;
}
function enqueueAutoHop(t, threadId, userText, onEvent) {
  if (!t || t.autoStopped) return false;
  // Auto-Claude must never receive NUDGE / AUTO_CONTINUE / AUTO_DIRECTIVE
  // as a user turn. A missing WRITE: line is not a failure — Claude has Write.
  if (t.runMode === 'auto' && isAskHarnessInjection(userText)) {
    t.autoStopped = true;
    saveThreads();
    return false;
  }
  ...
  // Failed RUN (syntax / command not found) is DONE-for-this-hop.
  // Never inject AUTO_CONTINUE as a user turn after that.
  if (t.lastRunFailed && isHarnessContinueText(userText)) {
    t.autoStopped = true;
    saveThreads();
    return false;
  }
  if ((t.autoSteps || 0) >= AUTO_MAX_STEPS) {
    t.autoStopped = true;
    saveThreads();
    return false;
  }
  if (identicalCommandCapped(t) && (userText === AUTO_EMPTY_RETRY
      || String(userText || '').startsWith('AUTO_EMPTY_RETRY:'))) {
    t.autoStopped = true;
    saveThreads();
    return false;
  }
  // Persist history even if bind fails — the next hop must remember this turn.
  saveThreads();
  const bindP = bindThread(t).catch(() => {});
  t.autoSteps = (t.autoSteps || 0) + 1;
  if (t.autoSteps > AUTO_MAX_STEPS) {
    t.autoStopped = true;
    t.autoSteps = AUTO_MAX_STEPS;
    saveThreads();
    return false;
  }
  bindP.finally(() => {
...
function isClaudeResume(text) {
  return String(text || '') === AUTO_CLAUDE_WAKE;
}
function wakeOnPing(x, extra) {
  // Never childKickoff. Ping is a short continue, not a first-day re-brief.
  // Auto-Claude must not receive AUTO_CONTINUE as a user turn — that string
  // tells it to emit WRITE:/RUN: and then bash `WRITE:5d_chess.py`.
  const text = pingWakeText(extra);
  if (x?.runMode === 'auto' && isAskHarnessInjection(text)) {
    kickTurn(x.id, AUTO_CLAUDE_WAKE).catch(() => {});
    return;
  }
  kickTurn(x.id, text).catch(() => {});
}
...
function parseRun(reply) {
  ...
  for (let i = 0; i < heads.length; i++) {
    ...
    // WRITE:/READ:/… as a RUN body becomes `WRITE:5d_chess.py: command not found`.
    if (cmd && looksLikeDirectiveAsBash(cmd)) return null;
...
function applyWriteDirective(originId, text, cwd) {
  const parsed = parseWriteDirective(text);
  if (!parsed?.path) return null;
  if (parsed.content == null) return writeDirectiveStopText(parsed);
  const root = cwd || (originId ? dirFor(originId) : '');
  if (!root) return writeDirectiveStopText(parsed);
  try {
    const full = safeResolveIn(root, parsed.path);
    mkdirSync(path.dirname(full), { recursive: true });
    writeFileSync(full, parsed.content);
    if (originId) noteFileForCorpus(originId, full);
    return `Wrote ${parsed.path} (${Buffer.byteLength(parsed.content)} bytes).`;
  } catch (e) {
    return `Couldn't write ${parsed.path}: ${e.message}`;
  }
}

function execCommand(command, cwd) {
  if (looksLikeProxyShell(command)) return Promise.resolve(PROXY_SHELL_REFUSE);
  const write = parseWriteDirective(command);
  if (write) return Promise.resolve(applyWriteDirective(null, command, cwd) || writeDirectiveStopText(write));
  if (looksLikeDirectiveAsBash(command)) {
    return Promise.resolve(`refused: ${String(command).trim().split(/\s/)[0]} is the Ask-mode harness, not a shell command.`);
  }
...
setInterval(() => {
  ...
  for (const t of threads.values()) {
    ...
    if (shouldKeepAuto(t, lastText) && (t.autoSteps || 0) < AUTO_MAX_STEPS && !t.autoStopped) {
      enqueueAutoHop(t, t.id, autoHopText(lastText));
    } else {
      t.status = 'idle';
      t.liveStatus = '';
      t.liveRace = null;
...
// Flush the user's turn to history + messages BEFORE any model call or hop.
// Compatible with persist #72 persist-before-send — do not revert that.
// Harness continues are not user bubbles; they still go into t.messages so
// the next hop remembers command output.
function persistUserTurn(t, userText, images) {
  if (!t) return { ok: false };
  // NUDGE / AUTO_CONTINUE / AUTO_DIRECTIVE are never user turns.
  if (isAskHarnessInjection(userText) || isClaudeResume(userText)) {
    saveThreads();
    return { ok: true, skipped: true };
  }
  if (isHarnessUserText(userText)) {
    if (Array.isArray(t.messages)) {
  ...
  const same = last && last.who === 'user' && String(last.text || '') === String(userText || '');
  if (!same) {
    t.history.push(images && images.length
  ...
  if (Array.isArray(t.messages)) {
    ...
    const alreadyMsg = lastMsg && lastMsg.role === 'user' && (
      lastMsg.content === want
      || (typeof lastMsg.content === 'string' && lastMsg.content === String(userText || ''))
    );
    if (!alreadyMsg) t.messages.push({ role: 'user', content: want });
  }
...
function autoClaudePrompt(t, userText, images) {
  // Claude Code uses Write/Bash/Read. Ask-mode RUN:/WRITE:/SPAWN grammar,
  // NUDGE, AUTO_DIRECTIVE, and AUTO_CONTINUE must never reach the PTY.
  if (isAskHarnessInjection(userText) || isClaudeResume(userText)) return '';
  const bits = [];
...
async function runAutoClaudeTurn(t, userText, images, paint, stillMine, turnAbort) {
  ...
  const result = await runClaudeCode({
    ...
    onEvent(folded) {
      ...
      if (folded.sessionId) persistClaudeSession(t, folded.sessionId);
  ...
  if (result.sessionId) persistClaudeSession(t, result.sessionId);
  const thinkAll = [thinking, ...toolLines].filter(Boolean).join('\n');
  let finalText = result.paymentFailed
    || result.text
    || visible
    || (result.missing ? CLAUDE_MISSING : '');
  const wrote = applyWriteDirective(t.id, result.text || visible || '');
  if (!result.paymentFailed && !result.missing) {
    finalText = sanitizeClaudeCanvas(finalText, { error: result.error });
  }
  if (wrote && /^Wrote /.test(wrote)) {
    finalText = finalText && !finalText.includes(wrote) ? `${finalText}\n${wrote}` : (finalText || wrote);
  }
  if (!finalText) finalText = result.error ? 'upstream HTTP 400' : AUTO_EMPTY_PTY_STOP;
...
async function runTurn(threadId, userText, onEvent, images) {
  ...
  if (t.members) {
    ...
    for (const m of t.members) {
      ...
      if (runCmd) {
        ...
        if (t.runMode === 'auto') {
          ...
          const runs = rememberAutoCommand(t, command);
          const shown = `$ ${command}\n${output}`;
          const failed = isFailedExecOutput(output);
          const stopSame = runs >= 2;
          if (failed) t.lastRunFailed = true;
          const botText = (stopSame || failed)
            ? `${command}\n${failed ? 'Auto stopped — command failed (syntax / not found). Not retrying.' : AUTO_SAME_COMMAND_STOP}`
            : command;
          t.history.push({
            who: 'bot', text: botText, runStatus: 'done', runOutput: output,
            name: m.name, color: m.color, thinking: memberThink.thinking,
          });
          paint({ type: 'final', name: m.name, color: m.color, text: botText, thinking: memberThink.thinking });
          memberReply = (stopSame || failed)
            ? `${shown}\n${failed ? 'Auto stopped — command failed (syntax / not found). Not retrying.' : AUTO_SAME_COMMAND_STOP}`
            : shown;
          if (stopSame || failed) t.autoStopped = true;
    ...
    persistAssistantTurn(t, memberReply);
    saveThreads();
  ...
  if (t.runMode === 'auto') {
    usedClaude = true;
    if (isAskHarnessInjection(userText)) {
      t.autoStopped = true;
      lastReply = 'Auto stopped — not injecting Ask-mode WRITE:/RUN: guidelines into Claude Code.';
      persistAssistantTurn(t, lastReply);
      saveThreads();
      return;
    }
    if (!isClaudeResume(userText)) persistUserTurn(t, userText, images);
    saveThreads();
    lastReply = await runAutoClaudeTurn(t, isClaudeResume(userText) ? '' : userText, images, paint, stillMine, turnAbort);
    persistAssistantTurn(t, lastReply);
    saveThreads();
    try { await bindThread(t); } catch { /* persist already flushed */ }
    saveThreads();
    // Empty PTY / 5xx / pay: canvas shows the error. Do not hop AUTO_CONTINUE
    // and do not PTY-retry the same prompt.
    if (isAutoStopReply(lastReply) || isEmptyClaudeReply(lastReply)) t.autoStopped = true;
    return;
  }
  ...
  if (!isHarnessUserText(userText) && !/^\((command output|directive result)\)/.test(userText)) {
    t.autoSteps = 0;
    delete t.autoNudged;
    delete t.autoStopped;
    delete t.lastRunFailed;
    delete t.lastAutoCommand;
    delete t.lastAutoCommandRaw;
    delete t.autoCommandRuns;
  }
  // Real user text was already flushed in persistUserTurn (history + messages).
  // Harness hops still need the continue in t.messages (persistUserTurn does that).
  if (isHarnessUserText(userText) && Array.isArray(t.messages)) {
    const lastMsg = t.messages[t.messages.length - 1];
    const want = contentFor(userText, images);
    if (!(lastMsg && lastMsg.role === 'user' && lastMsg.content === want)) {
      t.messages.push({ role: 'user', content: want });
    }
  }
  let reply = '';
  ...
  // AUTO_DIRECTIVE tells the model to emit WRITE:/RUN: as the first line.
  // That is Ask-mode harness grammar. Orange Auto is Claude Code — never
  // inject it here (and never as a user turn).
  ...
  try {
    ...
    for (let i = 0; !isAutoStopReply(reply) && (!reply || isTransientModelFail(reply)) && i < AUTO_EMPTY_RETRIES; i++) {
      paint({ type: 'status', name: t.name, color: t.color, detail: 'retrying…' });
      await new Promise((r) => setTimeout(r, 400 * (i + 1)));
      if (!stillMine()) return;
      reply = await ask(t.model ? 0 : i + 1);
      if (isAutoStopReply(reply)) break;
    }
    if (!reply) {
      reply = t.model
  ...
  if (runCmd) {
    ...
    if (t.runMode === 'auto') {
      ...
      const stopSame = runs >= 2;
      if (failed) t.lastRunFailed = true;
      const stopMsg = failed
        ? 'Auto stopped — command failed (syntax / not found). Not retrying.'
        : AUTO_SAME_COMMAND_STOP;
      const botText = (stopSame || failed) ? `${command}\n${stopMsg}` : command;
      t.history.push({ who: 'bot', text: botText, runStatus: 'done', runOutput: output, thinking: settled.thinking });
      paint({ type: 'final', name: t.name, color: t.color, text: botText, thinking: settled.thinking });
      lastReply = (stopSame || failed) ? `${shown}\n${stopMsg}` : shown;
      persistAssistantTurn(t, lastReply);
      saveThreads();
      if (stopSame || failed) {
        t.autoStopped = true;
      ...
      // FEED THE OUTPUT BACK. Cap identical command retries at 1, then stop.
      // BIND BEFORE CHAINING. Persist history even if bind fails.
  ...
  persistAssistantTurn(t, finalText);
  lastReply = finalText;
  ...
  if (t.runMode === 'auto' && !t.autoStopped && ack !== null && ack !== undefined
      && !isAutoStopReply(reply) && !isAutoStopReply(ack)
  ...
  // ANNOUNCING IS NOT DOING — and injecting NUDGE as a user turn is worse.
  // Measured loop: twelve "That reply announced work…" user bubbles after a
  // failed RUN. Claude Code's Bash path is the product. Do not invent a
  // follow-up user message; stop and let the user continue if they want.
  if (t.runMode === 'auto' && !t.autoStopped && !t.lastRunFailed && !isAutoStopReply(reply)
      && (ack === null || ack === undefined)
      && (STALLED_OFFER.test(reply) || ANNOUNCEMENT.test(reply))) {
    t.autoNudged = true;
    t.autoStopped = true;
    persistAssistantTurn(t, reply);
    saveThreads();
    return;
  }

  // After any auto reply that is not DONE: and not waiting on approval,
  // kick immediately — unless the reply is a 5xx / 402 / empty / pay stop.
  ...
  bindThread(t).catch(() => {});
    ...
    // Idle when this hop must not keep AUTO going. AUTO_MAX_STEPS is a hard
    // stop — do not fall through and kick again after the cap.
    const atCap = (t.autoSteps || 0) >= AUTO_MAX_STEPS;
    if (stillMine() && !chained && !parked) {
      if (!usedClaude && !t.autoStopped && !atCap
          && shouldKeepAuto(t, lastReply, userText)) {
...
export {
  ...
  parseRun, looksLikeMcpAsBash, looksLikeDirectiveAsBash, isFailedExecOutput, isHarnessContinueText,
  stripThinkTags, takeThink, safeResolveIn, inDir, listDir,
  ...
  AUTO_CONTINUE, AUTO_RACE_RETRY, AUTO_EMPTY_RETRY, AUTO_EMPTY_PTY_STOP, AUTO_SAME_COMMAND_STOP, NUDGE,
  pingWakeText, pingCanWake, shouldKeepAuto, isAutoStopReply, rememberAutoCommand, identicalCommandCapped,
  persistUserTurn, persistAssistantTurn, persistClaudeSession, claudeModelArg,
  isAskHarnessInjection, applyWriteDirective, parseWriteDirective, AUTO_DIRECTIVE,
