...
function enqueueAutoHop(t, threadId, userText, onEvent) {
  ...
  // Never inject NUDGE as a user turn. Measured loop: twelve "That reply
  // announced work…" bubbles after a failed RUN. Claude Code's Bash path
  // is the product — do not invent follow-up user messages.
  if (userText === NUDGE) {
    t.autoStopped = true;
    saveThreads();
    return false;
...
function execCommand(command, cwd) {
  ...
  if (looksLikeDirectiveAsBash(command)) {
    return Promise.resolve(`${String(command).split('\n')[0]}: command not found\n(exit 127)\nAuto stopped — that is a harness directive (WRITE/READ/…), not a shell command.`);
  }
...
function persistUserTurn(t, userText, images) {
  ...
  // NUDGE is never a user turn — not in history, not in messages.
  if (String(userText || '') === NUDGE) {
    saveThreads();
