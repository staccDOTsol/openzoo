...
import { peekDirectiveStatus, formatRaceStatus, formatSitrep, STALE_THINKING_MS, summarizeRaceFailures, RACE_EVERY_FAILED } from './livestatus.js';
...
const SLASH_COMMANDS = [
  ...
  { name: '/sitrep', args: '', help: 'session sitrep (thread + drawer)' },
...
async function handleSlash(task, t) {
  ...
  if (cmd === 'sitrep') {
    ...
    return formatSitrep({
      race: t.race, raceNeed: t.raceNeed, tier: t.tier, runMode: t.runMode,
      dir: t.dir, status: t.status, liveStatus: t.liveStatus, liveRace: t.liveRace,
      spentUsd: s?.spentUsd, cogsUsd: s?.cogsUsd, directUsd: s?.directUsd,
      paidCalls: s?.paidCalls, creditUsd: s?.creditUsd,
    });
  }
  if (cmd === 'help') {
    return 'Commands:\n'
...
<script>
  ...
  async function runSitrep() {
    ...
    await fetch(API + '/drive', {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ threadId: activeId, task: '/sitrep' }),
    });
    render();
  }
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/drive') {
    ...
    req.on('end', async () => {
      ...
      // Reply AFTER the dump is on the thread so /sitrep (and /cost, /help)
      // is in history when the client re-renders — not a mute toast.
      if (t && /^\//.test(task.trim())) {
