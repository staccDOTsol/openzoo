...
const USER_DIRECTIVE_LINE = /^[ \t>*-]*(?:SPAWN|SEND|PING|PEEK|WRITE|READ|EDIT|MULTIEDIT|NOTEBOOK|LS|LIST|DIR|GLOB|FIND|GREP|TODO|SERVE|FETCH|MCP|RUN|DONE):\s*/i;

function isToolDumpText(text) {
  const s = String(text ?? '');
  if (/^\((command output|directive result)\)/.test(s.trim())) return true;
  if (/^\$ [^\n]+\n/.test(s)) return true;
  return false;
}

function isCrewHopAck(text) {
  const s = String(text ?? '').trim();
  if (!s) return false;
  if (/^Spawned /.test(s)) return true;
  if (/^Messaged /.test(s)) return true;
  if (/ already exists — woke it/.test(s)) return true;
  if (/ did not exist — spawned it/.test(s)) return true;
  if (/^Not spawning /.test(s)) return true;
  if (/^Cannot create "/.test(s)) return true;
  if (/: pinged, working/m.test(s)) return true;
  if (/ is still working\.?$/m.test(s)) return true;
  if (/: still working/m.test(s)) return true;
  if (/: BLOCKED — waiting for approval/m.test(s)) return true;
  if (/^Sent down your branch to /.test(s)) return true;
  return false;
}

function crewHopNames(text) {
  const s = String(text ?? '').trim();
  let m = /^Spawned (\d+) together[^:]*:\s*(.+)$/s.exec(s);
  if (m) return m[2].split(',').map((x) => x.trim()).filter(Boolean);
  m = /^Spawned ([^—\n]+) —/.exec(s);
  if (m) return [m[1].trim()];
  m = /^Messaged ([^.]+)\.$/.exec(s);
  if (m) return [m[1].trim()];
  m = /^Sent down your branch to \d+ bot\(s\):\s*(.+)$/.exec(s);
  if (m) return m[1].split(',').map((x) => x.trim()).filter(Boolean);
  m = /^(.+) already exists —/.exec(s);
  if (m) return [m[1].trim()];
  m = /^(.+) did not exist —/.exec(s);
  if (m) return [m[1].trim()];
  const names = [];
  for (const line of s.split('\n')) {
    const lm = /^\s*([^:]+): (?:pinged, working|still working|BLOCKED)/.exec(line);
    if (lm) names.push(lm[1].trim());
    const still = /^(.+) is still working/.exec(line.trim());
    if (still) names.push(still[1].trim());
  }
  return names;
}

function formatCrewSummary(names, messageCount) {
  const uniq = [...new Set((names || []).filter((n) => n && n !== '_'))];
  const n = Number(messageCount) || names.length || 0;
  const m = uniq.length || n;
  if (m <= 1 && n <= 1) return uniq[0] ? `Messaged ${uniq[0]}` : 'Messaged 1 Bot';
  if (n <= m) return `Messaged ${m} Bots`;
  return `${n} messages with ${m} Bots`;
}

function isToolResultAck(text) {
  const s = String(text ?? '').trim();
  if (!s || isCrewHopAck(s)) return false;
  if (/^(Wrote|Edited|Couldn't |Serving |Preview:|GLOB |GREP |MULTIEDIT |NOTEBOOK |TODO[:\s]|MCP |EDIT |Done\b)/.test(s)) return true;
  if (/^[^:\n]+\.\w+:\n/.test(s)) return true;
  if (/^\S+ \((?:idle|thinking|blocked)\):\n/i.test(s)) return true;
  return false;
}

function stripDirectiveLines(text) {
  return String(text ?? '').split('\n').filter((line) => !USER_DIRECTIVE_LINE.test(line)).join('\n').trim();
}

function visibleChatText(text) {
  if (isHarnessUserText(text) || isToolDumpText(text) || isCrewHopAck(text) || isToolResultAck(text)) return '';
  return stripDirectiveLines(text);
}

function quietWorkingStatus(detail) {
  const s = String(detail || '');
  if (USER_DIRECTIVE_LINE.test(s)) return 'working…';
  return s;
}

function visibleTranscript(history) {
  const out = [];
  let hopNames = [];
  let hopCount = 0;
  const flush = () => {
    ...
    out.push({
      who: 'bot',
      text: formatCrewSummary(hopNames, hopCount),
      kind: 'crew',
      crewCount: new Set(hopNames.filter((n) => n && n !== '_')).size,
      messageCount: hopCount,
    });
    hopNames = [];
    hopCount = 0;
  };
  for (const h of history || []) {
    if (h.runId && h.runStatus === 'pending') {
      flush();
      out.push({ ...h, runQuiet: true });
      continue;
    }
    if (h.runId) continue;
    const text = String(h.text || '');
    if (h.who === 'user') {
      if (isHarnessUserText(text)) continue;
      flush();
      out.push(h);
      continue;
    }
    if (isToolDumpText(text) || isToolResultAck(text)) continue;
    if (h.kind === 'crew' || isCrewHopAck(text)) {
      const names = crewHopNames(text);
      hopNames.push(...(names.length ? names : ['_']));
      hopCount += Math.max(names.length, 1);
      continue;
    }
    const chat = visibleChatText(text);
    if (!chat && !(h.images && h.images.length)) continue;
    flush();
    out.push(chat && chat !== text ? { ...h, text: chat } : h);
  }
  flush();
  return out;
}

function recordBotTurn(t, { reply, ack, paint, name, color }) {
  const extra = {};
  if (name) extra.name = name;
  if (color) extra.color = color;
  const chat = visibleChatText(reply);
  if (chat) t.history.push({ who: 'bot', text: chat, ...extra });
  if (ack && isCrewHopAck(ack)) t.history.push({ who: 'bot', text: ack, kind: 'crew', ...extra });
  const shown = chat || (isCrewHopAck(ack) ? ack : '');
  paint?.({ type: 'final', name: name || t.name, color: color || t.color, text: shown });
  return shown;
}
...
async function runTurn(threadId, userText, onEvent, images) {
    ...
    for (const m of t.members) {
      ...
      if (runCmd) {
        ...
        if (t.runMode === 'auto') {
          ...
          const shown = `$ ${command}\n${output}`;
          const chat = visibleChatText(r);
          if (chat) t.history.push({ who: 'bot', text: chat, name: m.name, color: m.color });
          paint({ type: 'final', name: m.name, color: m.color, text: chat });
          memberReply = shown;
      ...
      recordBotTurn(t, { reply: r, ack, paint, name: m.name, color: m.color });
      memberReply = r;
    }
    bindThread(t).catch(() => {});
  ...
  if (runCmd) {
    ...
    if (t.runMode === 'auto') {
      ...
      const shown = `$ ${command}\n${output}`;
      const chat = visibleChatText(reply);
      if (chat) t.history.push({ who: 'bot', text: chat });
      paint({ type: 'final', name: t.name, color: t.color, text: chat });
  ...
  if (!stillMine()) return;
  recordBotTurn(t, { reply, ack, paint });
...
function threadSummary(t) {
  const vis = visibleTranscript(t.history);
  ...
  return { id: t.id, name: t.name, color: t.color, parent: t.parent, status: t.status,
    ...
    liveStatus: t.status === 'thinking' ? quietWorkingStatus(t.liveStatus || '') : '',
...
<style>
  ...
  #bootSplash { position: fixed; inset: 0; z-index: 4000; background: #000; color: #6f7080;
    display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 16px;
    font: 12px/1.4 -apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif;
    letter-spacing: .06em; -webkit-app-region: drag; }
  #bootSplash.gone { opacity: 0; pointer-events: none; transition: opacity .2s ease; }
  .bootmark { display: flex; gap: 6px; }
  .bootmark span { width: 6px; height: 6px; border-radius: 50%; background: #ececec;
    opacity: .22; animation: bootdot 1.2s infinite ease-in-out; }
  .bootmark span:nth-child(2) { animation-delay: .15s; }
  .bootmark span:nth-child(3) { animation-delay: .3s; }
  @keyframes bootdot { 0%, 80%, 100% { opacity: .18; } 40% { opacity: 1; } }
  #bootStatus { color: #6f7080; font-size: 12px; }
  .crewchip { display: inline-flex; align-items: center; gap: 8px; background: #131315;
    border: 1px solid #2c2c2e; color: #8e8e93; border-radius: 999px; padding: 5px 11px;
    font-size: 12px; letter-spacing: .02em; }
  .jobcard { display: inline-flex; align-items: center; gap: 10px; background: #111113;
    border: 1px solid #1c1c1e; color: #6f7080; border-radius: 12px; padding: 8px 12px; font-size: 12px; }
  @media (prefers-reduced-motion: reduce) {
    .bootmark span { animation: none; opacity: .5; }
    #bootSplash.gone { transition: none; }
  }
</style>
<style>
...
<body>
  <div id="bootSplash" role="status" aria-live="polite">
    <div class="bootmark" aria-hidden="true"><span></span><span></span><span></span></div>
    <div id="bootStatus">starting…</div>
  </div>
...
<script>
  ...
  async function openSitrep() {
    ...
    else if (thinking && (full && full.liveStatus)) flight = quietWorkingStatus(full.liveStatus);
    else if (thinking && t.liveStatus) flight = quietWorkingStatus(t.liveStatus);
  ...
  function quietWorkingStatus(s) {
    var t = String(s || '');
    if (/^[ \t>*-]*(SPAWN|SEND|PING|PEEK|WRITE|READ|EDIT|MULTIEDIT|NOTEBOOK|LS|LIST|DIR|GLOB|FIND|GREP|TODO|SERVE|FETCH|MCP|RUN|DONE):/i.test(t)) return 'working…';
    return t;
  }
  function stripDirectiveLines(text) {
    return String(text || '').split('\n').filter(function (line) {
      return !/^[ \t>*-]*(SPAWN|SEND|PING|PEEK|WRITE|READ|EDIT|MULTIEDIT|NOTEBOOK|LS|LIST|DIR|GLOB|FIND|GREP|TODO|SERVE|FETCH|MCP|RUN|DONE):/i.test(line);
    }).join('\n').trim();
  }
  ...
  window.setBootStatus = function (msg) {
    var el = document.getElementById('bootStatus');
    if (el && msg) el.textContent = msg;
  };

  let lastSpeaker = null;
  function addRow(who, text, color, name, run, images, kind) {
  ...
  async function render() {
    ...
    for (const h of full.history) {
      addRow(h.who, h.text, h.color || t.color, h.name || t.name,
        h.runId ? { id: h.runId, status: h.runStatus, output: '' } : undefined, h.images, h.kind);
    }
  ...
  function liveBubbleHtml() {
    ...
    if (streamBuf) {
      var vis = stripDirectiveLines(stripThinkTags(streamBuf));
      if (vis) return escapeHtml(vis);
    }
    const dots = '<span class="dots"><span></span><span></span><span></span></span>';
    const st = quietWorkingStatus(streamStatus);
    return '<div class="jobcard">' + dots + (st ? ' <span class="tstatus">' + escapeHtml(st) + '</span>' : '') + '</div>';
  }
  ...
  tick().then(dismissBootSplash, dismissBootSplash);
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && req.url.startsWith('/threads/')) {
    ...
    res.end(t ? JSON.stringify({
      id: t.id, history: visibleTranscript(t.history), status: t.status,
      liveStatus: t.status === 'thinking' ? quietWorkingStatus(t.liveStatus || '') : '',
...
export {
  ...
  visibleTranscript, formatCrewSummary, visibleChatText, isCrewHopAck, quietWorkingStatus,
};
