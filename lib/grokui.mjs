...
export function setAgentPtySpawnerForTest(fn) {
  agentPtySpawner = typeof fn === 'function' ? fn : null;
}

export function killAgentPty(threadId) {
...
export function agentPtySpawnSpec({
...
export function ensureAgentPty(tOrId, opts = {}) {
...
export function writeAgentPty(threadId, data) {
...
<script>
  ...
  function renderHeader(t) {
    document.getElementById('chatHeaderId').innerHTML =
      ...
      '">' + escapeHtml(t.dir || '') + '</div>' +
      '<button type="button" class="dirpick" id="dirPickBtn" title="Change working folder (same as /dir)">' +
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
      '<path d="M3 7h6l2 2h10v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7z"/></svg></button></div></div>';
    setModeButtons(runModeLabel(t));
    setDials(t);
    const pick = document.getElementById('dirPickBtn');
    if (pick) pick.addEventListener('click', pickThreadDir);
  }
