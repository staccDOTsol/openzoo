...
import { adaptiveTopK, brain, brainRace, brainStream, tierModels, MODEL, PROXY, TIER_NAMES, parseTier } from './podagent.mjs';
...
const SLASH_COMMANDS = [
  ...
  { name: '/tier', args: 'cheap|medium|expensive|grok4.6', help: 'how much to spend per turn when no model is pinned' },
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  #headerDials { display: flex; flex-wrap: wrap; align-items: center; gap: 6px; margin-left: auto; flex: 0 0 auto; }
  ...
  .dial { border: 1px solid #2c2c2e; background: #131315; color: #8e8e93; font: inherit; font-size: 11px;
          border-radius: 999px; padding: 4px 7px; cursor: pointer; -webkit-appearance: none; appearance: none; }
  /* Four bands (cheap / medium / expensive / grok 4.6) in one select — width
     is the selected label, not four side-by-side buttons, so race/wallet stay. */
  #tierSel { max-width: 7.6em; }
...
<script>
  ...
  function setDials(t) {
    ...
    if (!pinned && (t.tier === 'expensive' || t.tier === 'grok4.6' || (t.race || 0) >= 2)) {
