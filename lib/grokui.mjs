...
const SLASH_COMMANDS = [
  ...
  { name: '/mode', args: 'chat|agent', help: 'Agent = OCC TUI + Message composer; Chat = completions (ask|auto still work)' },
];
  ...
  if (cmd === 'mode') {
    if (!arg) {
      return `Run mode is ${runModeLabel(t)}.`
        + (isAgentMode(t)
          ? ' Agent = openzoo-claude TUI. Type in Message and press Enter — that works while tools run. Esc in the TUI interrupts.'
    ...
    saveThreads();
    return next === 'agent'
      ? 'Run mode set to agent — openzoo-claude TUI. Type in Message and press Enter (works while tools run). Esc in the TUI interrupts.'
...
<script>
  ...
  function bindPreviewFocusGuard(frame) {
    ...
    frame.setAttribute('tabindex', '-1');
    frame.addEventListener('focus', function () { bouncePreviewFocus(frame); });
    frame.addEventListener('load', function () {
      bouncePreviewFocus(frame);
      try {
        var w = frame.contentWindow;
        var doc = frame.contentDocument || (w && w.document);
        if (w) w.addEventListener('focus', function () { bouncePreviewFocus(frame); });
        if (doc) doc.addEventListener('focusin', function () { bouncePreviewFocus(frame); }, true);
      } catch (e) {}
    });
  }
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/drive') {
    ...
    req.on('end', async () => {
      ...
      // "/mode chat|agent" (ask|auto aliases). Agent is the openzoo-claude
      // PTY loop. Chat stays on completions; RUN: waits for approve/deny.
