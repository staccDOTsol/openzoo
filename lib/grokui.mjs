...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/drive') {
    ...
    req.on('end', async () => {
      ...
      // GROKROOM-BRIDGE v1: a room thread gossips to the room server instead of
      // kicking a model turn. The sync loop brings everyone's messages back.
      if (t && t.room) { grokroomSend(t, task); return; }
...
// ===== GROKROOM-BRIDGE v1 =====
// Rooms from the local gossip server (127.0.0.1:4799) as native group threads.
// One room = one sidebar thread; each remote participant = a member (no sidebar
// row of their own) so their messages bubble natively with their own name+color.
// Sends in a room thread POST a signed ed25519 envelope to the room server —
// the chain is the log. Additive: any failure here must never break chat.
import crypto from 'node:crypto';

const GROKROOM_API = process.env.OZ_GROKROOM_URL || 'http://127.0.0.1:4799';
const GROKROOM_ID_FILE = path.join(STORE_DIR, 'grokroom-identity.json');
...
function grokroomIdentity() {
  ...
  let id = null;
  try {
    const j = JSON.parse(readFileSync(GROKROOM_ID_FILE, 'utf8'));
  ...
  if (!id) {
    ...
    try { writeFileSync(GROKROOM_ID_FILE, JSON.stringify(id, null, 2)); } catch { /* in-memory is fine */ }
  }
  grokroomIdCache = id;
  return id;
}
...
async function grokroomPost(roomId, body) {
  ...
  const pub = id.pub;
  const canonical = JSON.stringify({ body, from, pub, room: roomId, ts }); // alphabetical — do not reorder
  const key = crypto.createPrivateKey({ key: Buffer.concat([GROKROOM_PKCS8_PREFIX, Buffer.from(id.seed, 'base64')]), format: 'der', type: 'pkcs8' });
  const sig = grokroomB58(crypto.sign(null, Buffer.from(canonical, 'utf8'), key));
  const r = await fetch(GROKROOM_API + '/submit', {
    method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ room: roomId, from, pub, body, ts, sig }),
    signal: AbortSignal.timeout(30000), // testnet confirm under 429 backoff can be slow
  });
  const j = await r.json().catch(() => ({}));
...
function grokroomMemberEnsure(t, name) {
  if (!t.members.some((m) => m.name === name)) {
    t.members.push({ name, color: colorFor(name), systemPrompt: '', remote: true });
  }
}

function grokroomSend(t, task) {
  const body = String(task || '').trim().slice(0, 400);
  if (!body) return;
  t.history.push({ who: 'user', text: body });
  t.lastActivityAt = Date.now();
  saveThreads();
  grokroomPost(t.room.id, body).catch((e) => {
    grokroomMemberEnsure(t, 'room');
    t.history.push({ who: 'bot', name: 'room', text: '⚠ could not reach the room: ' + e.message });
    saveThreads();
  });
}

function grokroomEnsureThread(roomId, name, addr) {
  for (const t of threads.values()) if (t.room && t.room.id === roomId) return t;
  // newThread(name, null, null): members=[] would throw on members[0].color (truthy
  // empty array), so build a plain thread and convert it to a group shape after.
  const t = newThread(name || ('# ' + roomId), null, null);
  t.messages = null;
  t.members = [];
  t.room = { id: roomId, addr: addr || '' };
  grokroomMemberEnsure(t, 'room');
  t.history.push({ who: 'bot', name: 'room',
    text: 'Linked to ' + t.name + ' on Solana testnet. Every message here is a signed memo on-chain — the chain is the log.' +
...
async function grokroomSync() {
  ...
  try {
    ...
    if (!grokroomSync._logged) { grokroomSync._logged = true; console.log('[grokroom] sync tick: rooms=' + rooms.map((r) => r.id).join(',')); }
    for (const r of rooms) {
      const roomId = r.id || r.name;
      if (!roomId) continue;
      const t = grokroomEnsureThread(roomId, r.name, r.addr || r.address);
      ...
      const seen = new Set(t.history.filter((h) => h.sig).map((h) => h.sig));
      console.log('[grokroom]', roomId, 'feedMsgs=' + msgs.length, 'history=' + t.history.length, 'seenSigs=' + seen.size);
      let dirty = false;
      for (const m of msgs) {
        ...
        if (!body) continue;
        // my own sends already echo locally as the human's bubble — if a recent
        // who:user entry carries this exact text, the feed copy is the same message
        // coming home; skip it. Stateless: survives restarts and the slow-confirm
        // race (client aborts at Ns, server still lands the tx) that dropped echoes.
        if (from === GROKROOM_HANDLE && t.history.slice(-50).some((h) => h.who === 'user' && h.text === body)) continue;
        grokroomMemberEnsure(t, from);
        const gm = t.members.find((mm) => mm.name === from);
        t.history.push({ who: 'bot', name: from, color: gm && gm.color, text: body, sig: msig, ts: m.ts || Date.now() });
        if (m.ts && m.ts > (t.lastActivityAt || 0)) t.lastActivityAt = m.ts;
        dirty = true;
      }
      if (dirty) { saveThreads(); console.log('[grokroom]', roomId, 'bubbles appended, history=' + t.history.length); }
    }
  } catch (e) { console.error('[grokroom] sync:', e.message); }
  finally { grokroomBusy = false; }
}
setInterval(grokroomSync, 4000);
setTimeout(grokroomSync, 1200);
// ===== END GROKROOM-BRIDGE =====
