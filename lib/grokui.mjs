...
import {
  ...
  PTY_PENDING, hasLiveClaudeSession,
} from './claudecode.js';
import {
...
const SLASH_COMMANDS = [
  ...
  { name: '/mode', args: 'chat|agent', help: 'Agent = openzoo-claude on a PTY; Chat = completions (ask|auto still work)' },
];
  ...
  if (cmd === 'mode') {
    if (!arg) {
      return `Run mode is ${runModeLabel(t)}.`
        + (isAgentMode(t)
          ? ' Agent = openzoo-claude on a PTY (long-running loop).'
    ...
    t.runMode = next;
    if (next !== 'agent') closeClaudeSession(t.id);
    saveThreads();
    return next === 'agent'
      ? 'Run mode set to agent — openzoo-claude on a PTY (x402). Long-running harness loop, not chat/completions.'
