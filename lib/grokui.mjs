...
import { ensureProxy } from './ensureproxy.js';
...
// Completions are ONLY http://127.0.0.1:8402/v1/chat/completions (OZ_PROXY unset).
// Standalone `node lib/grokui.mjs` used to leave :4173 up and :8402 dead —
// every turn was undici `fetch failed` / ECONNREFUSED. Start+supervise the
// proxy when this file is the process entrypoint (Electron child or checkout).
// Imports from tests must not spawn a sidecar.
{
  const entry = process.argv[1] && path.resolve(process.argv[1]);
  const self = fileURLToPath(import.meta.url);
  if (entry === self && process.env.OPENZOO_NO_PROXY !== '1') {
    ensureProxy().catch((e) => console.error('[grokui] proxy:', e.message));
  }
}
