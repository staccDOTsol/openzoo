...
function translateForeignToolCall(reply) {
  ...
  const blocks = [
    ...[...src.matchAll(/\[TOOL_CALL\]([\s\S]*?)\[\/TOOL_CALL\]/gi)].map((m) => ({ body: m[1], xml: false })),
    ...[...src.matchAll(/<tool_call>([\s\S]*?)<\/tool_call>/gi)].map((m) => ({ body: m[1], xml: true })),
  ];
  if (!blocks.length) return null;
  const out = [];
  for (const b of blocks) {
    const body = b.body;
    // The XML dialect puts the tool NAME bare, right after the open tag.
    const tool = b.xml
      ? (/^\s*([A-Za-z_][A-Za-z0-9_]*)/.exec(body) || [])[1]
      : (/tool\s*(?:=>|:)\s*"([^"]+)"/i.exec(body) || [])[1];
    if (!tool) continue;
...
async function tryDirective(reply, originId) {
  ...
  if (spawnAll.length > 1) {
    const out = [];
    for (const line of spawnAll) out.push(await tryDirective('SPAWN: ' + line, originId));
  ...
  if (spawn) {
    ...
    runTurn(sub.id, spawnBrief(threads.get(originId)) + task).catch(() => {}); // fire and forget
  ...
  if (sendM) {
    ...
    const sub = newThread(name, originId);
    runTurn(sub.id, spawnBrief(threads.get(originId)) + msg).catch(() => {});
