...
const SLASH_COMMANDS = [
  ...
  { name: '/race', args: '<0-4>', help: 'ask N random models from the tier at once, fastest real answer wins' },
...
async function handleSlash(task, t) {
  ...
  if (cmd === 'race') {
    if (!arg) {
      return `This thread: ${Number(t.race) || 0 ? `racing ${t.race} models per turn` : 'not racing'}\n`
        + 'Set with  /race <0-4>   — N models are drawn AT RANDOM from the current tier and asked at once;\n'
        + 'the first one to return a real answer wins and the rest are discarded.\n'
        + 'You pay for every entrant, so 4 costs about 4x a normal turn.';
    }
    const n = Math.max(0, Math.min(4, Math.round(Number(arg))));
    if (!Number.isFinite(n)) return `"${arg}" is not a number. Use /race 0 to turn it off, or /race 2-4.`;
    t.race = n; saveThreads();
    if (n < 2) { delete t.race; saveThreads(); return 'Racing off — one model per turn.'; }
    const pool = await tierModels(t.tier || 'medium', 99);
    return `Racing ${n} models per turn, drawn at random from the ${t.tier || 'medium'} tier (${pool.length} in the pool).\n`
...
async function runTurn(threadId, userText, onEvent, images) {
  ...
  const ask = async (attempt = 0) => {
    ...
    if (!t.model && race >= 2) {
      ...
      return (await brainRace(callMsgs, emit, t.contextId, models)).trim();
    }
...
function threadSummary(t) {
  ...
  return { id: t.id, name: t.name, color: t.color, parent: t.parent, status: t.status,
    ...
    tier: t.tier || 'medium', race: Number(t.race) || 0, model: t.model || '' };
}
...
const server = http.createServer((req, res) => {
  ...
  // Restart grokui in place. box-boot.sh supervises it, so exiting IS the
  // restart — that supervisor exists precisely so a dead service comes back
  // without taking the box with it. Used by the UI's reload button after an
  // openzoo update, so a user never has to redeploy a box to pick up a fix.
