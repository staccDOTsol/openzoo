...
const AUTO_RACE_RETRIES = Number(process.env.OZ_AUTO_RACE_RETRIES || 3);
...
function isDoneReply(text) {
  return DONE_LINE.test(String(text || ''));
}
function isRaceFailReply(text) {
  const s = String(text || '').trim();
  return s === RACE_EVERY_FAILED || /^\(race:\s*every model failed/i.test(s);
}
function isPaymentFailed(text) {
  return /\b(?:payment failed|HTTP 402|wallet is empty|empty wallet)\b/i.test(String(text || ''));
}
function isEmptyRetryNote(text) {
  return /returned nothing \d+ times|each returned nothing/i.test(String(text || ''));
}
// A question AUTO cannot answer for the user — a credential, which option
// they want, a fact only they have. STALLED_OFFER ("Should I proceed?") is
// not this; NUDGE already covers those.
function isBlockingQuestion(text) {
  const s = String(text || '');
  if (!/\?/.test(s)) return false;
  if (STALLED_OFFER.test(s)) return false;
  return /\b(?:cannot proceed|can(?:no|')t (?:proceed|continue|know)|need (?:your|the user)|only you|destructive|irreversible|please (?:provide|paste|confirm)|which (?:one|option|path|file|url|key|address|wallet|mint|rpc)\b|what (?:is|are) (?:the|your) (?:key|path|url|address|mint|wallet|rpc|secret))\b/i.test(s);
}
function shouldParkAuto(reply) {
  return isDoneReply(reply) || isPaymentFailed(reply) || isBlockingQuestion(reply);
}
// Solo-path gate before bindThread / finally. Park ONLY: ask mode, pendingRun,
// DONE:, 402/empty-wallet, a real missing-fact question, or the hard step cap.
// Always keep going on a directive/tool hop, RACE_EVERY_FAILED, `error:`,
// empty-retry notes, and t.lastRaceFail. STALLED_OFFER is not needs-input.
function shouldKeepAuto(t, reply, extra = {}) {
  if (!t || t.runMode !== 'auto') return false;
  if (t.pendingRun) return false;
  if ((t.autoSteps || 0) >= AUTO_MAX_STEPS) return false;
  const text = String(reply || '');
  if (isDoneReply(text)) return false;
  if (isPaymentFailed(text)) return false;
  if (isBlockingQuestion(text)) return false;
  if (extra.hadToolResult || extra.hadDirective) return true;
  if (isRaceFailReply(text) || t.lastRaceFail) return true;
  if (/^error:/i.test(text.trim())) return true;
  if (isEmptyRetryNote(text)) return true;
  return true;
}
function queueAutoHop(t, threadId, userText, onEvent) {
  bindThread(t).catch(() => {});
  t.autoSteps = (t.autoSteps || 0) + 1;
  if (t.autoSteps >= AUTO_MAX_STEPS) return false;
  runTurn(threadId, userText, onEvent).catch(() => {});
  return true;
}
...
setInterval(() => {
  ...
  for (const t of threads.values()) {
    ...
    if (t.runMode === 'auto' && !t.pendingRun && !isDoneReply(lastText) && !isPaymentFailed(lastText)) {
      kickTurn(t.id, AUTO_CONTINUE).catch(() => {});
    } else {
      t.status = 'idle';
      t.liveStatus = '';
      unlockWorktree(t);
...
async function runTurn(threadId, userText, onEvent, images) {
  ...
  if (t.members) {
    ...
    for (const m of t.members) {
      ...
      memberReply = r;
      if (ack !== null && ack !== undefined && !isDoneReply(r)) memberHadTool = true;
    }
    bindThread(t).catch(() => {});
    if (shouldKeepAuto(t, memberReply, { hadToolResult: memberHadTool })) {
      chained = queueAutoHop(t, threadId, AUTO_CONTINUE, onEvent);
    }
    return;
  }
  ...
  try {
    ...
    // A race that fails entirely is the same class of transient as empty:
    // retry in place. Parking on `(race: every model failed)` made AUTO a
    // one-shot — the parent sat idle and the user had to type continue.
    for (let i = 0; isRaceFailReply(reply) && i < AUTO_RACE_RETRIES; i++) {
      paint({ type: 'status', name: t.name, color: t.color, detail: 'retrying race…' });
      await new Promise((r) => setTimeout(r, 400 * (i + 1)));
      if (!stillMine()) return;
      reply = await ask(t.model ? 0 : i + 1);
    }
  ...
  // Empty-retry canned notes, race-wide failure, and `error:` must not park
  // AUTO. Post the note, then AUTO_CONTINUE — same hop the finally would
  // have killed if we fell through to idle.
  if (t.runMode === 'auto'
      && (isEmptyRetryNote(reply) || isRaceFailReply(reply) || /^error:/i.test(String(reply).trim()))) {
    t.messages.push({ role: 'assistant', content: reply });
    t.history.push({ who: 'bot', text: reply });
    paint({ type: 'final', name: t.name, color: t.color, text: reply });
  ...
  if (runCmd) {
    ...
    if (t.runMode === 'auto') {
      ...
      chained = queueAutoHop(t, threadId, condense('(command output)', output), onEvent);
      return;
    }
    {
      const runId = randomUUID();
  ...
  if (t.runMode === 'auto' && ack !== null && ack !== undefined
      && !isDoneReply(reply)) {
    chained = queueAutoHop(t, threadId, condense('(directive result)', ack), onEvent);
    return;
  }
  ...
  if (t.runMode === 'auto' && (ack === null || ack === undefined)
    ...
    t.autoNudged = true;
    chained = queueAutoHop(t, threadId, NUDGE, onEvent);
    return;
  }

  // AUTO DEFAULT: keep going. "auto is also conversation" parked a kid
  // after one ls; that design is the hole. shouldKeepAuto is the gate.
  if (shouldKeepAuto(t, reply)) {
    chained = true;
    t.autoSteps = (t.autoSteps || 0) + 1;
    bindThread(t).catch(() => {});
...
export {
  ...
  AUTO_CONTINUE, pingWakeText, pingCanWake, shouldKeepAuto, shouldParkAuto,
  isBlockingQuestion, isRaceFailReply, childKickoff, findByName, attachChildDir,
  finishChildDir,
