...
<script>
  ...
  async function submit() {
    ...
    // String compare. A sitrep word-boundary regex inside this template
    // literal is eaten by the backtick parser (backslash-b becomes a
    // backspace; backslash-slash closes the regex at caret-slash) and the
    // whole client script dies — empty sidebar, send is a no-op.
    const sitrepHead = task.toLowerCase();
    if (sitrepHead === '/sitrep' || sitrepHead.startsWith('/sitrep ')) {
      inp.value = '';
