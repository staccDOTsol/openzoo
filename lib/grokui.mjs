...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  body.agent-mode #agentTerm { display: flex; flex-direction: column; overflow: hidden; }
  body.agent-mode #log { display: none; }
  /* Overlay Message on the last TUI rows so OCC's own > sits under the composer. */
  body.agent-mode #bar { position: absolute; left: 0; right: 0; bottom: 0; z-index: 6;
                         background: #000; }
  ...
  function connectStream(id) {
    ...
    es.onmessage = (e) => {
      ...
      if (ev.type === 'pty') {
        ensureAgentTerm();
        if (!agentTerm) return;
        if (ev.reset) { try { agentTerm.reset(); } catch (err) {} lastPtySize = ''; }
        if (ev.data) {
