...
/** Bearer from the Authorization header only. Never query strings. */
export function bearerFromAuthorization(headers = {}) {
...
/**
 * Ask zoo.openzoo.fun whether this subscription key is live.
 * Never logs the key. Missing / invalid / expired → { ok: false, status: 401 }.
 */
export async function verifySubscriptionKey(key, { fetchImpl = fetch, origin = BILLING_ORIGIN } = {}) {
  ...
  let body = {};
  try {
    const r = await fetchImpl(`${origin}/api/billing/verify`, {
      method: 'POST',
      headers: {
  ...
  const expired = body?.expired === true || /expir/i.test(err);
  if (http === 401 || http === 403 || expired || body?.ok === false || body?.active === false) {
    return { ok: false, status: 401, error: expired ? 'expired' : (err || 'unauthorized') };
  }
  if (http >= 500) return { ok: false, status: 503, error: 'billing unavailable' };
  if (http < 200 || http >= 300) return { ok: false, status: 401, error: err || 'unauthorized' };
  return {
    ok: true,
    status: 200,
    tier: body.tier || null,
    tierName: body.tierName || body.name || null,
  };
}
