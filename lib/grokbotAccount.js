...
/** Paymaster `from` field: 1–24 chars, letters/numbers/space/._'- */
export function grokroomFromHandle(name, fallback = 'anon') {
  const fb = String(fallback || 'anon').replace(/[^A-Za-z0-9 _.'-]/g, '').trim().slice(0, 24) || 'anon';
  const s = String(name || fb).replace(/[^A-Za-z0-9 _.'-]/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 24);
  return s || fb;
}
...
/** Extract a paymaster room addr from a share URL, `/join …`, or a bare pubkey. */
export function grokroomAddrFromShare(s) {
  const t = String(s || '').trim();
  if (!t) return '';
  const m = t.match(/openzoo\.fun\/r\/([1-9A-HJ-NP-Za-km-z]{32,44})/i)
    || t.match(/(?:^|\s)\/join\s+(\S+)/i);
  const cand = m ? m[1] : t;
  const addr = String(cand).replace(/^https?:\/\/[^/]+\/r\//i, '').replace(/[/?#].*$/, '').trim();
  return grokroomShareUrl(addr) ? addr : '';
}

/** Bind an existing public room onto a local group (join). Does not mint a new key. */
export function bindOnchainRoom(agent, addr, home = os.homedir()) {
  if (!agent || typeof agent !== 'object') return agent;
  const web = grokroomShareUrl(addr);
  if (!web) return agent;
  const rec = {
    agentId: String(agent.id || ''),
    name: String(agent.name || 'group'),
    addr,
    web,
    createdAt: Date.now(),
  };
  const all = readOnchainRooms(home).filter((r) => r && r.agentId !== rec.agentId && r.addr !== addr);
  all.push(rec);
  writeOnchainRooms(all, home);
  return {
    ...agent,
    isGroup: true,
    room: { id: String(agent.room?.id || addr), addr, web },
  };
}

/** Share URL already on an overlay group (room.web or derived from addr). */
export function agentShareUrl(a) {
  if (!a || typeof a !== 'object') return '';
  return grokroomShareUrl(a.room?.addr || a.room?.address) || String(a.room?.web || '').trim();
}

/**
 * Overlay share payload for a focused tray row + grokroom /state rooms.
 * Grokroom groups have their own /r/<addr>. Local createGroup has no pubkey;
 * the HUD then lists the public grokroom URLs.
 */
export function groupShareState({ agent, rooms } = {}) {
  const url = agentShareUrl(agent);
  const list = (Array.isArray(rooms) ? rooms : []).map((r) => {
    const u = r && (r.web || grokroomShareUrl(r.addr || r.address));
    if (!u) return null;
    return { id: String(r.id || r.name || ''), name: String(r.name || r.id || ''), url: u };
  }).filter(Boolean);
  const isGroup = !!(agent && (agent.isGroup || agent.room));
  return { url: url || '', isGroup, rooms: list };
}
...
export function onchainRoomsPath(home = os.homedir()) {
  return path.join(home, '.openzoo', 'grokbot-onchain-rooms.json');
}

export function readOnchainRooms(home = os.homedir()) {
  try {
    const j = JSON.parse(fs.readFileSync(onchainRoomsPath(home), 'utf8'));
    return Array.isArray(j) ? j : [];
  } catch { return []; }
}

export function writeOnchainRooms(list, home = os.homedir()) {
  const p = onchainRoomsPath(home);
  fs.mkdirSync(path.dirname(p), { recursive: true });
  fs.writeFileSync(p, JSON.stringify(Array.isArray(list) ? list : [], null, 2));
}

/**
 * Mint a Solana ed25519 pubkey for a Grok Bot groupchat. The paymaster
 * indexes memos by that account; https://openzoo.fun/r/<addr> is the public page.
 */
export function mintOnchainRoom({ agentId, name, home = os.homedir() } = {}) {
  const pair = generateKeyPairSync('ed25519');
  const rawPub = pair.publicKey.export({ format: 'der', type: 'spki' }).subarray(-32);
  const addr = grokroomB58(Buffer.from(rawPub));
  const rec = {
    agentId: String(agentId || ''),
    name: String(name || 'group'),
    addr,
    web: grokroomShareUrl(addr),
    createdAt: Date.now(),
  };
  const all = readOnchainRooms(home).filter((r) => r && r.agentId !== rec.agentId);
  all.push(rec);
  writeOnchainRooms(all, home);
  return rec;
}

/** Reuse a stored room for this agent, or mint one. */
export function ensureOnchainRoom(agent, home = os.homedir()) {
  const addr = agent?.room?.addr || agent?.room?.address || '';
  const web = grokroomShareUrl(addr);
  if (web) return { agentId: String(agent.id || ''), name: String(agent.name || ''), addr, web };
  const hit = readOnchainRooms(home).find((r) => r && r.agentId === agent?.id && grokroomShareUrl(r.addr));
  if (hit) return { ...hit, web: grokroomShareUrl(hit.addr) };
  return mintOnchainRoom({ agentId: agent?.id, name: agent?.name, home });
}

export function attachOnchainRoom(agent, home = os.homedir()) {
  if (!agent || typeof agent !== 'object') return agent;
  if (!agent.isGroup && !(agent.room && agent.room.id)) return agent;
  if (agent.hidden) return agent;
  const rec = ensureOnchainRoom(agent, home);
  const id = String(agent.room?.id || rec.agentId || agent.id || '');
  return {
    ...agent,
    room: { id, addr: rec.addr, web: rec.web },
  };
}
...
export function shapeAgent(raw = {}) {
  ...
  if (a.room && typeof a.room === 'object' && a.room.id) {
    const addr = String(a.room.addr || a.room.address || '');
    const web = grokroomShareUrl(addr) || String(a.room.web || '').trim();
    room = { id: String(a.room.id), addr, ...(web ? { web } : {}) };
