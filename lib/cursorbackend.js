...
export function localExecFrame(command, cwd, timeout = 120) {
  return {
    kind: 'exec',
    ...
    serverMessage: {
      shellStreamArgs: { command: String(command), workingDirectory: cwd, timeout },
    },
  };
}
...
async function execLocal(command, cwd, log) {
  ...
  if (localExecSse.size > 0) {
    ...
    const got = await localExecAsk(localExecFrame(command, dir, 55), 60000);
...
async function runLocalTool(name, args, log, ctx = {}) {
  try {
    ...
    if (name === 'list_dir') {
      ...
      try {
        if (localExecSse.size > 0) {
          const got = await localExecAsk(localExecFrame(`ls -la ${JSON.stringify(abs)}`, os.homedir(), 40));
          return toolResultText(got.stdout || got.message || got) || '(empty)';
        }
