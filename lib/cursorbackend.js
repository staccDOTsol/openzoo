...
export function normalizeExecFrame(frame) {
  const out = { stdout: '', stderr: '', exitCode: undefined, message: '' };
  const seen = new Set();
  ...
  const visit = (node, depth) => {
    ...
    for (const k of ['message', 'clientMessage', 'serverMessage', 'result', 'data', 'payload', 'body']) {
      visit(node[k], depth + 1);
    }
  };
  visit(frame, 0);
  if (!out.stdout && !out.stderr && !out.message) out.message = toolResultText(frame);
  return out;
}
...
function handleLocalExecFrames(frames, log) {
  for (const f of frames || []) {
    ...
    if (f.kind === 'file') {
      ...
      w.resolve({
        kind: f.kind,
        message: norm.message || norm.stdout || toolResultText(f),
        stdout: norm.stdout,
        stderr: norm.stderr,
