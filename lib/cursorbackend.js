...
async function grokroomPost(roomId, body) {
  const id = grokroomIdentity();
  const ts = Date.now();
  const from = GROKROOM_HANDLE;
  const pub = id.pub;
  ...
  if (/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(room)) {
    endpoints.push('https://openzoo.fun/api/room/submit');
  }
...
async function grokroomSync(log = () => {}) {
  ...
  try {
    ...
    for (const r of rooms) {
      ...
      if (!roomId) continue;
      let group = grokroomEnsureGroup(roomId, r.name, r.addr || r.address);
      if (!group) continue;
      ...
      for (const from of names) group = grokroomMemberEnsure(group, from);
      const t = agentTranscript(group.id);
      ...
      for (const m of msgs) {
        ...
        if (!body) continue;
        if (from === GROKROOM_HANDLE && (t.entries || []).slice(-50).some((h) => h.role === 'user' && String(h.content || '') === body)) continue;
        group = grokroomMemberEnsure(group, from);
        const mid = grokroomMemberId(from);
        const line = fanoutLine(group.id, 'assistant', body, {
          author: { kind: 'agent', id: mid, name: from },
          sig: msig,
        });
        ssePush('transcript', { ...gatewayEntry(line), agentId: group.id });
        if (m.ts && m.ts > (agentActivity.get(group.id)?.updatedAt || 0)) {
          /* keep */
        }
        dirty += 1;
      }
      if (dirty) {
        bumpAgent(group.id, { preview: msgs[msgs.length - 1]?.body || '', notify: true, select: false });
        log(`cursor-backend:      grokroom ${roomId} +${dirty} bubbles history=${agentTranscript(group.id).entries.length}`);
      }
    }
  } catch (e) {
