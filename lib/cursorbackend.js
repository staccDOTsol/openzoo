 ...
 *  whose only shell case is `shellArgs`. We used to send `{command, cwd}` flat,
 *  so the daemon read an unset oneof and threw back
 *  `No handler found for server message of type undefined` — every local exec
 *  failed while the canvas showed only the raw control frame. Shape lives in
 *  lib/podagent.mjs, reverse-engineered from local-exec-daemon/main.cjs. */
export function localExecFrame(command, cwd, timeout = 120) {
  return {
    kind: 'exec',
    requestId: randomUUID(),
    approvalId: randomUUID(),
    serverMessage: { shellArgs: { command: String(command), workingDirectory: cwd, timeout } },
  };
}
