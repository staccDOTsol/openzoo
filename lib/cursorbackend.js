...
import {
  ...
  attachOnchainRoom, isGrokRoomAgent, isGrokRoomMember,
...
async function handleHijackedPodHttp(req, res, full, body, log) {
  ...
  if (name === 'sendPrompt') {
    ...
    {
      const live = (cachedAgentList() || []).find((a) => a.id === agentId);
      if (live?.room?.addr && !/^room-/.test(agentId)) grokroomSend(live, prompt, log);
    }
...
function grokroomMemberEnsure(group, name) {
