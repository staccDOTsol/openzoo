...
import { randomUUID, generateKeyPairSync, createPrivateKey, sign as cryptoSign } from 'node:crypto';
import { encodeAvailableModels, encodeForMethod, encodeEnsureSandBox, encodeGetGrokBotSendStatus, decodeProtoFields, unwrapConnect, fakeCursorAuthReply } from './cursorapi.js';
import {
  ...
  agentShareUrl, groupShareState, attachOnchainRoom, bindOnchainRoom,
...
import { zooModelIds, zooModelRow, mediaKindOf, resolveModel } from './models.js';
...
const TLS_DNS = [...CURSOR_HOSTS, ...ANTHROPIC_TLS_HOSTS, 'localhost'];
const TLS_IPS = ['127.0.0.1'];
const TLS_HOSTS = [...TLS_DNS, ...TLS_IPS];

/** Node-only self-signed PEM. Windows has no openssl in PATH (measured 2026-09-03).
 *  selfsigned@5 `generate` is async. */
export async function mintTlsPems() {
  const selfsigned = createRequire(import.meta.url)('selfsigned');
  const pems = await selfsigned.generate(
    [{ name: 'commonName', value: 'api2.cursor.sh' }],
    {
      keySize: 2048,
      days: 3650,
      algorithm: 'sha256',
      extensions: [{
        name: 'subjectAltName',
        altNames: [
          ...TLS_DNS.map((value) => ({ type: 2, value })),
          ...TLS_IPS.map((ip) => ({ type: 7, ip })),
        ],
      }],
    },
  );
  if (!pems?.cert || !pems?.private) throw new Error('selfsigned returned empty pem');
  return { cert: pems.cert, key: pems.private };
}

/** Generate (once) a self-signed cert covering every host we impersonate.
 *  Mint with the `selfsigned` package (no openssl). openssl is a fallback only. */
export async function ensureCert(log = () => {}) {
  ...
  try {
    ...
    fs.writeFileSync(key, pems.key);
    fs.writeFileSync(cert, pems.cert);
  } catch (e) {
    const san = `subjectAltName=${TLS_DNS.map((h) => `DNS:${h}`).join(',')},${TLS_IPS.map((h) => `IP:${h}`).join(',')}`;
    execFileSync('openssl', [
      'req', '-x509', '-newkey', 'rsa:2048', '-nodes',
      '-keyout', key, '-out', cert, '-days', '3650',
      '-subj', '/CN=api2.cursor.sh', '-addext', san,
    ], { stdio: 'ignore' });
    log(`cursor-tls: openssl fallback (${e.message})`);
  }
...
function bumpAgent(id, { preview = '', notify = true, select = true } = {}) {
  const now = Date.now();
...
/** Helper builds disagree about where the shell output lives: top-level
 *  `stdout` on some, `message: {stdout, exitCode}` or `clientMessage` on
 *  others. Dig it out as TEXT — an object that reaches the canvas paints as
 *  [object Object] (measured: `→ exec "ls" [object Object]` on Linux). */
export function normalizeExecFrame(frame) {
  const out = { stdout: '', stderr: '', exitCode: undefined, message: '', error: '' };
  const seen = new Set();
  ...
  const visit = (node, depth) => {
    ...
    seen.add(node);
    if (!out.stdout) out.stdout = pick(node, ['stdout', 'stdOut', 'output']);
    if (!out.stderr) out.stderr = pick(node, ['stderr', 'stdErr']);
    if (!out.message) out.message = pick(node, ['message', 'text', 'content']);
    if (!out.error) out.error = pick(node, ['error', 'errorMessage']);
    if (out.exitCode == null) {
      for (const k of ['exitCode', 'exit_code', 'code']) {
        if (typeof node[k] === 'number') { out.exitCode = node[k]; break; }
      }
    }
    for (const k of ['message', 'clientMessage', 'serverMessage', 'result', 'data', 'payload', 'body', 'throw']) {
      visit(node[k], depth + 1);
    }
  };
  visit(frame, 0);
  if (out.error) out.error = `local-exec: ${out.error}`;
  if (!out.stdout && !out.stderr && !out.message && !out.error) out.message = toolResultText(frame);
  return out;
}

/** The daemon streams `output`/`stdout` chunks and only then closes with
 *  `result`/`exit`, and those streaming frames do not always echo requestId.
 *  Keying purely on requestId dropped the body on the floor and waited out the
 *  45s timeout, so fall back to the sole in-flight request. */
function localExecWaiterFor(f) {
  if (f.requestId && localExecWaiters.has(f.requestId)) {
    return { id: f.requestId, w: localExecWaiters.get(f.requestId) };
  }
  if (!f.requestId && localExecWaiters.size === 1) {
    const [id, w] = localExecWaiters.entries().next().value;
    return { id, w };
  }
  return null;
}
...
function handleLocalExecFrames(frames, log) {
  for (const f of frames || []) {
    ...
    const hit = localExecWaiterFor(f);
    if (!hit) continue;
    const { id, w } = hit;
    if (f.kind === 'file') {
    ...
    } else if (f.kind === 'control' && f.message && f.message.heartbeat) {
      // 3s keepalive while a long command runs. It is a `control` frame like
      ...
      w.resolve({
        kind: f.kind,
        message: norm.error || norm.message || stdout || toolResultText(f),
        stdout,
        stderr: norm.error && !stderr ? norm.error : stderr,
...
function appendLine(agentId, role, text, extra = {}) {
  ...
  if (role === 'user') {
    ...
    e = {
      ...
      ...(extra.sig ? { sig: String(extra.sig) } : {}),
    };
  }
  t.entries.push(e);
  saveTranscripts();
...
export function formatZooToolLine({ name, args = {}, result } = {}) {
  ...
  const head = detail
    ...
    : String(name || 'tool');
  let tail = toolResultText(result).replace(/\s+/g, ' ').trim();
  if (tail.startsWith('{')) {
    try {
...
function seedBriefOnCanvas(agent, log) {
  if (!agent?.id || !String(agent.brief || '').trim()) return false;
  if (isGrokRoomAgent(agent) || isGrokRoomMember(agent)) return false;
...
async function seedPayNudge(list, activeId, log = () => {}) {
  ...
  if (isGrokRoomAgent({ id: activeId }) || /^roombot-/.test(String(activeId))) return false;
  payNudged = true;
  try {
...
function groupMemberRecords(agentId) {
  ...
  }).filter((m) => !/^roombot-/.test(m.id) && !isGrokRoomMember({ id: m.id }));
}

export function groupNormText(text) {
  return stripSpendFooter(String(text || ''))
    .replace(/^\s*[A-Za-z0-9 _.-]{1,40}:\s*/, '')
    .toLowerCase()
    .replace(/\s+/g, ' ')
    .trim();
}

export function groupReplyIsRepeat(text, recent = []) {
  const n = groupNormText(text);
  if (n.length < 40) return false;
  const head = n.slice(0, 90);
  return (Array.isArray(recent) ? recent : []).some((prev) => {
    const p = groupNormText(prev);
    if (p.length < 40) return false;
    return p.slice(0, 90) === head || p.startsWith(head) || n.startsWith(p.slice(0, 90));
  });
}
...
export function groupReplyIsPass(text) {
  ...
  if (/\binfinite\b.{0,48}\b(recursion|loop|chatter)\b/i.test(s)) return true;
  if (/\bbots? keep triggering\b/i.test(s)) return true;
  if (/\beach other'?s responses\b/i.test(s)) return true;
  if (/\bfeedback loop\b/i.test(s)) return true;
...
const GROUP_MAX_ROUNDS = Math.min(12, Math.max(1, Number(process.env.OZ_GROUP_MAX_ROUNDS || 2)));
const GROUP_MAX_CALLS = Math.min(24, Math.max(2, Number(process.env.OZ_GROUP_MAX_CALLS || 6)));

/** Members speak once, then at most a short peek. Repeats and loop-talk are PASS. */
async function runGroupQueue({ agentId, humanPrompt, parsed, nonce, log }) {
  ...
  const oneTurn = async (m, phase) => {
    ...
    const persona = phase === 'peek'
      ? `You are ${m.name} in a group with ${others}. If you have nothing new, or would repeat another member, reply with exactly PASS. Do not discuss loops, recursion, or that bots are talking to each other. One short in-character line or PASS.`
      : `You are ${m.name} in a group with ${others}. Answer the human as yourself in a few sentences. Do not speak as the human. Do not prefix with visitor shortnames. Do not discuss infinite loops or recursion unless the human asked about that.\n\nThe human said: ${humanPrompt}`;
    let bit = '';
    try {
    ...
    calls += 1;
    const pass = groupReplyIsPass(bit) || groupReplyIsRepeat(bit, painted);
    if (!pass) {
      ...
      painted.push(bit);
      {
        const live = (cachedAgentList() || []).find((a) => a.id === agentId);
        if (live?.room?.addr) grokroomSend(live, bit, log, { from: m.name });
      }
  ...
  if (painted.length >= 2 && groupReplyIsRepeat(painted[painted.length - 1], painted.slice(0, -1))) {
    log(`cursor-backend:      group stopped after speak (repeat) calls=${calls}`);
    return true;
  }
  ...
  while (round < GROUP_MAX_ROUNDS && calls < GROUP_MAX_CALLS) {
    ...
    if (passes >= named.length || painted.length === before) {
      log(`cursor-backend:      group concluded round=${round} calls=${calls}`);
      break;
    }
    if (painted.length >= 2 && groupReplyIsRepeat(painted[painted.length - 1], painted.slice(0, -1))) {
      log(`cursor-backend:      group stopped peek (repeat) round=${round}`);
      break;
    }
  }
...
function gatewayEntry(e) {
  const { seq, pulledRemote, promptRaw, ephemeral, sig, ...rest } = e;
  return rest;
}
...
async function resolveModelId(raw) {
  ...
  try {
    ...
    const mapped = resolveModel(s, ids);
    if (mapped) return mapped;
  } catch { /* catalog miss: keep explicit slash ids */ }
...
/** `serverMessage` is `agent.v1.ExecServerMessage` as protobuf-es JSON — a oneof,
 *  so the key IS the case name. Two wrong shapes before this one:
 *    `{command, cwd}` flat  -> "No handler found for server message of type undefined"
 *    `{shellArgs: {...}}`   -> "No handler found for server message of type shellArgs"
 *  `shellArgs` is a real case on the wire (field 2), but `buildLocalExecManager`
 *  in Grok Bot 0.36.0's dist/local-exec-daemon/main.cjs registers exactly FOUR
 *  handlers: shellStreamArgs, backgroundShellSpawnArgs, readArgs, lsArgs. The
 *  one-shot shell case is simply not wired into the local-exec lane — the
 *  streaming one is, and both carry the same `agent.v1.ShellArgs` payload.
 *  `authorizedBy*` are read as `=== true` by the daemon's approval gate; without
 *  them an unattended exec has nothing to approve it.
 *
 *  `timeout` is MILLISECONDS. The daemon reads it through a function named
 *  `resolveShellTimeoutMs` and uses the value verbatim, so the plausible-looking
 *  `timeout: 120` (seconds, as lib/podagent.mjs had it) aborts the shell after
 *  120 ms: `echo hi; pwd` came back exit 0 with empty stdout and `ls -lha ~`
 *  died mid-stream with "Cannot read properties of undefined (reading '3')".
 *  0 means "use the daemon's default", not "no timeout". */
export function localExecFrame(command, cwd, timeoutMs = 120_000) {
  return {
    kind: 'exec',
    requestId: randomUUID(),
    approvalId: randomUUID(),
    authorizedByStanding: true,
    authorizedByApproval: true,
    serverMessage: {
      shellStreamArgs: { command: String(command), workingDirectory: cwd, timeout: timeoutMs },
    },
  };
}
 ...
 *  Windows there is no POSIX shell at all, so every exec came back as a spawn
 *  error instead of output. */
export function execShell(command, platform = process.platform) {
  if (platform === 'win32') {
    return { file: process.env.ComSpec || 'cmd.exe', args: ['/d', '/s', '/c', command] };
  }
  if (platform === 'darwin') return { file: '/bin/zsh', args: ['-lc', command] };
  for (const sh of ['/bin/bash', '/usr/bin/bash', '/bin/zsh']) {
    try { if (fs.existsSync(sh)) return { file: sh, args: ['-lc', command] }; } catch { /* keep looking */ }
  }
...
async function execLocal(command, cwd, log) {
  ...
  try {
    ...
    // Node reports spawn <shell> ENOENT when cwd is missing — not a missing shell.
  ...
  const sh = execShell(command);
  log(`cursor-backend:      exec cwd=${dir} sh=${sh.file} ${JSON.stringify(String(command).slice(0, 80))}`);
  const execMs = Math.min(180000, Math.max(15000, Number(process.env.OPENZOO_EXEC_TIMEOUT_MS || 90000)));
  let stdout = '';
  let stderr = '';
  try {
    ({ stdout, stderr } = await execFileAsync(sh.file, sh.args, {
      cwd: dir,
      timeout: execMs,
    ...
    // A nonzero exit (grep with no match, a failing build) still has output the
    // model needs — bubbling the Error loses it and paints only "ERROR".
    stdout = toolResultText(e.stdout);
    stderr = toolResultText(e.stderr) || e.message;
    if (!stdout && !stderr) throw e;
  }
  return `${toolResultText(stdout)}${stderr ? `\nstderr:\n${toolResultText(stderr)}` : ''}`.trim() || '(no output)';
}
  ...
  try {
    ...
    if (name === 'list_dir') {
      ...
      try {
        ...
        return `list_dir failed ${abs}: ${e.message}`;
      }
    }
    if (name === 'exec') {
  ...
  const messages = [
    {
      ...
      content: chatOnly
        ...
        : [
          ...
          'Do not write spend totals, solscan links, memos, or ::oz-spend:: into the reply. The host paints a HUD pill.',
          'The operator wallet pays every zoo call via x402 (KISS/USDC on Solana; SOL gas is sponsored). If the spend HUD is visible or a prior turn paid, the call WAS paid. Do not say you have no money, cannot afford tools, or need the human to fund you. A 402/underfunded tool error is a host retry — continue the job.',
          'Prior turns of THIS Grok Bot chat are in the messages below. Do not claim the thread starts blank or that earlier questions did not arrive.',
        ].join(' '),
    },
  ];
    ...
    if (calls.length) {
      ...
      for (const c of calls) {
        ...
        usedTools.push({
          ...
          note: toolResultText(result).slice(0, 200),
        });
        ...
        messages.push({
          role: 'tool',
          tool_call_id: c.id,
          content: toolResultText(result).slice(0, 120000),
        });
      }
...
async function handleHijackedPodHttp(req, res, full, body, log) {
  ...
  if (name === 'ozShare') {
    const list = cachedAgentList() || [];
    const id = String(focusedAgentId || '');
    const agent = list.find((a) => a.id === id) || null;
    const share = groupShareState({ agent, rooms: grokroomSync._rooms });
    jsonSend(res, { ok: true, agentId: id, ...share });
    return true;
  }
  if (name === 'ozRevive') {
    let parsed = {};
    ...
    const wakes = readWakeups(HOME);
    const list = (cachedAgentList() || []).filter((a) => a?.id && !a.isGroup && !isGrokRoomAgent(a) && !isGrokRoomMember(a));
    const armed = [];
    const already = [];
    for (const a of list) {
  ...
  if (name === 'getHostStatus') {
    const host = {
      hostVersion: '0.36.0',
      latestHostVersion: '0.36.0',
      hostUpdateAvailable: false,
      isBusy: false,
      capabilities: [],
    };
    jsonSend(res, { status: 'ok', value: host, ...host });
    log('cursor-backend:      -> getHostStatus ready');
    return true;
  }
  if (name === 'resolveAgentCreation') {
    const route = { kind: 'box' };
    jsonSend(res, { status: 'ok', value: route, ...route });
    log('cursor-backend:      -> resolveAgentCreation box');
    return true;
  }
  if (name === 'getForeverBoxStatus' || name === 'ensureForeverBox') {
    let parsed = {};
    try { parsed = JSON.parse(String(body || '{}')); } catch { parsed = {}; }
    const box = {
      agentId: String(parsed.id || parsed.agentId || 'openzoo'),
      state: 'running',
    };
    jsonSend(res, { status: 'ok', value: box, ...box });
    log(`cursor-backend:      -> ${name} agentId=${box.agentId} state=${box.state}`);
    return true;
  }
  if (name === 'getBoxSecretsStatus') {
    const secrets = { keys: [], isApplied: false, lastAppliedAtMs: null };
    jsonSend(res, { status: 'ok', value: secrets, ...secrets });
    return true;
  }
  if (name === 'isEgressTunnelAvailable') {
    jsonSend(res, { status: 'ok', value: false });
    return true;
  }
  if (name === 'getAgentChannels') {
    const channels = { manifests: [], connections: [] };
    jsonSend(res, { status: 'ok', value: channels, ...channels });
    return true;
  }
  ...
  if (name === 'createAgent' || name === 'createAgentFromTemplate' || name === 'duplicateAgent') {
    ...
    const created = { agent, id: agent.id, ...agent };
    jsonSend(res, { status: 'ok', value: created, ...created });
  ...
  if (name === 'kickstartAgent') {
    ...
    const kick = { id, isIntroductionInFlight: false };
    jsonSend(res, { status: 'ok', value: kick, ...kick });
  ...
  if (name === 'createGroup') {
    ...
    const raw = mintLocalAgent({
      ...parsed,
      isGroup: true,
      memberIds,
      name: parsed.name || parsed.title || 'group',
    });
    const agent = mintLocalAgent(attachOnchainRoom(raw, HOME));
    const created = { agent, id: agent.id, ...agent };
    jsonSend(res, { status: 'ok', value: created, ...created });
    pushCreatedAgent(agent);
    log(`cursor-backend:      createGroup onchain id=${agent.id} share=${agent.room?.web || ''} members=${memberIds.length} name=${JSON.stringify(agent.name)}`);
    void paintGroupShareHint(agent, log);
    return true;
  }
  ...
  if (name === 'deleteAgents') {
    ...
    addDeletedIds(HOME, [...ids].filter((id) => !/^(room-|roombot-)/.test(id)));
    saveAgents(next);
    for (const id of ids) {
  ...
  if (name === 'listAgents' || name === 'getTrays' || name === 'countAgents' || name === 'searchAgents') {
    ...
    if (name === 'listAgents' || name === 'countAgents' || name === 'searchAgents') {
      let raw = mergeAgentLists([]);
      let minted = false;
      raw = raw.map((a) => {
        if (!a?.isGroup || a.hidden || /^room-/.test(String(a.id || '')) || agentShareUrl(a)) return a;
        minted = true;
        return shapeAgent(attachOnchainRoom(a, HOME));
      });
      if (minted) saveAgents(raw.map(shapeAgent));
      const list = rosterForEvent(raw, agentActivity);
      if (name === 'countAgents') {
  ...
  if (name === 'sendPrompt') {
    ...
    const roomAgent = grokroomLookup(agentId);
    if (roomAgent && /^room-/.test(agentId)) {
      ...
      noteFocus(agentId);
      bumpAgent(agentId, { preview: prompt, notify: false, select: false });
      ssePush('transcript', { ...gatewayEntry(userLine), agentId });
      grokroomSend(roomAgent, prompt, log);
      return true;
    }
    ...
    {
      const live = (cachedAgentList() || []).find((a) => a.id === agentId);
      if (live?.room?.addr) grokroomSend(live, prompt, log);
    }
    const joinAddr = /^\s*\/join\b/i.test(prompt) ? grokroomAddrFromShare(prompt)
      : (/^\s*https?:\/\/openzoo\.fun\/r\//i.test(prompt) ? grokroomAddrFromShare(prompt) : '');
    ...
    setImmediate(async () => {
      ...
      try {
        if (modelCmd) {
          ...
          grouped = true;
          const joined = joinSharedGroup(joinAddr);
          if (!joined) {
            text = `could not join — not an openzoo.fun/r/<addr> share`;
          } else {
            noteFocus(joined.id);
            ssePush('agent-upserted', { agent: joined, activeAgentId: joined.id });
            ssePush('agents', { agents: rosterForEvent(cachedAgentList() || [], agentActivity), activeAgentId: joined.id });
            void paintGroupShareHint(joined, log);
            text = `joined ${joined.name}\n${joined.room?.web || ''}\nIn your OpenZoo Bot: add your bots to this group. They speak here; remote nicks only show up as people, they are not local bots.\nAnyone else: /join ${joined.room?.web || joinAddr}`;
          }
        } else {
          const live = (cachedAgentList() || []).find((a) => a.id === agentId);
          const members = groupMemberRecords(agentId);
          const isSharedRoom = !!(live?.room?.addr || isGrokRoomAgent(live || { id: agentId }));
          if (members.length) {
            grouped = true;
      ...
      if (!grouped) {
        ...
        {
          const live = (cachedAgentList() || []).find((a) => a.id === agentId);
          if (live?.room?.addr && text) grokroomSend(live, text, log);
        }
  ...
  if (name === 'getAgentTranscriptTail' || name === 'getAgentTranscriptWindow' || name === 'openAgentTail') {
    ...
    if (isGrokRoomAgent({ id }) || /^roombot-/.test(id)) t.pulledRemote = true;
  ...
  const stubs = {
    getHostStatus: {
      hostVersion: '0.36.0', latestHostVersion: '0.36.0',
      hostUpdateAvailable: false, isBusy: false, capabilities: [],
    },
    ...
    getForeverBoxStatus: { agentId: 'openzoo', state: 'running' },
    ensureForeverBox: { agentId: 'openzoo', state: 'running' },
    getBoxSecretsStatus: { keys: [], isApplied: false, lastAppliedAtMs: null },
    getAgentChannels: { manifests: [], connections: [] },
    resolveAgentCreation: { kind: 'box' },
...
// ===== GROKROOM-BRIDGE v1 (overlay) =====
// Rooms from 127.0.0.1:4799 as native Grok Bot group threads.
// One room = one sidebar row (# main / # bots / # random / # lobby).
// Each remote sender is a hidden bubble-member (no sidebar row). Sends POST a
// signed ed25519 envelope to /submit — the chain is the log. Additive: any
// failure here must never break zoo chat. Interval starts from startCursorBackend
// so importing this module in tests does not poll.
const GROKROOM_API = process.env.OZ_GROKROOM_URL || 'http://127.0.0.1:4799';
const GROKROOM_PAYMASTER = (process.env.OZ_GROKROOM_PAYMASTER || 'https://openzoo.fun').replace(/\/$/, '');
const GROKROOM_ID_FILE = path.join(HOME, '.openzoo', 'grokroom-identity.json');
const GROKROOM_PKCS8_PREFIX = Buffer.from('302e020100300506032b657004220420', 'hex');
const GROKROOM_HANDLE = (process.env.OZ_ROOM_HANDLE || process.env.USER || 'anon')
  .replace(/[^A-Za-z0-9 _.'-]/g, '').trim().slice(0, 24) || 'anon';
const GROKROOM_B58 = '123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz';

function grokroomB58(buf) {
  const digits = [0];
  ...
  return out + digits.reverse().map((d) => GROKROOM_B58[d]).join('');
}

let grokroomIdCache = null;
function grokroomIdentity() {
  if (grokroomIdCache) return grokroomIdCache;
  let id = null;
  try {
    const j = JSON.parse(fs.readFileSync(GROKROOM_ID_FILE, 'utf8'));
    if (j.pub && j.seed) id = j;
  } catch { /* first run */ }
  if (!id) {
    const pair = generateKeyPairSync('ed25519');
    const rawPub = pair.publicKey.export({ format: 'der', type: 'spki' }).subarray(-32);
    const rawSeed = pair.privateKey.export({ format: 'der', type: 'pkcs8' }).subarray(-32);
    id = { pub: grokroomB58(Buffer.from(rawPub)), seed: Buffer.from(rawSeed).toString('base64') };
    try {
      fs.mkdirSync(path.dirname(GROKROOM_ID_FILE), { recursive: true });
      fs.writeFileSync(GROKROOM_ID_FILE, JSON.stringify(id, null, 2));
    } catch { /* in-memory is fine */ }
  }
  grokroomIdCache = id;
  return id;
}

async function grokroomPost(roomId, body, { from: fromRaw } = {}) {
  const id = grokroomIdentity();
  const ts = Date.now();
  const from = grokroomFromHandle(fromRaw || GROKROOM_HANDLE, GROKROOM_HANDLE);
  const pub = id.pub;
  const room = String(roomId || '');
  const canonical = JSON.stringify({ body, from, pub, room, ts });
  const key = createPrivateKey({ key: Buffer.concat([GROKROOM_PKCS8_PREFIX, Buffer.from(id.seed, 'base64')]), format: 'der', type: 'pkcs8' });
  const sig = grokroomB58(cryptoSign(null, Buffer.from(canonical, 'utf8'), key));
  const payload = JSON.stringify({ room, from, pub, body, ts, sig });
  const endpoints = [];
  if (/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(room)) {
    endpoints.push(GROKROOM_PAYMASTER + '/api/room/submit');
  }
  endpoints.push(GROKROOM_API + '/submit');
  let last = new Error('no submit endpoint');
  for (const url of endpoints) {
    try {
      ...
      last = new Error(j.error || ('submit HTTP ' + r.status));
    } catch (e) {
      last = e;
    }
  }
...
function grokroomLookup(agentId) {
  const list = cachedAgentList() || [];
  return list.find((a) => a.id === agentId && isGrokRoomAgent(a)) || null;
}

function grokroomUpsertAgent(parsed, { select = false } = {}) {
  const id = String(parsed.id);
  const prev = (cachedAgentList() || []).find((a) => a.id === id) || {};
  const agent = shapeAgent({
    ...prev,
  ...
  const list = cachedAgentList() || [];
  const i = list.findIndex((a) => a.id === id);
  if (i < 0) list.unshift(agent);
  else list[i] = { ...list[i], ...agent };
  saveAgents(list.map(shapeAgent));
  agentTranscript(id);
  if (agent.hidden) {
    const visible = rosterForEvent(list, agentActivity);
    ssePush('agents', { agents: visible, activeAgentId: focusedAgentId || visible[0]?.id || id });
  } else {
    pushCreatedAgent(agent, { select });
  }
  return agent;
...
function grokroomMemberEnsure(group, name, { speak = false } = {}) {
  const mid = grokroomMemberId(name);
  const list = cachedAgentList() || [];
  let member = list.find((a) => a.id === mid);
  if (!member) {
    member = grokroomUpsertAgent({
      id: mid,
      name: String(name || 'anon').slice(0, 80),
      hidden: true,
  ...
  if (!speak) return group;
  const have = group.memberIds || [];
  if (have.includes(mid)) return group;
  return grokroomUpsertAgent({
    id: group.id,
    name: group.name,
    isGroup: true,
    memberIds: [...have, mid],
...
function joinSharedGroup(addr, { name } = {}) {
  const web = grokroomShareUrl(addr);
  if (!web) return null;
  const list = cachedAgentList() || [];
  const existing = list.find((a) => a?.room?.addr === addr);
  if (existing) return existing;
  const raw = mintLocalAgent({
    isGroup: true,
    name: String(name || `group ${addr.slice(0, 6)}`).slice(0, 80),
    memberIds: [],
    room: { id: addr, addr, web },
  });
  return mintLocalAgent(bindOnchainRoom(raw, addr, HOME));
}

function paintShareIfMissing(agent, log = () => {}) {
  const url = agentShareUrl(agent);
  if (!agent?.id || !url) return false;
  const t = agentTranscript(agent.id);
  const has = (t.entries || []).some((e) => /openzoo\.fun\/r\//.test(String(e?.content || '')));
  if (has) return false;
  const nonce = `oz-share-${agent.id}`;
  const line = fanoutLine(agent.id, 'assistant', 'Share this room: ' + url, {
    clientNonce: nonce,
    requestId: nonce,
  });
  ssePush('transcript', { ...gatewayEntry(line), agentId: agent.id });
  log(`cursor-backend:      group share painted agent=${agent.id}`);
  return true;
}

async function paintGroupShareHint(agent, log = () => {}) {
  if (!agent?.id) return;
  let rooms = grokroomSync._rooms;
  if (!Array.isArray(rooms)) {
    try {
      const st = await fetch(GROKROOM_API + '/state', { signal: AbortSignal.timeout(4000) }).then((r) => r.json());
      rooms = Array.isArray(st.rooms) ? st.rooms : [];
      grokroomSync._rooms = rooms;
    } catch { rooms = []; }
  }
  const share = groupShareState({ agent, rooms });
  let text = '';
  if (share.url) {
    text = 'Share this room on openzoo.fun: ' + share.url;
  } else if (share.rooms.length) {
    text = 'This group is local to this house. Public grokrooms — send this URL:\n'
      + share.rooms.map((r) => `${r.name}  ${r.url}`).join('\n');
  } else {
    text = 'This group is local to this house. To share on openzoo.fun, use a grokroom (# main / # bots / # random / # lobby). Start ~/grokroom on :4799 if those rows are missing.';
  }
  const line = fanoutLine(agent.id, 'assistant', text, {});
  ssePush('transcript', { ...gatewayEntry(line), agentId: agent.id });
  log(`cursor-backend:      group share hint agent=${agent.id}`);
}

function grokroomEnsureGroup(roomId, name, addr) {
  const id = grokroomAgentId(roomId);
  if (!id) return null;
  const list = cachedAgentList() || [];
  const prev = list.find((a) => a.id === id || a.room?.id === roomId);
  const nextAddr = addr || prev?.room?.addr || '';
  const web = grokroomShareUrl(nextAddr);
  if (prev) {
    const need = (nextAddr && prev.room && prev.room.addr !== nextAddr)
      || (web && prev.room && prev.room.web !== web);
    const agent = need
      ? grokroomUpsertAgent({ ...prev, room: { id: roomId, addr: nextAddr, web } }, { select: false })
      : prev;
    paintShareIfMissing(agent);
    return agent;
  }
  const agent = grokroomUpsertAgent({
    id,
    name: name || ('# ' + roomId),
    isGroup: true,
    memberIds: [],
    brief: '',
    description: '',
    room: { id: roomId, addr: nextAddr, web },
  }, { select: false });
  const nonce = `oz-room-${id}`;
  grokroomMemberEnsure(agent, 'room');
  const text = 'Linked to ' + agent.name + ' on Solana testnet. Every message here is a signed memo on-chain — the chain is the log.'
    + (web ? ' Share: ' + web : '')
    + (nextAddr ? ' Explorer: https://solscan.io/account/' + nextAddr + '?cluster=testnet' : '');
  const line = fanoutLine(id, 'assistant', text, {
    clientNonce: nonce,
    requestId: nonce,
    author: { kind: 'agent', id: grokroomMemberId('room'), name: 'room' },
  });
  ssePush('transcript', { ...gatewayEntry(line), agentId: id });
  return agent;
}

function grokroomSend(agent, body, log = () => {}, { from } = {}) {
  const text = stripSpendFooter(String(body || '')).trim().slice(0, 400);
  const roomKey = agent?.room?.addr || agent?.room?.id;
  if (!text || !roomKey) return;
  if (/^PASS\b/i.test(text)) return;
  grokroomPost(roomKey, text, { from }).then((j) => {
    log(`cursor-backend:      grokroom posted room=${String(roomKey).slice(0, 8)}… sig=${String(j?.sig || '').slice(0, 12)} from=${from || GROKROOM_HANDLE}`);
  }).catch((e) => {
    grokroomMemberEnsure(agent, 'room');
    const line = fanoutLine(agent.id, 'assistant', '⚠ could not reach the room: ' + e.message, {
      author: { kind: 'agent', id: grokroomMemberId('room'), name: 'room' },
    });
    ssePush('transcript', { ...gatewayEntry(line), agentId: agent.id });
    log(`cursor-backend:      grokroom send fail ${roomKey} ${e.message}`);
  });
}

function grokroomIngestMsgs(group, msgs, log = () => {}) {
  if (!group?.id || !Array.isArray(msgs) || !msgs.length) return 0;
  let agent = group;
  const names = new Set();
  for (const m of msgs) {
    const from = String(m.from || m.node || '').slice(0, 24);
    if (from) names.add(from);
  }
  for (const from of names) agent = grokroomMemberEnsure(agent, from);
  const t = agentTranscript(agent.id);
  const seen = new Set((t.entries || []).filter((h) => h.sig).map((h) => h.sig));
  const recent = (t.entries || []).slice(-80).map((h) => String(h.content || ''));
  let dirty = 0;
  for (const m of msgs) {
    const msig = m.sig || m.txSig || m.envsig || (m.pub && m.ts ? m.pub + ':' + m.ts : null);
    if (!msig || seen.has(msig)) continue;
    seen.add(msig);
    const from = String(m.from || m.node || 'anon').slice(0, 24);
    const body = String(m.body || m.text || '');
    if (!body) continue;
    if (recent.includes(body)) continue;
    agent = grokroomMemberEnsure(agent, from);
    const mid = grokroomMemberId(from);
    const line = fanoutLine(agent.id, 'assistant', body, {
      author: { kind: 'agent', id: mid, name: from },
      sig: msig,
    });
    ssePush('transcript', { ...gatewayEntry(line), agentId: agent.id });
    dirty += 1;
  ...
  if (dirty) {
    bumpAgent(agent.id, { preview: msgs[msgs.length - 1]?.body || '', notify: true, select: false });
    log(`cursor-backend:      grokroom ${agent.room?.id || agent.id} +${dirty} bubbles`);
  }
  return dirty;
}

async function grokroomSyncPaymaster(log = () => {}) {
  const list = cachedAgentList() || [];
  const seen = new Set();
  for (const a of list) {
    const addr = a?.room?.addr;
    if (!grokroomShareUrl(addr) || seen.has(addr)) continue;
    seen.add(addr);
    try {
      const r = await fetch(`${GROKROOM_PAYMASTER}/api/room/history?addr=${encodeURIComponent(addr)}`, {
        signal: AbortSignal.timeout(8000),
      });
      if (!r.ok) continue;
      const j = await r.json().catch(() => ({}));
      const msgs = Array.isArray(j.msgs) ? j.msgs : [];
      grokroomIngestMsgs(a, msgs, log);
    } catch (e) {
      log(`cursor-backend:      grokroom history ${String(addr).slice(0, 8)}… ${e.message}`);
    }
  }
}

let grokroomBusy = false;
async function grokroomSync(log = () => {}) {
  if (grokroomBusy) return;
  grokroomBusy = true;
  try {
    try {
      const stRes = await fetch(GROKROOM_API + '/state', { signal: AbortSignal.timeout(4000) });
      if (stRes.ok) {
        const st = await stRes.json();
        const rooms = Array.isArray(st.rooms) ? st.rooms : [];
        grokroomSync._rooms = rooms;
        if (!grokroomSync._logged) {
          grokroomSync._logged = true;
          log(`cursor-backend:      grokroom sync tick: rooms=${rooms.map((r) => r.id).join(',')}`);
        }
        for (const r of rooms) {
          const roomId = r.id || r.name;
          if (!roomId) continue;
          const group = grokroomEnsureGroup(roomId, r.name, r.addr || r.address);
          if (!group) continue;
          const feedRes = await fetch(GROKROOM_API + '/feed?room=' + encodeURIComponent(roomId), { signal: AbortSignal.timeout(4000) });
          if (!feedRes.ok) continue;
          const feed = await feedRes.json();
          grokroomIngestMsgs(group, Array.isArray(feed.messages) ? feed.messages : [], log);
        }
      }
    } catch (e) {
      if (!grokroomSync._localMiss) {
        grokroomSync._localMiss = true;
        log(`cursor-backend:      grokroom local :4799 down (${e.message}); paymaster only`);
      }
    }
    await grokroomSyncPaymaster(log);
  } catch (e) {
    log(`cursor-backend:      grokroom sync: ${e.message}`);
  } finally {
    grokroomBusy = false;
  }
}

function startGrokroomBridge(log = () => {}) {
  if (startGrokroomBridge._on) return;
  if (process.env.OZ_GROKROOM === '0') {
    log('cursor-backend:      grokroom bridge off (OZ_GROKROOM=0)');
    return;
  }
  startGrokroomBridge._on = true;
  const tick = () => grokroomSync(log);
  setTimeout(tick, 800);
 ...
 */
export async function startCursorBackend({ port = 8443, models, log = () => {} } = {}) {
  restoreAgentWakeups(log);
  startGrokroomBridge(log);
  const { cert, key } = await ensureCert(log);
  ...
  const server = http2.createSecureServer(
    ...
    async (req, res) => {
      ...
      // HIJACK LOGIN. Measured Linux 2026-09-03: GET /auth/poll -> empty-ok
      // (`{}`) so SandBackendLoginManager threw bad_200 and the UI stayed on
      // "Log in with Cursor". Poll must return accessToken+refreshToken JWT.
      // /oauth/token refresh is the same session — do NOT passthrough fake
      // tokens to real api2. Sniff/passthru keeps the real issuer.
      {
        const fakeAuth = fakeCursorAuthReply(full, { passthrough: sniffOn() || process.env.OPENZOO_PASSTHRU === '1' });
        if (fakeAuth) {
          if (req.method === 'OPTIONS') { res.writeHead(204, CORS); res.end(); return; }
          res.writeHead(200, { 'content-type': fakeAuth.contentType, ...CORS });
          res.end(fakeAuth.body);
          log(`cursor-backend:      -> FAKE ${fakeAuth.kind} logged-in (no Cursor account)`);
          return;
        }
      }
