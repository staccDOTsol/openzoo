...
async function grokroomPost(roomId, body) {
  ...
  const pub = id.pub;
  const canonical = JSON.stringify({ body, from, pub, room: roomId, ts });
  ...
  const r = await fetch(GROKROOM_API + '/submit', {
    method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ room: roomId, from, pub, body, ts, sig }),
    signal: AbortSignal.timeout(30000),
  });
  const j = await r.json().catch(() => ({}));
  if (!r.ok || j.ok === false) throw new Error(j.error || ('submit HTTP ' + r.status));
  return j;
}
...
async function paintGroupShareHint(agent, log = () => {}) {
  if (!agent?.id) return;
  const own = grokroomShareUrl(agent.room?.addr) || String(agent.room?.web || '');
  let text = '';
  if (own) {
    text = 'Share this room on openzoo.fun: ' + own;
  } else {
    ...
    const links = rooms.map((r) => {
      const u = r.web || grokroomShareUrl(r.addr || r.address);
      return u ? `${r.name || ('# ' + r.id)}  ${u}` : '';
    }).filter(Boolean);
    if (links.length) {
      text = 'This group is local to this house (anyone on this hijack/cafe sees the tray). Public grokrooms — send this URL:\n' + links.join('\n');
    } else {
      text = 'This group is local to this house. To share on openzoo.fun, use a grokroom (# main / # bots / # random / # lobby) — each has Share: https://openzoo.fun/r/<room-pubkey>. Start ~/grokroom on :4799 if those rows are missing.';
    }
  }
...
function grokroomEnsureGroup(roomId, name, addr) {
  ...
  if (prev) {
    if (addr && prev.room && !prev.room.addr) {
      return grokroomUpsertAgent({ ...prev, room: { id: roomId, addr } }, { select: false });
    }
    return prev;
  }
...
function grokroomSend(agent, body, log = () => {}) {
  ...
  if (!text || !agent?.room?.id) return;
  grokroomPost(agent.room.id, text).catch((e) => {
    grokroomMemberEnsure(agent, 'room');
