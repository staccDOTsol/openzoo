...
import { readHouseRoster, groupShareState } from './grokbotAccount.js';
...
export async function startGrokBotWeb(opts = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (urlPath === '/oz-share') {
      ...
      let rooms = [];
      try {
        const st = await fetch('http://127.0.0.1:4799/state', { signal: AbortSignal.timeout(1500) }).then((r) => r.json());
        rooms = Array.isArray(st.rooms) ? st.rooms : [];
      } catch { /* grokroom down */ }
      const share = groupShareState({ agent: { isGroup: rooms.length > 0 }, rooms });
      res.end(JSON.stringify({ ok: true, ...share }));
      return;
    }
