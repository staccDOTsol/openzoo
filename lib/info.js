...
export async function quotedPrices() {
  ...
  try {
    ...
    const { timedFetch, probeTimeoutMs } = await import('./upstream.js');
    const r = await timedFetch(`${config.apiBase}/v1/chat/completions`, {
      method: 'POST',
...
export async function creditBalance() {
  ...
  try {
    const { timedFetch, probeTimeoutMs } = await import('./upstream.js');
    const r = await timedFetch(`${config.apiBase}/v1/credits`, { headers: withNamespace({}) }, { timeoutMs: probeTimeoutMs() });
