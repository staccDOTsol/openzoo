...
import { CREDIT_TIMEOUT_MS, fetchProbe } from './fetch.js';
...
export async function quotedPrices() {
  ...
  try {
    ...
    const r = await fetchProbe(`${config.apiBase}/v1/chat/completions`, {
      method: 'POST',
...
export async function creditBalance() {
  const { withNamespace } = await import('./namespace.js');
  try {
    const r = await fetchProbe(`${config.apiBase}/v1/credits`, {
