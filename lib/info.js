...
/**
 * Balance across every rail: Solana (USDC + KISS + SOL) and each EVM chain
 ...
 * printing `$?`. So a wallet holding 845,486 KISS summed to "≈ $0.11 known".
 *
...
export async function quotedPrices() {
  ...
  try {
    ...
    // holding at $1: MEASURED, a wallet of 776,302 KISS (actually worth ~$178 at the
    ...
    // uses an unfunded signing identity so discovery cannot spend wallet credit.
 ...
 * our balance by it. That way KISS (whose USD price we do not carry) is
...
export async function affordableUsd() {
  ...
  for (const row of ch.accepts || []) {
    ...
    try {
      ...
      // 14,847 KISS their wallet "covers only $1.0230".
      let usd;
...
export async function topUp(usdArg) {
  ...
  //
  // It used to be, and the result was alarming: with a KISS-heavy wallet the
...
export async function creditBalance(wallet) {
  const { withNamespace } = await import('./namespace.js');
  const r = await fetch(`${config.apiBase}/v1/credits`, {
    headers: withNamespace({}, wallet), cache: 'no-store',
    signal: AbortSignal.timeout(CREDIT_TIMEOUT_MS),
  });
  if (!r.ok) throw new Error('Credit balance is temporarily unavailable');
  const j = await r.json();
  if (typeof j.balanceUsd !== 'number' || !Number.isFinite(j.balanceUsd) || j.balanceUsd < 0) throw new Error('Credit balance is temporarily unavailable');
  return j.balanceUsd;
}
