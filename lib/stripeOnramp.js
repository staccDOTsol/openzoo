...
export function whopFundBlurb(solana, evm) {
  ...
  return [
    ...
    addr,
    // OR FUND THE WALLET DIRECTLY. Every failed payment ends here: card via
    // Whop, or USDC to either address — those are the only three moves a
    // stuck caller has, so they are the whole message.
    '',
    `Or send USDC directly — Solana: ${addr}${base ? `  ·  Base: ${base}` : ''}`,
  ].join('\n');
}
