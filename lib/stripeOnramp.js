...
export function whopFundBlurb(solana, evm) {
  const addr = String(solana || '').trim();
  if (!addr) return '';
  const url = whopBuyUrl();
  const base = String(evm || '').trim();
  return [
    'Funding options:',
    `Card: ${url}`,
...
export function settleFailCopy(q402) {
  ...
  const message = fund
    ? 'Your available balance does not cover this request yet. If you just added funds, wait for confirmation and try again.'
    : 'We couldn’t complete the payment for this request. Please try again. If you already added funds, you don’t need to add them again.';
  return { message, status: fund ? 402 : 503, fund, reason, code: String(code || '') };
}

export async function withOnrampLink(text, dest) {
  const body = String(text || '').trim();
  if (!dest?.force && !isFundInstruction(body, dest)) return body;
  const blurb = whopFundBlurb(dest?.solana, dest?.evm);
