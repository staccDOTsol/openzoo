 ...
 */
export function loadOrCreateWallet({ create = true } = {}) {
  ...
  if (fs.existsSync(p)) {
    ...
    if (!create) throw new Error("wallet.json is missing. Finish replacing the wallet file and try again.");
    solanaBytes = Keypair.generate().secretKey;
    created = true;
  }
  // Validate before migrating, so a partial replacement is never overwritten.
  const keypair = Keypair.fromSecretKey(solanaBytes);
  if (evmKey && !/^0x[0-9a-f]{64}$/i.test(evmKey)) throw new Error("Invalid EVM wallet key");
  if (!evmKey) {
