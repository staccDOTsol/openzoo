...
import { billingObserver, finalBilling } from './billing.js';
import { liveBalance } from './funding-status.js';
import { walletReloader } from './wallet-reload.js';
...
import { PayClient, QuoteTooHighError, UnderfundedError, BalanceUnavailableError, resetBalanceCache, resetRailMemory } from './pay.js';
import { withOnrampLink, settleFailCopy, whopFundBlurb } from './stripeOnramp.js';
...
import { detectModelCommand, applyModelCommand, serveModelCommand, readOverride } from './modelcommand.js';
 ...
 *  Windows: netstat -ano + taskkill — lsof/kill do not exist (measured 2026-09-03).
 *  Every openzoo subcommand that binds :8402 goes through this. Hopping to
 *  8403 was the second-burner bug. */
export function killListen(port, run = execSync, platform = process.platform) {
  const n = Number(port);
  const seen = new Set();
  const addFrom = (out) => {
  ...
  const tryRun = (cmd, opts = {}) => {
    try {
      return run(cmd, { encoding: 'utf8', timeout: 2000, windowsHide: true, stdio: ['ignore', 'pipe', 'ignore'], ...opts });
    } catch { return ''; }
  };
  if (platform === 'win32') {
    const ns = tryRun('netstat -ano -p tcp');
    for (const line of String(ns).split(/\r?\n/)) {
      if (!/LISTENING/i.test(line)) continue;
      const m = line.match(new RegExp(`[:\\]]${n}\\s+\\S+\\s+LISTENING\\s+(\\d+)\\s*$`, 'i'));
      if (!m) continue;
      const pid = Number(m[1]);
      if (Number.isInteger(pid) && pid > 0 && pid !== process.pid) seen.add(pid);
    }
    for (const pid of seen) {
      try { run(`taskkill /PID ${pid} /F`, { stdio: 'ignore', timeout: 2000, windowsHide: true }); } catch { /* already gone */ }
    }
    return [...seen];
  }
  addFrom(tryRun(`lsof -nP -iTCP:${n} -sTCP:LISTEN -t`));
  addFrom(tryRun(`fuser -n tcp ${n}`));
  if (platform === 'linux' && seen.size === 0) {
...
export async function startProxy({ silent = false, requireToken = null, sessionMaxUsd = null, autoTunnel = false } = {}) {
  let client = new PayClient();
  const currentWallet = walletReloader(client, { file: client.walletPath, load: () => new PayClient(loadOrCreateWallet({ create: false })) });
  // HOUSE OUTAGE GATE. Measured 2026-09-01: KISS payments settled on chain
  ...
  let comparedSpent = restored.comparison?.spentUsd ?? 0, comparedDirect = restored.comparison?.directUsd ?? 0, comparedCalls = restored.comparison?.calls ?? 0;
  let verifiedCharged = restored.billing?.chargedUsd ?? 0, verifiedCalls = restored.billing?.calls ?? 0;
  const ledger = receiptLedger(sessionSpendFile() + '.receipts.json', { billing: restored.billing, comparison: restored.comparison });
  const applyLedger = ({billing, comparison}) => {
    verifiedCharged = billing.chargedUsd; verifiedCalls = billing.calls;
    comparedSpent = comparison.spentUsd; comparedDirect = comparison.directUsd; comparedCalls = comparison.calls;
  };
  applyLedger(ledger.totals());
  const rememberSpend = () => {
    saveSessionSpend({ spentUsd: sessionSpent, cogsUsd: sessionCogs, directUsd: sessionDirect, paidCalls, billing: { chargedUsd: verifiedCharged, calls: verifiedCalls }, comparison: { spentUsd: comparedSpent, directUsd: comparedDirect, calls: comparedCalls } });
  };
  ...
  let tunnelSpent = 0;
  // Last `from -> to` model rewrite we announced. The override applies to EVERY
  // paid request, and saying so on every one buries the payment lines it sits
  // next to; the user needs to know once, when it changes.
  let saidOverride = null;
  ...
  // CREDIT, CACHED. Refreshed every few seconds and served stale in between —
  ...
  let walletUsd = null;
  let walletAt = 0;
  const walletStatus = liveBalance(async () => {
    const readingClient = client;
    const snap = await snapshotBalances(readingClient);
    if (readingClient !== client) throw new Error("Wallet changed during balance read");
    if (!snap.length) throw new Error('Balances unavailable');
    const previous = new Map((lastSnap || []).map(b => [`${b.chain}:${b.symbol}`, b]));
    for (const b of snap) {
      const old = previous.get(`${b.chain}:${b.symbol}`);
      if (b.ui > (old?.ui ?? 0)) { resetBalanceCache(); resetRailMemory(); }
      previous.set(`${b.chain}:${b.symbol}`, b);
    }
    lastSnap = [...previous.values()];
    return priceHoldings(lastSnap, lastPrices).chainUsd;
  });
  const creditStatus = liveBalance(() => creditBalance(client));
  const refreshWallet = async (force = false) => {
    const readingClient = client;
    const value = await walletStatus.refresh(force);
    if (readingClient !== client) return walletUsd;
    walletUsd = value;
    walletAt = walletStatus.checkedAt;
    return walletUsd;
  };
  const refreshCredit = async (force = false) => {
    const readingClient = client;
    const value = await creditStatus.refresh(force);
    if (readingClient !== client) return creditUsd;
    creditUsd = value;
    creditAt = creditStatus.checkedAt;
    return creditUsd;
  };
  let pricesInflight = null;
  const refreshPrices = async () => {
    if (Date.now() - pricesAt < 60000) return lastPrices;
    if (!pricesInflight) {
      pricesAt = Date.now();
      pricesInflight = quotedPrices().then(prices => { lastPrices = prices; })
        .catch(() => {}).finally(() => { pricesInflight = null; });
    }
  ...
  const reloadWallet = () => {
    ...
    if (next !== client) {
      client = next;
      resetBalanceCache(); resetRailMemory();
      walletStatus.reset(); creditStatus.reset();
      lastSnap = null; walletUsd = creditUsd = null; walletAt = creditAt = 0;
      say(`wallet replaced · now using Solana ${client.address} · Base ${client.evmAddress}`);
      refreshWallet(true); refreshCredit(true);
    }
    return client;
  };
  const server = http.createServer(async (req, res) => {
    // Every request keeps one client even if the file changes mid-response.
    let client;
    try { client = reloadWallet(); } catch (error) { jsonErr(res, 503, error.message); return; }
    ...
    if (req.method === 'GET' && (req.url || '').split('?')[0] === '/v1/funding') {
      await Promise.all([refreshCredit(true), refreshWallet(true), refreshBalances()]);
      res.writeHead(200, { 'content-type': 'application/json', 'cache-control': 'no-store' });
      res.end(JSON.stringify({ creditUsd, walletUsd, creditCheckedAt: creditAt || null, walletCheckedAt: walletAt || null }));
      return;
    }
    ...
    // unauthenticated: discovery must never be the thing that is gated.
    {
      ...
      if (req.method === 'GET' && (p0 === '/v1/info' || p0 === '/info')) {
        ...
        res.end(JSON.stringify({
          ...
          billing: { chargedUsd: verifiedCharged, calls: verifiedCalls, scope: "saved verified receipts" },
          comparison: { spentUsd: comparedSpent, directUsd: comparedDirect, calls: comparedCalls },
          ...
          auth: viaTunnel
            ? 'this public URL requires the configured oz_… bearer for memory and inference; /v1/models is public'
    ...
    if (viaTunnel) {
      const got = (req.headers.authorization || '').replace(/^Bearer\s+/i, '').trim();
      // Only the configured token authorizes signing with the operator's wallet.
      const authed = Boolean(tunnelGate?.token) && got === tunnelGate.token;
    ...
    if (isPaidPost) {
      ...
      // `/model <name>` IS A COMMAND, NOT A QUESTION.
      //
      // A user typed `/model fable-5` into ChatGPT desktop (Codex side, pointed
      // here). The app has no such command, so it sent the line to the model as
      // an ordinary turn — which then spent six minutes and real money shelling
      // around the gateway trying to work out what was being asked, and the
      // model never changed. Answer it here instead: no upstream call, no 402,
      // no payment, and the choice is remembered in ~/.openzoo/model so it
      // outlives the proxy. Must sit AHEAD of the replay key and the pay path.
      try {
        const parsed = JSON.parse(bodyBuf.toString('utf8'));
        const cmd = parsed ? detectModelCommand(parsed) : null;
        if (cmd) {
          let ids = [];
          ...
          const out = applyModelCommand(cmd.arg, ids);
          say(`   /model ${cmd.arg || '(status)'} — answered locally, nothing paid`);
    ...
    try {
      ...
      const requestReceiptId = crypto.randomUUID();
      let accounted = paid && receipt?.ok && typeof receipt.billedUsd === 'number' ? receipt.billedUsd : 0;
      let counted = paid && receipt?.ok && typeof receipt.billedUsd === 'number';
      let finalX = {}, finalUsage = {}, comparison = null, actualPair = null, verified = null;
      const meterFinal = (x = {}) => {
        const { usage, ...fields } = x;
        finalX = { ...finalX, ...fields };
        finalUsage = { ...finalUsage, ...usage };
        const final = finalBilling(finalX, finalUsage);
        if (final.charged === null) return;
        if (verified === null) verifiedCalls++;
        verifiedCharged += final.charged - (verified ?? 0);
        verified = final.charged;
        const delta = final.charged - accounted;
        sessionSpent += delta;
        if (viaTunnel) tunnelSpent += delta;
        if (!counted) { paidCalls++; counted = true; }
        accounted = final.charged;
        if (comparison) { comparedSpent -= comparison.charged; comparedDirect -= comparison.direct; comparedCalls--; comparison = null; }
        if (final.direct !== null) {
          comparison = final;
          comparedSpent += final.charged; comparedDirect += final.direct; comparedCalls++;
        }
        if (actualPair) { sessionActual -= actualPair.upstream; billedWithActual -= actualPair.charged; actualCalls--; }
        actualPair = final.upstream === null ? null : final;
        if (actualPair) { sessionActual += final.upstream; billedWithActual += final.charged; actualCalls++; }
        applyLedger(ledger.record(finalX.settle?.transaction || finalX._responseId || receipt?.tx || requestReceiptId, final));
        rememberSpend();
        log(`final billing: charged $${final.charged.toFixed(6)} · upstream ${final.upstream === null ? 'unreported' : '$' + final.upstream.toFixed(6)} · direct ${final.direct === null ? 'unreported' : '$' + final.direct.toFixed(6)}`);
      };
      ...
      if (isPaidPost && response.ok && upCt.includes('application/json')) {
        ...
        meterFinal({ ...data?.x402, usage: data?.usage, _responseId: data?.id, _completedResponse: response.ok });
      ...
      const meterStreamed = meterFinal;
      ...
      if (response.status === 402) {
        ...
        // EVERY PAYMENT FAILURE ENDS WITH HOW TO PAY. A caller staring at a
        // facilitator's wording ("execution reverted", "authorization value
        // mismatch") has exactly three moves — card via Whop, USDC on Solana,
        // USDC on Base — so every 402 after a payment carries all three, not
        // only the ones the gateway managed to classify as insufficient funds.
        msg = await withOnrampLink(msg, { solana: client.address, evm: client.evmAddress, usd, code: copy.code, force: true });
        log(/At checkout/i.test(msg) ? 'onramp: whop + copy-paste solana' : `402: ${msg.slice(0, 140)}`);
        log(`payment diagnostic: ${copy.reason}`);
      ...
      if (err instanceof QuoteTooHighError) {
        log(err.message);
        paymentError(res, req, init, 400, 'This request exceeds your spending limit. Try a shorter request or change your limit.');
      } else if (err instanceof UnderfundedError) {
        const msg = await withOnrampLink(err.message, {
          solana: client.address, evm: client.evmAddress, usd: Number(err.accept?.extra?.billedUsd),
        });
        log(msg);
        paymentError(res, req, init, 402, `Your available balance does not cover this request yet.\n\n${whopFundBlurb(client.address, client.evmAddress)}`);
      } else if (err instanceof BalanceUnavailableError) {
        paymentError(res, req, init, 503, `${err.message}\n\n${whopFundBlurb(client.address, client.evmAddress)}`);
      } else {
        ...
        paymentError(res, req, init, 502, 'OpenZoo couldn’t connect right now. Please try again.');
      }
    }
  });
  ...
  // KISS top-up, and an old npx cache answering /v1 with no version.
  {
  ...
  // Funding checks run for desktop/background launches as well as terminal sessions.
  refreshBalances = () => refreshWallet(true);
  refreshWallet(); refreshCredit(); refreshPrices();
  const fundingTimer = setInterval(() => { try { reloadWallet(); refreshCredit(); refreshWallet(); refreshPrices(); } catch { /* request path reports an incomplete wallet replacement */ } }, 3000);
  fundingTimer.unref?.();
  server.on('close', () => clearInterval(fundingTimer));
