...
export function killListen(port, run = execSync, platform = process.platform) {
  ...
  if (platform === 'win32') {
    ...
    for (const pid of seen) {
      try { run(`taskkill /PID ${pid} /F`, { stdio: 'ignore', timeout: 2000 }); } catch { /* already gone */ }
    }
    return [...seen];
  }
...
export async function startProxy({ silent = false, requireToken = null, sessionMaxUsd = null, autoTunnel = false } = {}) {
  ...
  const creditStatus = liveBalance(creditBalance);
  const refreshWallet = async (force = false) => {
    walletUsd = await walletStatus.refresh(force);
    walletAt = walletStatus.checkedAt;
    return walletUsd;
  };
  const refreshCredit = async (force = false) => {
    creditUsd = await creditStatus.refresh(force);
  ...
  const fundingTimer = setInterval(() => { refreshCredit(); refreshWallet(); refreshPrices(); }, 3000);
  fundingTimer.unref?.();
