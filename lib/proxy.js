...
import { rewritablePath, modelsListForRequest, isHarnessAliasId, rewriteChatModel, zooModelIds, CLASSIFY_MAX_TOKENS, raiseReasoningMaxTokens, isAutoModel, isTinyClassify } from './models.js';
import {
  bashLoops, extractLastBashCommand, extractLastBashStdout, hasCompileArtifact, bashStopText,
} from './bashloop.js';
import {
...
export async function startProxy({ silent = false, requireToken = null, sessionMaxUsd = null, autoTunnel = false } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // Identical/near-identical Bash/grep: do not pay or execute more than twice.
    // After a compile artifact (.so), further declare_id greps stop.
    if (isChat) {
      try {
        const loopBody = JSON.parse(bodyBuf.toString('utf8'));
        const sid = req.headers['x-claude-code-session-id']
          || req.headers['x-session-id']
          || req.headers['x-claude-session-id']
          || req.headers['x-openzoo-namespace']
          || '__default__';
        const cmd = extractLastBashCommand(loopBody.messages);
        const out = extractLastBashStdout(loopBody.messages);
        if (cmd && out) bashLoops.rememberStdout(sid, cmd, out);
        if (hasCompileArtifact(loopBody.messages)) bashLoops.markArtifact(sid);
        if (cmd) {
          const tiny = isTinyClassify(loopBody, bodyBuf.length);
          const verdict = bashLoops.decide(sid, cmd, { messages: loopBody.messages, stdout: out });
          if (verdict.stop) {
            const text = tiny ? 'no' : (verdict.text || bashStopText(verdict.stdout, verdict.reason));
            say(`bash-loop stop (${verdict.reason || 'repeat'}) � not re-paid: ${String(cmd).slice(0, 80)}`);
            const data = {
              id: `chatcmpl-loopstop-${Date.now()}`,
              object: 'chat.completion',
              created: Math.floor(Date.now() / 1000),
              model: loopBody.model || 'openzoo',
              choices: [{ index: 0, message: { role: 'assistant', content: text }, finish_reason: 'stop' }],
            };
            if (responsesMode) {
              if (wantsStream) { writeResponsesSse(res, data, responsesModel, null, responsesCustom); return; }
              res.writeHead(200, { 'content-type': 'application/json' });
              res.end(JSON.stringify(chatToResponses(data, responsesModel, responsesCustom)));
              return;
            }
            if (anthropicMode) {
              const msg = openAIToAnthropic(data, anthropicModel);
              if (wantsStream) { writeAnthropicSse(res, msg, null); return; }
              res.writeHead(200, { 'content-type': 'application/json' });
              res.end(JSON.stringify(msg));
              return;
            }
            if (wantsStream) { serveAsSse(res, data, null); return; }
            res.writeHead(200, { 'content-type': 'application/json' });
            res.end(JSON.stringify(data));
            return;
          }
          if (tiny) bashLoops.note(sid, cmd);
        }
      } catch { /* loop guard must never break a call */ }
    }
