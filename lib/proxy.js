...
import { rewritablePath, modelsListForRequest, ALIAS_IDS, rewriteChatModel, zooModelIds, CLASSIFY_MAX_TOKENS, raiseReasoningMaxTokens, isAutoModel } from './models.js';
import {
    ...
    if (rewritablePath(req.method, req.url)) {
      try {
        ...
        if (!policy.tiny && (policy.auto || isAutoModel(parsed?.model))) {
          autoRoute = routeChatBody(parsed, { allow_free: false, bindable: true });
          if (!autoRoute.model) {
