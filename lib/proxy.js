...
import { hrrAskHeaders, retrieveBoundCorpus } from './retrieve.js';
import {
...
import {
  ...
  isNoToolDumpId, routeInputFromChat,
...
import {
  receiptUsedCogs, receiptDirectUsd, pairActualBilled,
  userChargedUsd, hasX402Charge, responseLooksFailed,
} from './racesettle.js';
import { rewriteWrapClientError } from './wrap.js';
import { fetchHeaders, fetchProbe } from './fetch.js';
...
async function spillTranscript(body, log, req, stats, extra = {}) {
  ...
  return {
    ...
    sent,
    msgs: msgs.length,
    retrieve: decision.retrieve || null,
  };
}
...
async function maybeCacheCorpus(req, bodyBuf, log, stats, extra = {}) {
  ...
  return {
    ...
    sent: 1,
    msgs: msgs.length,
    retrieve: retrieveBoundCorpus(ask, corpus),
  };
}
...
export async function startProxy({ silent = false, requireToken = null, sessionMaxUsd = null, autoTunnel = false } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
      ...
      if (req.method === 'GET' && (p0 === '/v1/info' || p0 === '/info')) {
        ...
        res.end(JSON.stringify({
          ...
          context: {
            ...
            retrieval: 'lossy top-k via retrieval_dispatch + recall guard, NOT lossless compression',
          },
    ...
    if (rewritablePath(req.method, req.url)) {
      try {
        ...
        if (!policy.tiny && (policy.auto || isAutoModel(parsed?.model))) {
          const routed = routeInputFromChat(parsed);
          autoRoute = routeChatBody(parsed, {
            allow_free: false,
            bindable: true,
            needs_tools: routed.needs_tools,
          ...
          if (routed.needs_tools && autoRoute?.model && isNoToolDumpId(autoRoute.model)) {
            const swap = fallbackChain(autoRoute).find((id) => !isNoToolDumpId(id));
            if (swap) autoRoute = { ...autoRoute, model: swap };
          }
          if (!autoRoute.model) {
    ...
    if (req.method === 'GET' && path === '/v1/models') {
      ...
      try {
        const response = await fetchProbe(url, init);
        if (response.ok) {
    ...
    try {
      ...
      const send = (buf, ctxId, topK, corpusChars) => client.fetch(url, {
        ...
        headers: ctxId
          ? {
            ...init.headers,
            // dispatch is the retrieve front door (exact ? dense margin ?
            // BM25-on-shortlist). top-k and corpus-chars stay for HUD / pay.
            ...hrrAskHeaders({ contextId: ctxId, topK, corpusChars }),
          }
          : init.headers,
      });
      let didSpill = false;
      ...
      if (upCt.includes('application/json')) {
        try { jsonBody = await response.clone().json(); } catch { /* not JSON after all */ }
      }
      const failedUp = responseLooksFailed(response.status, jsonBody);
      const bodyX402 = jsonBody && jsonBody.x402 && typeof jsonBody.x402 === 'object' ? jsonBody.x402 : null;
      const meterBag = bodyX402 && receipt ? { ...receipt, ...bodyX402 }
        : (bodyX402 || ((receipt && hasX402Charge(receipt)) ? receipt : null));
      let meteredThisCall = false;
      // Failures still cost the USER. receipt.ok must not drop a billed
      // completion. Never add cogs without adding spent. A 400 with no x402
      // (and no paid receipt) does not invent a charge.
      const applySessionMeter = (x, { failed: fail, usage, streamed = false } = {}) => {
        if (!hasX402Charge(x)) return;
        const cogsAdd = receiptUsedCogs(x);
        const spentAdd = Math.max(userChargedUsd(x, usage, { failed: fail }), cogsAdd);
        if (spentAdd <= 0 && cogsAdd <= 0) return;
        sessionSpent += spentAdd;
        sessionCogs += cogsAdd;
        sessionDirect += receiptDirectUsd(x);
        noteQuote({ ...x, billedUsd: spentAdd });
        if (didSpill) {
          if (!fail) log(spillPricedLine(x, streamed ? { streamed: true } : undefined));
          spill.spillSpend += spentAdd;
          spill.spillDirect += receiptDirectUsd(x);
        }
        paidCalls += 1;
        if (viaTunnel) tunnelSpent += spentAdd;
        rememberSpend();
        meteredThisCall = true;
        const label = fail
          ? `failed HTTP ${response.status} -> $${spentAdd.toFixed(6)}`
          : (paid && receipt?.line ? receipt.line : `credit -> $${spentAdd.toFixed(6)}`);
        if (requireToken) say(`${label}  �  session $${sessionSpent.toFixed(6)}`);
        else if (viaTunnel) say(`${label}  �  public-url session $${tunnelSpent.toFixed(6)}`);
        else say(label);
      };
      if (upCt.includes('application/json')) {
        if (meterBag) applySessionMeter(meterBag, { failed: failedUp, usage: jsonBody?.usage });
      } else if (paid && receipt && hasX402Charge(receipt)) {
        applySessionMeter(receipt, { failed: !receipt.ok || failedUp });
      }
      if (paid && receipt) {
        if (!meteredThisCall) {
        ...
        // TUI's content.
        if (sayFile) {
          try { process.stderr.write(`\x1b]0;openzoo $${sessionSpent.toFixed(4)} � ${paidCalls} call${paidCalls === 1 ? '' : 's'}\x07`); } catch { /* no tty */ }
        }
      ...
      if (isChat && jsonBody && response.ok && upCt.includes('application/json')) {
        const data = jsonBody;
        ...
        // Prepaid 200s (and failed JSON with x402) already hit applySessionMeter
        // above. Do not add billed/cogs a second time here.
      ...
      const meterStreamed = (x) => {
        ...
        if (paid || meteredThisCall || !hasX402Charge(x)) return;
        applySessionMeter(x, { failed: false, usage: x?.usage, streamed: true });
      };
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      // Healer children must stay on :8402. Walking 8403?8406 leaves clients
      // talking to a wedged LISTEN on 8402 (TypeError: fetch failed forever).
      if (process.env.OPENZOO_NO_PORT_WALK === '1') throw e;
      if (attempt === 0) {
        try {
