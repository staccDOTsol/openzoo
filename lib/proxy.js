...
import { rewritablePath, modelsListForRequest, isHarnessAliasId, rewriteChatModel, zooModelIds, CLASSIFY_MAX_TOKENS, isAutoModel } from './models.js';
import {
  route as routeTask, fallbackChain, isRetryableStatus,
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (rewritablePath(req.method, req.url)) {
      try {
        ...
        // Fly (x402-tokens) races Auto and retries 429s. The sidecar must
        // not run a second classifier � leave openzoo/auto on the wire.
        if (policy.auto || isAutoModel(parsed?.model) || isAutoModel(policy.from)) {
          autoRoute = null;
          say('openzoo/auto passthrough (no local rewrite)');
        }
        if (policy.tiny) {
          ...
          if (policy.to && policy.to !== policy.from && !policy.auto) {
    ...
    try {
      ...
      // Walk a local shortlist on 429/5xx � cheapest-first among models that
      // cleared the bar. Finite: one pass over fallbackChain(), never a loop.
      // Auto leaves autoRoute=null so 429s stay on openzoo/auto for Fly.
