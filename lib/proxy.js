...
export async function startProxy({ silent = false, requireToken = null, sessionMaxUsd = null, autoTunnel = false } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'GET' && (req.url || '').split('?')[0] === '/v1/session') {
      // IN-MEMORY ONLY. Never await credit/price probes or the same outbound
      // queue as chat completions � a hung fly.dev fetch used to wedge this
      // handler while the process still LISTENed, which grokui reports as
      // `(seed-2.0-code failed: fetch failed)`. Kick refreshes off the clock.
      refreshCredit();
      refreshPrices();
