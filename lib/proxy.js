...
import {
  ...
  planSpillTail, msgText, createSpillStats, corpusCharsForSend,
} from './spill.js';
...
async function spillTranscript(body, log, req, stats) {
  ...
  // Cut, min-turns floor, never-spill-the-ask, and bound-file stubs live in
  // planSpillTail � the agent-shaped ratio sweep measures the same function.
  const plan = planSpillTail(msgs, { boundAbs: previouslyBoundAbs, stub: true });
  if (!plan.ok) {
    const label = plan.reason === 'under-6'
      ? 'conversation under 6 messages, background'
      : plan.reason === 'no-severable-cut'
        ? 'no severable cut, files only'
        : 'no spill cut, files only';
    bindFilesInBackground(label);
    return null;
  }
  const { cut, firstSpillable, corpus, forwarded } = plan;
  ...
  if (plan.dropped) {
    log(`file-stub stubbed=${plan.stubbed} dropped=${plan.dropped}`);
  }
  ...
  return {
    body: Buffer.from(JSON.stringify({ ...body, messages: forwarded })),
    topK,
    contextId: bind.contextId,
    hash: bind.hash,
    corpus,
