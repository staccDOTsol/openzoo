...
export function packedResourceRoots({
  ...
  if (env.OZ_PACKED_RESOURCES) {
    push(env.OZ_PACKED_RESOURCES);
    push(path.join(env.OZ_PACKED_RESOURCES, 'app'));
    push(path.join(env.OZ_PACKED_RESOURCES, 'app.asar.unpacked'));
  }
  if (resourcesPath) {
    push(resourcesPath);
  ...
  // First-boot copy from extraResources (NSIS resources/node-pty).
  const home = env.HOME || env.USERPROFILE || os.homedir();
  push(path.join(home, '.openzoo', 'packed'));
  push(path.join(home, '.local', 'lib'));
...
export function resourcesFromExecPath(execPath) {
  ...
  // NSIS extraResources land at <install>/resources/node-pty (not only
  ...
  out.push(path.join(dir, 'resources', 'app', 'node_modules'));
  out.push(dir);
  return out;
}
...
export function loadNodePtyFrom(dir) {
  ...
  try {
    ...
    try {
      ...
      if (pkg.main) candidates.push(path.join(dir, pkg.main));
    } catch { /* package.json optional for a copied lib/ tree */ }
    candidates.push(path.join(dir, 'lib', 'index.js'));
