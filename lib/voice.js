...
export async function voiceText(draft, { log = () => {}, kind = 'post' } = {}) {
  ...
  const routedModel = data?.model || VOICE_MODEL;
  const billedUsd = Number(data?.x402?.billedUsd ?? data?.usage?.cost ?? 0);
  const directUsd = Number(data?.x402?.directUsd ?? 0);
  const receipt = priceLine({ routedModel, billedUsd, directUsd });
  const stage = recalled?.stage ?? 'attach';

  // Kept, not just printed: see lib/receipts.js. A terminal line proves
  // nothing an hour later, and the multiple is the whole claim.
  recordReceipt({
    kind: `voice:${kind}`,
    tool: 'voice',
    model: routedModel,
