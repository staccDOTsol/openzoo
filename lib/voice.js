 ...
 * bind uses the authenticated gateway for both text and scored item recall. A rewrite recalls against
...
/**
 * Scored item bind through the wallet-authenticated gateway.
 * Legacy daemonCtx state fields now refer to private gateway contexts;
 * old daemon contexts must be re-ingested.
 *
...
async function daemonRecall(contextId, query, topK) {
  const r = await fetch(`${config.apiBase}/v1/hrr/recall`, {
    method: 'POST',
    headers: withNamespace({ 'content-type': 'application/json' }),
    body: JSON.stringify({ context_id: contextId, query, top_k: topK }),
  });
