...
let completionsFetchGaveUp = null;
export function setCompletionsFetchGiveUp(fn) {
  completionsFetchGaveUp = typeof fn === 'function' ? fn : null;
}
...
async function postChat(body, contextId, topK, onStatus, signal) {
  ...
  for (let attempt = 0; attempt <= PAYMENT_RETRIES; attempt++) {
    ...
    r = await fetchRetry(`${completionsProxy()}/chat/completions`, {
      method: 'POST',
      headers: {
      ...
      onGiveUp() { completionsFetchGaveUp?.(); },
    });
