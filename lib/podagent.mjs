...
export async function brainRace(messages, onDelta, contextId, models, need = 1, maxTokens, onStatus, hooks = {}) {
  ...
  // launched. A slow 3rd never enters this set.
  const cands = done.slice(0, want);
  if (!cands.length) {
    feed.settle(null);
    return '(race: no model returned a reply)';
  }
  ...
  if (picked.reason === 'tie' && picked.tied.length > 1) {
    ...
    picked = { winner: broken || picked.tied[picked.tied.length - 1], reason: 'tiebreak', tied: picked.tied };
  }
