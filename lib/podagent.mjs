...
export async function brainStream(messages, onDelta, contextId, modelOverride, maxTokens, round = 0) {
  const vision = hasImages(messages);
  const model = vision ? VISION_MODEL : (modelOverride || MODEL);
  messages = vision ? messages : stripImages(messages);
...
/**
 * Ask N models the same thing at once; first COMPLETE answer wins.
 *
 * This is a latency and a reliability play, and the reliability half is the
 * reason it exists: empty completions and provider 5xx are per-model and
 * uncorrelated, so the fix for "the model returned nothing 4 times" is not a
 * fourth try at the same model — it is a second model. Racing makes that the
 * default rather than something the user has to notice and do by hand.
 *
 * Streaming is deliberately NOT forwarded while the race runs: nobody knows
 * which model is winning until one finishes, and interleaving deltas from
 * three of them would render as noise. The winner's text is emitted whole.
 *
 * Every entrant is paid for — this trades money for latency, which is why it
 * is opt-in and capped.
 */
export async function brainRace(messages, onDelta, contextId, models, maxTokens) {
  ...
  const attempts = list.map((m) => brainStream(messages, () => {}, contextId, m, maxTokens)
    .then((text) => {
      // An empty reply is a LOSS, not a win — otherwise the fastest model to
      // fail decides the race, which is the exact bug racing is meant to fix.
      if (!text || !text.trim()) throw new Error(`${m} returned nothing`);
      return { model: m, text };
    }));
  ...
  try {
    ...
    settled = true;
    if (winner.text) onDelta(winner.text);
    return winner.text;
  } catch (e) {
    // Promise.any rejects only when EVERY entrant failed.
    return '';
  } finally {
    // Losers keep streaming into a dead callback otherwise; nothing to cancel
    // at the HTTP layer here, but swallow their rejections so an unhandled
    // rejection cannot take the process down after a win.
...
/**
 * Ask N models the same thing and let a small model pick the BEST answer.
 *
 * The sibling of brainRace, and the opposite trade. Racing optimises latency:
 * whoever finishes first wins, which on a hard question rewards the model that
 * thought least. This waits for all of them and judges, so it is as slow as the
 * SLOWEST entrant — but it is the only one of the two that can actually improve
 * an answer rather than just deliver one sooner.
 *
 * The judge is deliberately a cheap model. Judging is a much easier task than
 * answering: it is reading four finished replies and comparing them against a
 * question, not solving anything. Paying frontier prices to referee frontier
 * models would roughly double the cost of the expensive tier for no measured
 * gain.
 *
 * Candidates are presented BLIND, as A/B/C/D. A judge told "this one is Claude
 * and this one is a 4B llama" is being handed the answer, and it will take it —
 * which would make the whole thing an expensive way to re-pick the tier's
 * first entry.
 */
export async function brainBest(messages, onDelta, contextId, models, maxTokens) {
  const list = (models || []).filter(Boolean).slice(0, RACE_MAX);
  if (list.length < 2) return brainStream(messages, onDelta, contextId, list[0], maxTokens);

  const settled = await Promise.allSettled(
    list.map((m) => brainStream(messages, () => {}, contextId, m, maxTokens).then((text) => ({ model: m, text }))),
  );
  const cands = settled
    .filter((s) => s.status === 'fulfilled' && s.value.text && s.value.text.trim())
    .map((s) => s.value);

  if (!cands.length) return '';
  // Nothing to judge between — do not spend a call to rubber-stamp one answer.
  if (cands.length === 1) { onDelta(cands[0].text); return cands[0].text; }

  const letters = cands.map((_, i) => String.fromCharCode(65 + i));
  // The question, not the whole transcript: the judge needs to know what was
  // ASKED, and a full history would cost more to judge than to answer.
  ...
  try {
    ...
    // First letter in range anywhere in the reply. A judge that ignores "one
    // letter and nothing else" and writes "The best is B." still gets counted,
    // which is most of them.
    const hit = String(verdict).toUpperCase().split('').find((ch) => {
      const n = ch.charCodeAt(0) - 65;
      return n >= 0 && n < cands.length;
    });
    if (hit) pick = hit.charCodeAt(0) - 65;
  } catch {
    // A dead judge must not lose the answers. Falling back to the first
    // candidate degrades this to "first entrant wins" — worse than judged,
    // far better than empty.
    pick = 0;
  }

  const winner = cands[pick] || cands[0];
  onDelta(winner.text);
  return winner.text;
}
