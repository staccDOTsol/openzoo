...
function execFrame(command, cwd = '/tmp') {
  return {
    ...
    // `timeout` is MILLISECONDS (the daemon reads it via resolveShellTimeoutMs).
    // This said 120, which aborted every command after 120ms — exit 0, no output.
    serverMessage: { shellArgs: { command: guardFindCwd(command, cwd), workingDirectory: cwd, timeout: 120_000 } },
  };
}
