...
// ---------------------------------------------------------------------------
// MODEL TIERS  ·  cheap / medium / expensive / grok4.6
// ---------------------------------------------------------------------------
...
//
// grok4.6 is NOT a fourth price band — it pins race/auto to the Grok 4.6
// family only. Live catalog (x402-tokens + OpenRouter, 2026-08-20) serves
// `x-ai/grok-4.6` and no 4.6-fast / dated twin. Close cousins (grok-4.5,
// grok-4.3, grok-4.20) stay out. Opus/GPT stay in expensive, not here.
export const TIERS = {
  ...
  // Grok 4.6 family only. Race draws from THIS list, not the whole expensive
  // tier. Add a served 4.6 variant here when the catalog actually has it.
  'grok4.6': [
    'x-ai/grok-4.6',                     // 18
  ],
};
...
/** Canonical slug, or null. Accepts `grok 4.6` / `grok-4.6` / `grok4.6`. */
export function parseTier(raw) {
  const s = String(raw ?? '').trim().toLowerCase();
  if (!s) return null;
  if (TIER_NAMES.includes(s)) return s;
  const compact = s.replace(/[\s_\-]+/g, '');
  if (TIER_NAMES.includes(compact)) return compact;
  if (compact === 'grok46') return 'grok4.6';
  return null;
}

/** x-ai/grok-4.6 and served 4.6 variants (fast, dated, :suffix). Not 4.5 / 4.20. */
export function isGrok46Model(id) {
  return /^x-ai\/grok-4\.6(?=$|[-/:])/i.test(String(id || ''));
}
...
export async function tierModels(tier, n = 1, random = false) {
  const name = parseTier(tier) || (TIERS[tier] ? tier : 'medium');
  let want = TIERS[name] || TIERS.medium;
  const ids = await servedIds();
  // grok4.6 also picks up any served 4.6 twin the catalog grew since we
  // last edited the curated list — still family-only, never opus/gpt.
  if (name === 'grok4.6' && ids) {
    const extra = [...ids].filter((id) => isGrok46Model(id) && !want.includes(id));
    if (extra.length) want = want.concat(extra);
  }
