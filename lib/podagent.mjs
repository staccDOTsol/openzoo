...
//
// grok4.6 is NOT a fourth price band — it pins race/auto to Grok chat
// models, 4.6 first. One slug cannot fill a 4-wide race, so sibling grok
// chat models (4.5 / 4.3 / 4.20) fill the pool. No imagine / stt / tts /
// video. Opus/GPT stay in expensive, not here.
export const TIERS = {
  ...
  // 4.6 first; siblings fill a 4-wide race. Chat only — no imagine/stt/tts/video.
  // Key MUST be quoted: bare grok4.6: is SyntaxError (parsed as grok4 . 6).
  'grok4.6': [
    'x-ai/grok-4.6',
    'x-ai/grok-4.5',
    'x-ai/grok-4.3',
...
/** Canonical slug, or null. /tier grok, grok 4.6, grok-4.6, grok4.6 → grok4.6. */
export function normalizeTier(raw) {
  const s = String(raw ?? '').trim().toLowerCase();
  if (!s) return null;
  if (TIER_NAMES.includes(s)) return s;
  if (TIER_ALIASES[s]) return TIER_ALIASES[s];
...
export async function tierModels(tier, n = 1, random = false) {
  const name = normalizeTier(tier) || (TIERS[tier] ? tier : 'medium');
  const want = TIERS[name] || TIERS.medium;
