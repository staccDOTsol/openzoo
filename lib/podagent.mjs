...
async function httpErrorNote(status) {
  if (status === 402) {
    ...
    try {
      ...
      if (w?.funding && w?.evm) {
        return `(payment failed — HTTP 402. The x402 retry gave up on this call; it often settles on a second try, so just send it again. If it keeps failing, the wallet may need topping up: ${w.funding}. EVM (Base/Robinhood): ${w.evm}.)`;
      }
    } catch { /* proxy unreachable — fall through to the generic note */ }
    return '(payment failed — HTTP 402. The x402 retry gave up on this call; it often settles on a second try, so just send it again. Run `npx openzoo` to see wallet balances if it keeps failing.)';
  }
