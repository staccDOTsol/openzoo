 ...
 * a wallet rich in KISS but short of USDC pays the KISS row instead of
...
export function orderAccepts(body, preferredSymbol, { allowRH = false, forceRail = null } = {}) {
  ...
  // bills $KISS/$LEOS at 2x its cost and USDC/USDG at 4x, so two rows for the
...
export async function tokenBalance(connection, owner, mintStr) {
  try {
    ...
    if (/could not find account|account.*(?:does not exist|not found)/i.test(error.message)) return { raw: 0n, ui: 0 };
    throw error;
  }
}
...
export function receiptLine(accept, settle) {
  ...
  // Only counterfactual pricing supplies a retail comparison. Other modes
  // may put internal costs in directUsd; they do not establish price parity.
  const counterfactual = x.pricing === 'counterfactual';
  const saves = !counterfactual
    ? ' (direct-price comparison unavailable)'
