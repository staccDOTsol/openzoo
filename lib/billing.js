    ...
    while ((i = pending.indexOf('\n')) >= 0) {
      ...
      try {
        if (line.startsWith(': x402 ')) onReceipt(JSON.parse(line.slice(7)));
          ...
          if (value.usage || value.x402) onReceipt({ ...value.x402, usage: value.usage, _responseId: value.id, _completedResponse: event.type === 'response.completed' || value.status === 'completed' });
        }
