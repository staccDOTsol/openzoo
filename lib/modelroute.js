 ...
 *
 * Artifacts: vendor/modelroute/catalog.json + router.json (Python reference
 * holographic_modelroute.py is not a runtime dep).
 */
...
export class Outcomes {
  ...
  constructor(filePath = defaultOutcomesPath()) {
    this.path = filePath;
    this.tab = {};
    if (filePath && existsSync(filePath)) {
      try { this.tab = JSON.parse(readFileSync(filePath, 'utf8')); } catch { this.tab = {}; }
    }
  }
  ...
  record(taskClass, modelId, ok) {
    ...
    const [s, n] = this.tab[k] || [0, 0];
    this.tab[k] = [s + (ok ? 1 : 0), n + 1];
    if (this.path) {
      mkdirSync(path.dirname(this.path) || '.', { recursive: true });
      const keys = Object.keys(this.tab).sort();
      const ordered = {};
      for (const key of keys) ordered[key] = this.tab[key];
      writeFileSync(this.path, JSON.stringify(ordered, null, 0));
    }
    return this.tab[k];
  }
...
export class Catalog {
  ...
  feasible(cons, allowFree = false) {
    const ok = new Array(this.ids.length).fill(true);
    for (let i = 0; i < this.ids.length; i++) {
      if (!allowFree && !(this.price_in[i] > 0 || this.price_out[i] > 0)) ok[i] = false;
...
export function route(text, {
  ...
  const feas = cat.feasible(cons, allow_free);
...
export function getOutcomes() {
  if (!_outcomes) _outcomes = new Outcomes(defaultOutcomesPath());
  return _outcomes;
}
