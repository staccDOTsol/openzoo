 ...
 * User pays for every entrant we actually launched. Failures still cost the
 * USER (OpenRouter was paid). race_unused on a receipt is informational — do
 * not treat it as a user refund. A failed completion that carries x402 is
 * billed at least at house cost. HUD embers only when cogs has no matching
 * billed — a house meter bug, not a correctly passed-through 400.
 */
 ...
 * grant-back is not a user refund. Failures are a user charge equal to
 * house cost (see userChargedUsd). Does not clamp to billed.
 *
...
/** True when the bag has a real x402 charge — do not invent one on a bare 400. */
export function hasX402Charge(x) {
  if (!x || typeof x !== 'object') return false;
  return money(x.cogsUsd) != null || money(x.billedUsd) != null
    || money(x.actualUsd) != null || money(x.usage?.cost) != null;
}

/** HTTP 400/402/5xx or an error-shaped JSON body. */
export function responseLooksFailed(status, data) {
  const n = Number(status);
  if (Number.isFinite(n) && n >= 400) return true;
  if (data && typeof data === 'object') {
    if (data.error) return true;
    if (data.object === 'error') return true;
...
/**
 * What the user is billed for this completion.
 *
 * Failed: max(billedUsd, cogsUsd, usage.cost). Do not keep a cheap spill
 * quote when the call failed. Clamp to directUsd only if direct is a real
 * number and ≥ cogs — never clamp below cogs.
 *
 * Success: keep the spill/quote billed when it already covers cogs.
 * Always: charge ≥ cogs so session never adds cogs without spent.
 */
export function userChargedUsd(x, usage, { failed = false } = {}) {
  if (!x || typeof x !== 'object') return 0;
  const billed = money(x.billedUsd) ?? 0;
  const cogs = receiptUsedCogs(x);
  const cost = money(usage?.cost) ?? money(x.actualUsd) ?? money(usage?.actualUsd)
    ?? money(x.usage?.cost) ?? 0;
  if (!failed) return Math.max(billed, cogs);
  let charge = Math.max(billed, cogs, cost);
  const direct = money(x.directUsd);
  if (direct != null && direct >= cogs) charge = Math.min(charge, direct);
  return Math.max(charge, cogs);
}
...
/**
 * Session meter. spent is what the user is charged — at least house cost
 * on a failure, and never less than cogs (never add cogs without spent).
 * race_unused is not a user refund; failed/unused entrants stay on the tab.
 */
export function meterRaceReceipt(x, usage, opts = {}) {
  const failed = opts.failed === true;
  return {
    spentUsd: userChargedUsd(x, usage, { failed }),
    cogsUsd: receiptUsedCogs(x),
    directUsd: receiptDirectUsd(x),
  };
}
