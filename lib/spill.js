...
/** Chars the gateway will see in a forwarded message list (content + tool_call args). */
export function forwardedChars(msgs) {
  let n = 0;
  if (!Array.isArray(msgs)) return 0;
  for (const m of msgs) {
    n += toolContentLength(m?.content);
    const calls = [
      ...(Array.isArray(m?.tool_calls) ? m.tool_calls : []),
      ...(m?.function_call ? [m.function_call] : []),
    ];
    for (const c of calls) {
      n += String(c?.function?.name ?? c?.name ?? '').length;
      const args = c?.function?.arguments ?? c?.arguments;
      n += typeof args === 'string' ? args.length : JSON.stringify(args ?? '').length;
    }
  }
  return n;
}
...
/**
 * Live defaults. Env still overrides.
 *
 * MEASURED 2026-08-19 on test/spill-agent-fixture.js (250k ledger, 105 msgs,
 * last 15 = fat Read/Bash of already-bound files + npm test + grep + ask):
 *
 *   stub  maxCh  minT  sentChars  ratio  ask  nonFile
 *   no     6000     6     154401   1.62  yes  yes      ← live 1.2–1.5x shape
 *   yes    *        2        182  1374   yes  no       ← starved
 *   yes    *        4      30299   8.25  yes  yes
 *   yes    6000     6      34451   7.26  yes  yes      ← sweet spot
 *
 * TAIL_MAX_CHARS does not move the ratio once a 40k tool_result blows the
 * budget — MIN_TURNS does. Stubbing alone lands 5–10x; do not also starve
 * the tail (minTurns 2 drops npm test). keep 6000 / 6.
 */
export const SPILL_DEFAULTS = {
  keepTailMsgs: 8,
  tailMaxChars: 6000,
  tailMinTurns: 6,
};

export function spillTailOptions(overrides = {}) {
  const num = (v, fallback) => {
    const n = Number(v);
    return Number.isFinite(n) && n > 0 ? n : fallback;
  };
  return {
    keepTailMsgs: num(overrides.keepTailMsgs ?? process.env.OPENZOO_KEEP_TAIL_MSGS, SPILL_DEFAULTS.keepTailMsgs),
    tailMaxChars: num(overrides.tailMaxChars ?? process.env.OPENZOO_TAIL_MAX_CHARS, SPILL_DEFAULTS.tailMaxChars),
    tailMinTurns: num(overrides.tailMinTurns ?? process.env.OPENZOO_TAIL_MIN_TURNS, SPILL_DEFAULTS.tailMinTurns),
  };
}

/**
 * Cut + optional file-body stub. Shared by the proxy and the agent-shaped
 * ratio sweep so the test measures the same tail the request forwards.
 */
export function planSpillTail(msgs, {
  keepTailMsgs,
  tailMaxChars,
  tailMinTurns,
  boundFiles,
  ...
  ({ keepTailMsgs, tailMaxChars, tailMinTurns } = spillTailOptions({
    keepTailMsgs, tailMaxChars, tailMinTurns,
  }));
  const empty = (reason) => ({
    ok: false,
    reason,
    ...
    corpusChars: 0,
    forwarded: msgs,
    sentChars: forwardedChars(msgs),
    sentTokensApprox: Math.round(forwardedChars(msgs) / 4),
    stubbed: 0,
    dropped: 0,
    lastAsk: '',
    lastAskKept: false,
  ...
  if (!Array.isArray(msgs) || msgs.length < 6) return empty('under-6');

  const keepTail = Math.min(keepTailMsgs, Math.max(2, Math.floor(msgs.length / 2)));
  const firstSpillable = msgs.findIndex((m) => m?.role !== 'system');
  if (firstSpillable < 0) return empty('no-system');
  ...
  if (cut <= firstSpillable) return empty('no-severable-cut');
  ...
  let tailStart = cut;
  {
    ...
    for (let i = msgs.length - 1; i >= cut; i--) {
      ...
      if (used > tailMaxChars && severable(i)) { tailStart = i; break; }
    }
  }
  ...
  if (realTurns(cut) < tailMinTurns) {
    for (let i = cut - 1; i > firstSpillable; i--) {
      if (severable(i) && realTurns(i) >= tailMinTurns) { cut = i; break; }
  ...
  const head = msgs.slice(0, firstSpillable);
  const corpus = msgs.slice(firstSpillable, cut).map(msgText).filter(Boolean).join('\n\n');
  const lastAsk = lastUser >= 0
    ? (typeof msgs[lastUser].content === 'string' ? msgs[lastUser].content : msgText(msgs[lastUser]))
    : '';
  const rewritten = stub
    ? stubBoundFileResults(msgs, { boundFiles, boundAbs, cwd, fromIndex: cut })
    : { messages: msgs, stubbed: 0, dropped: 0 };
  const tail = rewritten.messages.slice(cut);
  const forwarded = [...head, ...tail];
  const sentChars = forwardedChars(forwarded);
  return {
    ok: true,
    reason: null,
    cut,
    ...
    forwarded,
    sentChars,
    sentTokensApprox: Math.round(sentChars / 4),
    stubbed: rewritten.stubbed,
    dropped: rewritten.dropped,
    lastAsk,
    lastAskKept: Boolean(lastAsk) && lastUser >= cut && forwarded.some((m) => (
      m.role === 'user' && (m.content === lastAsk || (typeof m.content === 'string' && m.content.includes(lastAsk)))
    )),
    realTurns: realTurns(cut),
    lastUser,
  };
}
