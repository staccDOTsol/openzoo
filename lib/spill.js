...
import { spillRetrieve } from './retrieve.js';
 ...
 */
function withSpillRetrieve(decision, opts = {}) {
  if (opts.retrieve === false) return decision;
  const retrieve = spillRetrieve(decision, opts.retrieveOpts);
  if (retrieve) decision.retrieve = retrieve;
  return decision;
}
...
export function decideChatSpill(body, opts = {}) {
  ...
  let minChars = opts.minPrefixChars ?? SPILL_MIN_PREFIX_CHARS;
  const dx = Number(opts.dollarX);
  const model = String(body?.model || '').trim().toLowerCase();
  const auto = model === 'openzoo/auto' || model === 'openzoo-auto' || model === 'auto';
  // Thin HUD / Auto: bind way earlier than 16k. 2.1x session is cheap-model
  // vs cheap-model because the prefix never cleared the gate.
  if (auto || (Number.isFinite(dx) && dx > 0 && dx < 5)) {
    minChars = Math.min(minChars, 2048);
  }
