...
export function buildMcpServer() {
  ...
  const server = new McpServer({ name: 'openzoo', version: pkg.version }, {
    instructions: [
      ...
      'IMAGE AND VIDEO ARE SERVED, on their own tools — zoo_ask cannot emit them.',
      '  zoo_image        synchronous, returns hosted URLs (FLUX, Imagen, Seedream, Qwen-Image, gpt-image…)',
  ...
  server.registerTool('zoo_ask', {
    ...
    try {
      ...
      let data; let receipt; let reuse = null; let retrieve = null;
      ...
      if (context_id) {
        const { response, receipt: r } = await client.fetch(`${config.apiBase}/v1/chat/completions`, {
          ...
          headers: {
            ...
            ...withNamespace(hrrAskHeaders({ contextId: context_id })),
          },
      ...
      if (corpus && corpus.length > BIND_MIN_CHARS && !contextCacheDisabled()) {
        ...
        ({ data, receipt } = out);
        retrieve = out.retrieve || retrieveBoundCorpus(prompt, corpus);
      ...
      return text({
        ...
        ...(retrieve ? {
          retrieve: {
            ...
            shortlistSize: retrieve.shortlistSize,
            bm25Docs: retrieve.bm25Docs,
            certificate: retrieve.certificate,
          },
        } : {}),
      });
    } catch (err) {
   ...
   * never engage — and implying otherwise would sell holographic memory as
  ...
  server.registerTool('zoo_image', {
    description:
      'Generate an IMAGE through openzoo.fun, paid per call from the local burner wallet via x402 — no '
  ...
  server.registerTool('zoo_video', {
    description:
      ...
      + 'ASYNCHRONOUS: this returns a job id immediately — poll zoo_video_result with that id until status '
      ...
      + 'than image — check the quote in the receipt.',
    inputSchema: {
  ...
  server.registerTool('zoo_video_result', {
    description:
      'Fetch the state of a zoo_video job. FREE — the render was already paid for at submit, so poll as often '
      + 'as you like. status is "in_progress", "completed" or "failed"; on completion outputs.video_url holds '
      + 'the clip and outputs.cost the real upstream charge. A failed job reports why in error.message — note '
