...
const ENTRYPOINT = [
  ...
  // /workspace is an EMPTY MOUNT that hides anything baked there, so /opt is
  // the source of truth and this is a copy, not an install. The "starts with
  // 429" test heals a box whose persisted volume was poisoned by the old
  // fetch-at-boot path.
  + 'if [ ! -f /workspace/.oz-app/bin/openzoo.js ]; then mkdir -p /workspace/.oz-app; cp -a /opt/openzoo/. /workspace/.oz-app/; fi; '
  + 'mkdir -p /workspace/.grokui; '
  + 'if [ ! -f /workspace/.grokui/grokui.mjs ] || head -c 3 /workspace/.grokui/grokui.mjs | grep -q 429; then cp /opt/grokui/grokui.mjs /opt/grokui/podagent.mjs /workspace/.grokui/; fi; '
  + 'OPENZOO_BIND=0.0.0.0 node /workspace/.oz-app/bin/openzoo.js > /var/log/openzoo/proxy.log 2>&1 & '
  + 'OZ_GROKUI_BIND=0.0.0.0 OZ_GROKUI_PORT=4173 node /workspace/.grokui/grokui.mjs > /var/log/openzoo/grokui.log 2>&1 & '
