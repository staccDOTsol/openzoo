...
/** nvm Node bin dirs, v24 first. Finder-launched grokui has no ~/.nvm on PATH. */
export function nvmNodeBinDirs(home = os.homedir(), env = process.env) {
  const dirs = [];
  if (process.platform === 'win32' || env.NVM_HOME) {
    const nvmHome = env.NVM_HOME
      || path.join(env.APPDATA || path.join(home, 'AppData', 'Roaming'), 'nvm');
    try {
      for (const d of fs.readdirSync(nvmHome)) {
        if (/^v?24\b/.test(d)) dirs.push(path.join(nvmHome, d));
      }
  ...
  try {
    ...
    const names = fs.readdirSync(versions).filter((d) => /^v\d+/.test(d));
    const prefer24 = names.filter((d) => /^v24\b/.test(d));
    const rest = names.filter((d) => !/^v24\b/.test(d));
    for (const d of [...prefer24, ...rest]) {
      dirs.push(path.join(versions, d, 'bin'));
    }
  } catch { /* no nvm */ }
  return dirs;
}

/** Extra bin dirs (~/.local/bin, nvm 24, Homebrew). Finder-launched
...
export function claudeCodeBinDirs(home = os.homedir(), env = process.env) {
  return [
    path.join(home, '.local', 'bin'),
    ...nvmNodeBinDirs(home, env),
    '/opt/homebrew/bin',
    '/usr/local/bin',
  ];
}
...
function pathSearchDirs(env = process.env) {
  const extras = env.OPENZOO_CLAUDE_PATH_ONLY === '1'
    ? []
    : claudeCodeBinDirs(env.HOME || os.homedir(), env);
...
export function claudeZooEnv(baseEnv = process.env, { base, port } = {}) {
  // 127.0.0.1 not localhost: win32 fetch('http://localhost') can hit ::1
  // while the sidecar binds 127.0.0.1 — undici then throws "fetch failed".
  const url = base || `http://127.0.0.1:${port ?? config.port}/v1`;
  const env = { ...baseEnv };
  delete env.ANTHROPIC_API_KEY;
  env.ANTHROPIC_BASE_URL = url;
  env.ANTHROPIC_AUTH_TOKEN = zooAuthToken(baseEnv);
  const extras = baseEnv.OPENZOO_CLAUDE_PATH_ONLY === '1'
    ? []
    : claudeCodeBinDirs(baseEnv.HOME || os.homedir(), baseEnv).filter((d) => {
