...
/** Dirs Claude's install.sh uses, plus Homebrew / nvm-windows.
 *  Finder-launched grokui has no ~/.zshrc PATH, so PATH-only lookup
 *  misses `~/.local/bin/claude`. Windows uses nvm-windows, not unix nvm. */
...
export async function launchClaude(argv) {
  if (argv.includes('--setup')) {
    // First-run / grokui Auto: point Claude at the local proxy without
    // launching the TUI or waiting on :8402. Same writer as a full `openzoo claude`.
    const { applyOpenzooClaudeSetup } = await import('./harness-install.js');
    applyOpenzooClaudeSetup({ env: process.env, port: config.port });
    console.error('openzoo: Claude Auto pointed at the local proxy (no Anthropic login).');
    return;
  }
  ...
  const rest = argv.filter((a) => !['--terminal', '-t', '--desktop', '--setup'].includes(a));
