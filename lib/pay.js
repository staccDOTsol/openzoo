...
import { config, fundingLine, evmRpcFor, evmRpcForNetwork } from './config.js';
...
export class BalanceUnavailableError extends Error {
  constructor() { super('We couldn’t check your balance right now. Please try again. You don’t need to add funds again.'); }
}
 ...
 * worse: `openzoo balance` is live, so a KISS top-up shows in the CLI while
...
/**
 * HARD CEILING ON STALENESS — the SWR branch has no age bound of its own.
 *
 * It returns the cached figure and merely KICKS OFF a refresh, so a positive
 * entry was served forever whenever the refresh kept losing: an RPC that keeps
 * throwing leaves `refreshing` false and the old number in place, and several
 * workers sharing one wallet never see each other's spends at all — each
 * process only ever debits what IT paid.
 *
 * MEASURED 2026-09-03: deposit wallet 4QtgGwMt… holding 0.003603 USDC kept
 * signing the 0.009204 USDC row. Every settle came back `Error: insufficient
 * funds` (Tokenkeg custom program error 0x1) in a tight retry loop, while the
 * KISS row it could pay (47,411 held vs 19.28 needed) sat next to it in the
 * same accepts[]. Past this ceiling, block on a live read instead of guessing.
 */
const BALANCE_STALE_MAX_MS = Number(process.env.OPENZOO_BALANCE_STALE_MAX_MS || 60_000);
const balanceCache = new Map();   // key -> { raw, ui, at, refreshing }

const balKey = (owner, mint) => `${owner}|${mint}`;

/** Drop a cached figure so the next read is live. Called when the gateway
 *  refuses a payment we already signed — the cache is what mispriced it. */
export function forgetCachedBalance(owner, mint) {
  balanceCache.delete(balKey(owner?.toBase58 ? owner.toBase58() : String(owner), mint));
}
...
async function cachedTokenBalance(connection, owner, mint, { force = false } = {}) {
  ...
  const fresh = hit && !force && !zero && age < BALANCE_TTL_MS;
  if (hit && !force && !zero && age < BALANCE_STALE_MAX_MS) {
...
/**
 * One line to stderr about which rail was chosen or skipped. Quiet by default
 * would defeat the point — this is the answer to "why did my deposit not get
 * used", and it is only printed when a rail was actually passed over, so a
 * normal single-rail call stays silent. OPENZOO_QUIET=1 turns it off.
 */
function railNote(msg) {
  if (process.env.OPENZOO_QUIET === '1') return;
  try { process.stderr.write(`openzoo: ${msg}\n`); } catch { /* stderr closed */ }
}

/** "Base (eip155:8453)" — the name a human knows plus the id a log needs. */
function railLabel(accept) {
  const n = chainName(accept?.network);
  return n === accept?.network ? String(n) : `${n} (${accept.network})`;
}

/** A chain a human recognises, for funding instructions. */
const CHAIN_NAMES = { 1: 'Ethereum', 10: 'Optimism', 137: 'Polygon', 480: 'World Chain', 4663: 'Robinhood Chain', 8453: 'Base', 42161: 'Arbitrum' };
export function chainName(network) {
  const m = /^eip155:(\d+)$/.exec(String(network || ''));
  return (m && CHAIN_NAMES[Number(m[1])]) || network;
}

const memoKey = (a) => `${a?.network || ''}|${a?.asset || ''}`;

/** Prefer a confirmed successful payer; recheck other balances on every request. */
export function orderCandidatesByMemory(cands) {
  const good = (c) => lastGoodAsset && memoKey(c) === lastGoodAsset;
  return [...cands].sort((a, b) => good(b) - good(a));
}
...
export class PayClient {
  ...
  async balanceForAccept(accept) {
    ...
    // By chain id, for the reason spelled out in buildPaymentFor: the rail name
    // cannot tell one non-Base EVM chain from another.
    const rpcUrl = evmRpcForNetwork(accept.network) ?? evmRpcFor(rail);
    if (!rpcUrl) return 0n;
    const raw = await evmTokenBalance({
      rpcUrl, token: accept.asset, owner: this.evmAddress,
    });
    return BigInt(raw || 0);
  }
   ...
   * wallet holding 845,486 plain KISS read as "$3.14 covered" — the sliver
  ...
  async buildPaymentFor(accept, onStage, challenge) {
    ...
    if (rail === 'base' || rail === 'evm' || (rail === 'robinhood' && (this.allowRH || config.rail === 'robinhood'))) {
      ...
      //
      // THE RPC IS CHOSEN BY THE ROW'S CHAIN ID, NEVER BY ITS RAIL NAME.
      // railOf() calls every chain that is not Base or Robinhood "evm", so the
      // old `rail === 'base' ? baseRpc : rhRpc` sent a POLYGON row's balance
      // read to Robinhood Chain, where that token address is not a contract.
      // The read threw, the catch below swallowed it, and this signed an
      // authorization for a chain holding nothing — the facilitator reverted
      // it and the caller was told "execution reverted" with a funded Base
      // wallet one row away. MEASURED 2026-09-09.
      const rpcUrl = evmRpcForNetwork(accept.network);
      const owner = this.evmAddress;
      // A CHAIN WE CANNOT READ IS NOT A CHAIN WE SIGN FOR. Skipping the row
      // costs a caller nothing — the walk moves to the next one and reports
      // honestly if none is payable — while signing it blind spends the call
      // on a revert and teaches the caller nothing about why.
      if (owner && !rpcUrl) {
        throw new UnderfundedError(accept, null, owner, {
          line: `No RPC configured for ${accept.network}, so this wallet's balance there cannot be checked — skipping that rail rather than signing blind. Set OPENZOO_POLYGON_RPC (or the matching chain's RPC) to use it.`,
        });
      }
      if (owner) {
        const bal = await evmTokenBalance({ rpcUrl, token: accept.asset, owner }).catch(() => { throw new BalanceUnavailableError(); });
        if (bal !== null && bal < BigInt(accept.maxAmountRequired)) {
          ...
          const where = rail === 'robinhood' ? 'Robinhood Chain' : rail === 'base' ? 'Base' : chainName(accept.network);
          throw new UnderfundedError(accept, null, owner, {
  ...
  async fetch(url, init = {}, { onStage } = {}) {
    ...
    for (const cand of candidates) {
      ...
      try {
        ...
        if (fundErrs.length) railNote(`paying on ${railLabel(cand)}`);
        accept = cand;
        ...
        if (e instanceof BalanceUnavailableError) { balanceError = e; continue; }
        ...
        // SAY WHICH RAIL WAS PASSED OVER AND WHY.
        //
        // A caller whose wallet is funded on one chain and empty on five
        // others sees only the LAST row's failure, and the chain that failed
        // is not the chain they funded. Reading that as "my deposit is not
        // seen" is the correct reading of what they were shown — the walk was
        // silent about the five it skipped. One line per skip, on stderr,
        // makes the choice inspectable without a flag or a debug build.
        railNote(`skipping ${railLabel(cand)} — ${e.message.replace(/^openzoo wallet underfunded: /, '')}`);
      }
    }
    if (!payment) {
    ...
    let response = await fetchHeaders(url, {
      ...init,
      headers: withNamespace({ ...stripAuthorization(init.headers || {}), ...paymentHeaders(payment.header) }, { keypair: this.keypair }),
    });
      ...
      for (const cand of plain) {
        try {
          ...
          const again = await fetchHeaders(url, {
            ...init,
            headers: withNamespace({ ...stripAuthorization(init.headers || {}), ...paymentHeaders(retry.header) }, { keypair: this.keypair }),
          });
          if (again.status !== 402) {
            accept = cand; payment = retry; response = again;
            if (again.ok) lastGoodAsset = memoKey(cand);
            break;
          }
        } catch (e) {
          if (!(e instanceof UnderfundedError) && !(e instanceof BalanceUnavailableError)) throw e;
        }
      }
    }
    ...
    if (response.status === 402 && !this._rebuilt) {
      this._rebuilt = true;
      // The gateway just refused a payment we signed. If the reason was funds
      // rather than a dead blockhash, the cached balance is exactly what got it
      // wrong — drop it so the rebuild re-reads live and the candidate walk can
      // fall through to a rail the wallet can actually pay.
      try { forgetCachedBalance(this.keypair?.publicKey, accept?.asset); } catch { /* advisory */ }
      try {
    ...
    if (response.ok) lastGoodAsset = memoKey(accept);
    else if (lastGoodAsset === memoKey(accept)) lastGoodAsset = null;
    ...
    // (slot 441794097, 0.147837 KISS moved). The gateway's chat path did not
