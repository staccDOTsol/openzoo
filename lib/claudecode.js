...
import { ensureHarness, getHarnessState, shouldSkipHarnessAutostart } from './harness-install.js';
...
export const CLAUDE_MISSING = 'Claude Auto is still installing Claude, Node 24, and openzoo. '
  + 'Orange Auto works when that finishes — the app runs the install; do not paste a shell recipe. '
...
async function attachSession({ cli, cwd, sessionId, model, system, env, key }) {
  const resume = sessionId || latestClaudeSessionId(cwd);
  const args = claudeInteractiveArgs({ sessionId: resume, model, system });
...
export async function runClaudeCode({
  ...
  let cli = resolveClaudeCli(zooEnv);
  if (!cli && !shouldSkipHarnessAutostart(env) && env.OPENZOO_CLAUDE_PATH_ONLY !== '1') {
    const harness = await ensureHarness({
      env: zooEnv,
      port,
      onStatus: onEvent
        ? (s) => { try { onEvent({ kind: 'status', text: s.message }); } catch { /* paint */ } }
        : undefined,
    });
    cli = resolveClaudeCli(claudeZooEnv(env, { port }));
    if (!cli) {
      const failed = getHarnessState().error
        ? 'Could not install Claude Auto. The app tried the official Claude installer, Node 24, and openzoo. Retry orange Auto.'
        : CLAUDE_MISSING;
      return { text: failed, error: true, paymentFailed: '', sessionId: sessionId || '', missing: true, harness };
    }
  }
  if (!cli) {
  ...
  if (!sess || sess.dead) {
    try {
      sess = await attachSession({
        cli, cwd, sessionId: wantResume, model, system, env: zooEnv, key,
      });
    } catch (e) {
      return {
  ...
  try {
    ...
    if (sess.dead && wantResume && !signal?.aborted) {
      ...
      try {
        sess = await attachSession({
          cli, cwd, sessionId: sess.sessionId || wantResume, model, system, env: zooEnv, key,
        });
        liveSessions.set(key, sess);
        if (prompt != null && String(prompt) !== '') sess.writeLine(prompt);
        await waitIdle(sess, { signal });
