...
export async function runClaudeCode({
  ...
  const resolved = resolvePackedOpenzooClaude({ env: zooEnv }) || resolveOpenzooClaude(zooEnv);
  if (!resolved) {
  ...
  if (currentPlatform() === 'win32' && !loadNodePty()) {
    return { text: PTY_PENDING, error: true, paymentFailed: '', sessionId: sessionId || '', ptyPending: true };
  }
