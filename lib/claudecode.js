 ...
 * (ANTHROPIC_BASE_URL=http://127.0.0.1:8402/v1, ANTHROPIC_AUTH_TOKEN=sk-openzoo,
 * ANTHROPIC_API_KEY unset). cwd is the thread dir. bypassPermissions stays.
 *
 * Attach order: packed/rebuilt node-pty (every dmg/exe/AppImage ships it),
 * else Mac/Linux `script` which allocates a host PTY when node-pty failed to
 * load AND a host PTY exists. Never fall back to --print. Never paint an
 * install recipe on the canvas.
 */
...
import {
  CANVAS_PTY_RECIPE, findPackedNodePty, loadNodePtyFrom, resolvePackedOpenzooClaude,
...
/** Packed PTY is still copying. Never an install recipe on the canvas. */
export const PTY_PENDING = 'Auto is starting…';

/** Missing harness is the same pending state — Auto never falls through to chat. */
export const CLAUDE_MISSING = PTY_PENDING;

let ptyLoaderOverride = null;
let platformOverride = null;

export function setNodePtyLoaderForTest(fn) {
  ptyLoaderOverride = typeof fn === 'function' ? fn : null;
}
export function setPtyPlatformForTest(p) {
  platformOverride = p == null ? null : String(p);
}
...
const HARNESS_DUMP_LINE = /^(?:RUN|WRITE|SPAWN|READ|EDIT|GLOB|GREP|MULTIEDIT|LS):\s*.*$/gim;
const TOOL_USE_TAG = /<tool-use\b[^>]*>[\s\S]*?<\/tool-use>/gi;
const TOOL_USE_OPEN = /<tool-use\b[^>]*>[\s\S]*$/i;
/** Claude TUI / Grok-Bot tool trail — never the user canvas. */
const TOOL_RUNNING_LINE = /^\s*\[(?:LS|Read|Write|Edit|Bash|Glob|Grep|Task|Agent|NotebookEdit|MultiEdit|WebFetch|WebSearch|TodoWrite)\]\s*running\b.*$/gim;
const TOOL_RUNNING_ONE = /^\s*\[(?:LS|Read|Write|Edit|Bash|Glob|Grep|Task|Agent|NotebookEdit|MultiEdit|WebFetch|WebSearch|TodoWrite)\]\s*running\b/i;
...
export function isToolRunningLine(line) {
  return TOOL_RUNNING_ONE.test(String(line || '').trim());
}
...
export function stripClaudeNoise(text) {
  ...
  s = s.replace(TOOL_USE_TAG, '');
  s = s.replace(TOOL_USE_OPEN, '');
  s = s.replace(CURRENT_DIR_LINE, '');
  s = s.replace(HARNESS_DUMP_LINE, '');
  s = s.replace(TOOL_RUNNING_LINE, '');
  return s.replace(/^\n+|\n+$/g, '').trim();
}
...
export function sanitizeClaudeCanvas(text, { error = false } = {}) {
  ...
  if (CANVAS_PTY_RECIPE.test(s)) return '';
  return s;
}
...
export function foldTuiText(raw) {
  ...
  for (const line of plain.split('\n')) {
    ...
    if (isToolRunningLine(trimmed)) continue;
...
function loadNodePty() {
  if (ptyLoaderOverride) {
    try { return ptyLoaderOverride(); } catch { return null; }
  }
  try {
...
/** Wait for packed / harness-copied node-pty. Never dump an install recipe. */
export async function waitForPackedPty({ timeoutMs, env = process.env } = {}) {
  const cap = Math.max(0, Number(timeoutMs ?? env.OZ_PTY_WAIT_MS ?? 8_000));
  const start = Date.now();
  let pty = loadNodePty();
  if (pty) return pty;
  try {
    const { copyPackedHarness } = await import('./harness-install.js');
    copyPackedHarness({ env });
  } catch { /* copy is best-effort */ }
  pty = loadNodePty();
  if (pty || cap === 0) return pty;
...
export function spawnClaudePty({ cli, args, cwd, env }) {
  ...
  if (currentPlatform() === 'win32') {
    const err = new Error(PTY_PENDING);
...
export function hasLiveClaudeSession(key) {
  const sess = liveSessions.get(key);
  return Boolean(sess && !sess.dead);
}
...
function sessionIdle(sess) {
  // Kept for tests / callers that still ask "is the TUI showing a prompt".
  // waitIdle must not treat this as send-complete.
  return tuiLooksIdle(sess.term.text()) || tuiLooksIdle(sess.screenText().text);
}

/**
 * Wait until this PTY send is actually over.
 * Idle prompt + visible text is NOT "goal done" — a question or one-shot
 * dummy write must not complete the grokui send while the session is alive.
 * Finish on: child exit / result, abort, or the wall-clock hard cap.
 * Spinner / think / keepFold must not reset the hard cap.
 * A writeLine that never paints (no activity, no text) settles at promptWait.
 */
export function waitIdle(sess, {
  ...
  return new Promise((resolve) => {
    ...
    const tryDone = () => {
      // Alive + idle + text is not done. The harness (liveSessions) keeps looping.
      if (sess.dead || sawResult) {
        finish();
        return true;
      }
      return false;
    ...
    hardTimer = setTimeout(finish, hard);
...
async function resolveClaudeOrPending(zooEnv) {
  const once = () => resolvePackedOpenzooClaude({ env: zooEnv }) || resolveOpenzooClaude(zooEnv);
  let resolved = once();
  if (resolved) return resolved;
  const tries = zooEnv.OPENZOO_CLAUDE_PATH_ONLY === '1'
    ? 1
    : Math.max(1, Number(zooEnv.OZ_CLAUDE_RESOLVE_TRIES || 4));
  const delay = Math.max(0, Number(zooEnv.OZ_CLAUDE_RESOLVE_MS || 200));
...
export async function runClaudeCode({
  ...
  if (runnerOverride) {
    return runnerOverride({
      prompt, cwd, sessionId, model, system, env, port, onEvent, signal, sessionKey, waitIdleMs, stayLive,
    });
  }
  const zooEnv = claudeZooEnv(env, { port });
  const resolved = await resolveClaudeOrPending(zooEnv);
  if (!resolved) {
    return {
      text: PTY_PENDING, error: true, paymentFailed: '', sessionId: sessionId || '',
      missing: true, ptyPending: true, live: false,
    };
  }
  const cli = resolved.command;
  const prefixArgs = resolved.prefixArgs;
  if (!loadNodePty()) {
    await waitForPackedPty({ env: zooEnv });
  }
  if (currentPlatform() === 'win32' && !loadNodePty()) {
    return {
      text: PTY_PENDING, error: true, paymentFailed: '', sessionId: sessionId || '',
      ptyPending: true, live: false,
    };
  }
  ...
  if (!sess || sess.dead) {
    try {
      ...
      const text = CANVAS_PTY_RECIPE.test(msg) || e.code === 'PTY_PENDING'
        ? PTY_PENDING
        : `could not launch claude: ${msg}`;
      return {
        text: CANVAS_PTY_RECIPE.test(text) ? PTY_PENDING : text,
        error: true,
        paymentFailed: '',
        sessionId: wantResume,
        ptyPending: text === PTY_PENDING,
        live: false,
      };
    }
  ...
  try {
    ...
    // Stay subscribed on a still-alive session so the harness can keep looping.
    // Each waitIdle is hard-capped; grokui Auto passes stayLive: true.
    while (stayLive && !sess.dead && !signal?.aborted) {
      await waitIdle(sess, { signal, hardCapMs: waitIdleMs });
    }
