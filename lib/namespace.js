 ...
 * nothing at all.
 *
 * BREAKING: old namespaces stay isolated. Re-bind their corpora under the
 * authenticated namespace; there is no shared-memory fallback.
 */
export const STACC_NAMESPACE = 'stacc';

export function namespaceHeaderValue(wallet) {
  // A failed signer must fail the operation; never send an unsigned request.
  if (wallet !== undefined && !wallet?.keypair) throw new Error('The supplied wallet cannot sign.');
...
export function withNamespace(headers = {}, wallet) {
  ...
  if (!ns) throw new Error('A wallet-signed namespace is required; shared memory is disabled.');
  // The gateway rejects an unsigned namespace. Signing failure must stop
  // the operation locally instead of sending an unauthenticated request.
  const clean = Object.fromEntries(new Headers(headers).entries());
  for (const name of Object.keys(clean)) {
    if (name === 'x-openzoo-session' || name.startsWith('x-openzoo-namespace')) delete clean[name];
  }
  return { ...clean, 'x-openzoo-namespace': ns, ...signNamespace(ns, wallet) };
}
...
/** Price discovery must never consume the operator's existing gateway credit. */
export function withQuoteNamespace(headers = {}) {
  return withNamespace(headers, { keypair: Keypair.generate() });
}
