...
export async function startChatGptSpend({ profile, proxyPort = 8402, spawnImpl = spawn } = {}) {
  const app = fileURLToPath(new URL('../launcher/', import.meta.url));
  const electron = path.join(app, 'node_modules/electron/dist', process.platform === 'darwin'
    ? 'Electron.app/Contents/MacOS/Electron' : process.platform === 'win32' ? 'electron.exe' : 'electron');
  const env = { ...process.env, OPENZOO_SAVINGS_PORT: String(proxyPort) };
  delete env.ELECTRON_RUN_AS_NODE;
  const readyDir = fs.mkdtempSync(path.join(os.tmpdir(), 'openzoo-pill-'));
  env.OPENZOO_SAVINGS_READY = path.join(readyDir, 'ready');
  const logDir = profile || path.join(os.homedir(), '.openzoo');
  fs.mkdirSync(logDir, { recursive: true });
  const logPath = path.join(logDir, 'openzoo-savings.log');
  const fd = fs.openSync(logPath, 'a', 0o600);
  let child, failure;
  try {
    if (fs.existsSync(electron)) {
      child = spawnImpl(electron, [app, '--savings-only'], { detached: true, stdio: ['ignore', fd, fd], env });
    } else {
      // npm caches the standalone display runtime. The CLI ships the display itself.
      // No ChatGPT debugging port or separate OpenZoo installer is required.
      const args = ['exec', '--yes', '--package=electron@44.3.0', '--', 'electron', app, '--savings-only'];
      const npmScript = process.env.npm_execpath;
      child = npmScript && fs.existsSync(npmScript)
        ? spawnImpl(process.execPath, [npmScript, ...args], { detached: true, stdio: ['ignore', fd, fd], env })
        : spawnImpl(process.platform === 'win32' ? 'npm.cmd' : 'npm', args, { detached: true, stdio: ['ignore', fd, fd], env, shell: process.platform === 'win32' });
    }
    child.once('error', error => { failure = error; });
    child.once('exit', code => { if (code) failure = new Error(`Display exited (${code})`); });
    child.unref();
    const deadline = Date.now() + 120000;
    while (!fs.existsSync(env.OPENZOO_SAVINGS_READY)) {
      if (failure) throw failure;
      if (Date.now() > deadline) throw new Error(`Display did not open. See ${logPath}`);
      await new Promise(resolve => setTimeout(resolve, 200));
    }
  } finally { fs.closeSync(fd); fs.rmSync(readyDir, { recursive: true, force: true }); }
}
