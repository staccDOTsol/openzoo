 ...
 *
 * Auto (`openzoo/auto` and aliases): always pass through as openzoo/auto.
 * Fly owns classification and 429 retry. Do not pin Auto to flash, even
 * when isTinyClassify is true (short body, max_tokens=16 or 16384).
 *
 * Tiny classify (non-Auto only): pin to a fast non-reasoning catalog id,
 * leave max_tokens alone, ignore OPENZOO_DEFAULT_MODEL. Never fall back to
 * `from` when that id is a reasoner or a heavy/flagship (the zoo default
 * is opus-5). A catalog miss or an opus-only list used to keep the
 * classify on opus-5 and AUTO hard-blocked Bash. Everything else:
 * resolveModel (which honours the default) then the reasoning floor.
 */
export function rewriteChatModel(parsed, ids, { bodyLen } = {}) {
  ...
  if (isAutoModel(from)) {
    const next = parsed?.model === AUTO_MODEL_ID ? parsed : { ...parsed, model: AUTO_MODEL_ID };
    return { parsed: next, auto: true, tiny: false, from, to: AUTO_MODEL_ID, raised: false };
  }
