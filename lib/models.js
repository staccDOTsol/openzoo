...
export function unopenrouter(requested, ids) {
  ...
  // Telnyx advertises `zai-org/GLM-5.2`, not OpenRouter's `z-ai/glm-5.2`.
  const want = bare.toLowerCase().replace(/[._]+/g, '-');
  const hit = ids.find((id) => {
    if (id === s) return false;
    const suffix = String(id).slice(String(id).lastIndexOf('/') + 1).toLowerCase().replace(/[._]+/g, '-');
    return suffix === want;
  });
  return hit || null;
}
