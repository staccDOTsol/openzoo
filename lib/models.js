...
import { AUTO_MODEL_ID, autoModelListEntry, isAutoModel } from './modelroute.js';

export { AUTO_MODEL_ID, isAutoModel };
...
export function augmentModelList(payload) {
  ...
  const virtual = [];
  if (!have.has(AUTO_MODEL_ID)) {
    virtual.push(autoModelListEntry());
    have.add(AUTO_MODEL_ID);
  }
  return { ...payload, object: payload?.object || 'list', data: [...virtual, ...data, ...branded, ...aliases] };
}
