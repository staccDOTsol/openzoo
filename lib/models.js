...
import { fetchProbe } from './fetch.js';
import {
...
export async function zooModelIds() {
  ...
  const r = await fetchProbe(`${config.apiBase}/v1/models`);
...
function messageHasToolCalls(m) {
  if ((Array.isArray(m?.tool_calls) && m.tool_calls.length) || m?.function_call || m?.role === 'tool') {
    return true;
  }
  const c = m?.content;
  if (Array.isArray(c) && c.some((b) => b && (b.type === 'tool_use' || b.type === 'tool_result'))) {
    return true;
  }
  return false;
}
...
export function isTinyClassify(body, bodyLen) {
  ...
  // Named model + large max_tokens is a real chat, not a classify nub.
  // Cheap race / model pick stay Auto-only.
  const mdl = String(parsed?.model || '');
  if (mdl && !isAutoModel(mdl) && mt > CLASSIFY_MAX_TOKENS) return false;
...
export function rewriteChatModel(parsed, ids, { bodyLen } = {}) {
  ...
  // Named catalog ids (especially x-ai/grok-4.6) stay put on a non-tiny turn.
  // Bare Anthropic ids are never live — they still go through resolveModel.
  if (typeof from === 'string' && !isAutoModel(from) && !isTinyClassify(parsed, len)
      && !anthropicNativeAlias(from)
      && Array.isArray(ids) && ids.includes(from)) {
