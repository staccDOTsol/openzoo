...
import { hrrAskHeaders, retrieveBoundCorpus, HRR_DISPATCH_GATE } from './retrieve.js';

export { hrrAskHeaders, HRR_DISPATCH_GATE } from './retrieve.js';
...
export async function askWithContext(client, corpus, bodyObj, { onStage } = {}) {
  ...
  const ask = Array.isArray(bodyObj?.messages)
    ? [...bodyObj.messages].reverse().find((m) => m?.role === 'user' && typeof m.content === 'string')?.content
    : '';
  const retrieve = (ask && corpus)
    ? retrieveBoundCorpus(ask, corpus)
    : null;
  for (let attempt = 0; ; attempt++) {
    const { response, receipt } = await client.fetch(`${config.apiBase}/v1/chat/completions`, {
      ...
      headers: {
        ...
        ...withNamespace(hrrAskHeaders({ contextId: bind.contextId })),
      },
