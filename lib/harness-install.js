 ...
 *
 * The dmg/exe/AppImage already contains `openzoo-claude` + node-pty (Electron
 * ABI) + an Electron-as-node shim. First boot copies those packed bits into
 * ~/.local/bin offline. Network `npm i -g` / npx is not the product path.
 *
...
import {
  findPackedNodePty, findPackedOpenzooClaude, openzooClaudeEntry,
...
export function localPackedDir(home = os.homedir()) {
  return path.join(home, '.openzoo', 'packed');
}

function copyPkgTree(from, dest) {
  if (!from || !existsSync(from)) return false;
  if (path.resolve(from) === path.resolve(dest)) return true;
...
/**
 * Offline copy of packed openzoo-claude + Electron-as-node shim into ~/.local.
 * Product path for first boot. Returns null when nothing packed is present.
 */
export function copyPackedHarness({
  home = os.homedir(),
  env = process.env,
  platform = process.platform,
  electronPath,
  ...
  const packedClaude = findPackedOpenzooClaude({ env });
  const packedPty = findPackedNodePty({ env });
  const packedRoot = localPackedDir(home);
  if (packedPty) copyPkgTree(packedPty, path.join(packedRoot, 'node-pty'));
  if (packedClaude) copyPkgTree(packedClaude, path.join(packedRoot, 'openzoo-claude'));
  const entry = openzooClaudeEntry(packedClaude);
  if (!packedClaude || !entry) return packedPty ? {
    ok: true, via: 'packed', packedPty, packedClaude: packedClaude || null, claudePath: null,
  } : null;
  const bin = localBinDir(home);
  mkdirSync(bin, { recursive: true });
  ...
  const dest = path.join(bin, platform === 'win32' ? 'openzoo-claude.cmd' : 'openzoo-claude');
  if (platform === 'win32') {
    writeWinShim(dest, electron, extraEnv);
    // Rewrite to exec packed entry — writeWinShim only execs the binary.
    writeFileSync(dest, `@echo off\r\nset ELECTRON_RUN_AS_NODE=1\r\n"${electron}" "${entry}" %*\r\n`);
  } else {
    const body = `#!/bin/sh\nexport ELECTRON_RUN_AS_NODE=1\nexec ${JSON.stringify(electron)} ${JSON.stringify(entry)} "$@"\n`;
    writeFileSync(dest, body);
    chmodSync(dest, 0o755);
  }
  const nodePath = resolveHostNode({ env, home, exists });
  const npxPath = resolveHostNpx({ env, home, nodePath, exists });
  ensureLocalNodeNpx({
    home, platform, nodePath, npxPath, electronPath: nodePath ? null : electron,
  });
  return {
    ok: true,
    via: 'packed',
    claudePath: dest,
    packedClaude,
    packedPty: packedPty || findPackedNodePty({ env }),
  };
}
...
async function defaultInstall({
  ...
  const packed = copyPackedHarness({ home, env, electronPath });
  if (packed?.ok) {
    const again = detectHarness({ env, home });
    if (again.claude) return { ok: true, via: 'packed', ...again, ...packed };
  }
