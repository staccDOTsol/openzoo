...
#
#   ./scripts/release-openzoo-bot-mac.sh                 build + sign + notarize
#   ./scripts/release-openzoo-bot-mac.sh --publish       ...and upload to the
#                                            current openzoo-bot-v* tag
#
...
# "Apple could not verify OpenZoo Bot is free of malware" on first launch.
set -euo pipefail
...
# Refuse to pack if grokbot-app is not on openzoo@latest. A caret on 0.x is
# how 1.5.78 shipped last week's sidecar inside the DMG.
node "$REPO_ROOT/scripts/assert-grokui-pin.mjs" grokbot-app \
  || die "grokbot-app must depend on openzoo latest (not ^0.48.x, not an exact leftover)"
...
APP="dist/mac-arm64/OpenZoo Bot.app"
[ -d "$APP" ] || APP="$(find dist -maxdepth 2 -name 'OpenZoo Bot.app' | head -1)"
if [ -d "$APP" ]; then
...
# ---- publish ----------------------------------------------------------------
if [ "$PUBLISH" = 1 ]; then
  TAG="$(git -C "$REPO_ROOT" describe --tags --abbrev=0 --match 'openzoo-bot-v*')"
  ...
  say "not published (pass --publish to upload to the latest openzoo-bot-v* release)"
fi
