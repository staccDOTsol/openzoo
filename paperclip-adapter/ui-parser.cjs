"use strict";
/**
 * UI stdout parser for openzoo_claude (contract v1).
 *
 * The run's stdout is Claude Code stream-json NDJSON — the same protocol the
 * built-in claude_local adapter emits — so this is that adapter's parser,
 * vendored as a self-contained CJS file (the frontend loads this source raw;
 * it cannot resolve package imports). The one divergence: acpx.* events fall
 * through to plain stdout, because this adapter forces the CLI engine.
 */
