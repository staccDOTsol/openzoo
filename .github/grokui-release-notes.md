...
1.6.11: Desktop `#chatHeader` gets a compact Copy / Paste / Dir / Stop bar at the top-left, next to the thread name and cwd. Copy takes the xterm selection if any, else the visible chat selection, and toasts. Paste reads the OS clipboard into `#inp` (Agent does not dump into the PTY — edit, then Enter). Dir is the #88 folder picker, not a second control. Stop writes ESC via `String.fromCharCode(27)` to `/threads/:id/pty`. Not mobile. 1.6.10 (#88) is unchanged: Message stays visible in Agent, Enter is CR via `String.fromCharCode(13)`, xterm `disableStdin`, packed OCC.
