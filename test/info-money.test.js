...
test('USDC counts as on-chain dollars without a quote', () => {
  const { chainUsd, holdings } = priceHoldings([
    ...
    { symbol: 'KISS', ui: 18584, chain: 'solana' },
  ]);
  ...
  assert.equal(formatHoldingMoney(holdings[1]), '18584 KISS');
});

test('402 tokenUsd turns a KISS pile into money', () => {
  const { chainUsd, holdings } = priceHoldings(
    [{ symbol: 'KISS', ui: 18584, chain: 'solana' }],
    { KISS: 0.00022906 },
  );
  assert.ok(Math.abs(chainUsd - 18584 * 0.00022906) < 1e-9);
  assert.match(formatHoldingMoney(holdings[0]), /KISS\s+\(\$4\.26\)/);
});
